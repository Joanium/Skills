---
name: Rust Interop Boundaries
trigger: rust interop boundaries, help with rust interop boundaries, plan rust interop boundaries, improve rust interop boundaries, expert rust interop boundaries
description: Expert-level guidance for rust interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rust Interop Boundaries is an expert programming-language skill for Rust. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Rust Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Rust Interop Boundaries?
- Which abstraction in Rust Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Rust Interop Boundaries first?
- How will Rust Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of Rust Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Rust Interop Boundaries.
- A failure-mode note that explains the sharp edges inside Rust Interop Boundaries.
...
- [ ] The main language-level risk in Rust Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend Rust Interop Boundaries without reverse-engineering hidden contracts.
