---
name: Exploratory QA Deterministic Timing
trigger: exploratory qa deterministic timing, help with exploratory qa deterministic timing, plan exploratory qa deterministic timing, improve exploratory qa deterministic timing, expert exploratory qa deterministic timing
description: Expert-level guidance for exploratory qa deterministic timing with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Deterministic Timing is an expert testing and quality skill for Exploratory QA. The focal concern is Deterministic Timing, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Deterministic Timing needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Deterministic Timing is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Deterministic Timing would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Deterministic Timing depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Deterministic Timing need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Deterministic Timing.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Deterministic Timing should mean.
...
- [ ] The key behavior protected by Exploratory QA Deterministic Timing is explicit.
