---
name: Failure Injection Lab Triage Heuristics
trigger: failure injection lab triage heuristics, help with failure injection lab triage heuristics, plan failure injection lab triage heuristics, improve failure injection lab triage heuristics, expert failure injection lab triage heuristics
description: Expert-level guidance for failure injection lab triage heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Triage Heuristics is an expert testing and quality skill for Failure Injection Lab. The focal concern is Triage Heuristics, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Triage Heuristics needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Triage Heuristics is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Triage Heuristics would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Triage Heuristics depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Triage Heuristics need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Triage Heuristics.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Triage Heuristics should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Triage Heuristics is explicit.
