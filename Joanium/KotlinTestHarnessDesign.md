---
name: Kotlin Test Harness Design
trigger: kotlin test harness design, help with kotlin test harness design, plan kotlin test harness design, improve kotlin test harness design, expert kotlin test harness design
description: Expert-level guidance for kotlin test harness design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kotlin Test Harness Design is an expert programming-language skill for Kotlin. The focal concern is Test Harness Design, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Kotlin Test Harness Design affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Kotlin Test Harness Design?
- Which abstraction in Kotlin Test Harness Design becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Kotlin Test Harness Design first?
- How will Kotlin Test Harness Design behave when multiple contributors evolve it independently?
- Which part of Kotlin Test Harness Design is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Kotlin Test Harness Design.
- A failure-mode note that explains the sharp edges inside Kotlin Test Harness Design.
...
- [ ] The main language-level risk in Kotlin Test Harness Design is named explicitly.
...
- [ ] Future contributors can extend Kotlin Test Harness Design without reverse-engineering hidden contracts.
