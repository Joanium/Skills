---
name: Visual Diff Triage Failure Screenshots
trigger: visual diff triage failure screenshots, help with visual diff triage failure screenshots, plan visual diff triage failure screenshots, improve visual diff triage failure screenshots, expert visual diff triage failure screenshots
description: Expert-level guidance for visual diff triage failure screenshots with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage Failure Screenshots is an expert browser automation skill for Visual Diff Triage. The focal concern is Failure Screenshots, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage Failure Screenshots needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage Failure Screenshots unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage Failure Screenshots is least stable under real conditions?
- How would Visual Diff Triage Failure Screenshots behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage Failure Screenshots capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage Failure Screenshots is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage Failure Screenshots.
...
- [ ] The state model behind Visual Diff Triage Failure Screenshots is explicit.
