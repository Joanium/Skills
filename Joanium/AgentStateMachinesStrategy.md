---
name: Agent State Machines Strategy
trigger: agent state machines strategy, help with agent state machines strategy, plan agent state machines strategy, improve agent state machines strategy, expert agent state machines strategy
description: Expert-level guidance for agent state machines strategy, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---
...
- Use this when Agent State Machines Strategy influences user-visible output, automation, or safety-sensitive decisions.
...
- What decision is Agent State Machines Strategy supposed to improve over the next planning horizon?
- Which constraints are fixed, and which are merely habits that can be challenged?
- Where does failure in Agent State Machines Strategy create the largest technical, business, or trust cost?
...
- A written decision statement for Agent State Machines Strategy.
