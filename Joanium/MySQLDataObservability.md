---
name: MySQL Data Observability
trigger: mysql data observability, help with mysql data observability, plan mysql data observability, improve mysql data observability, expert mysql data observability
description: Expert-level guidance for mysql data observability with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

MySQL Data Observability is an expert data-systems skill for MySQL. The focal concern is Data Observability, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when MySQL Data Observability affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would MySQL Data Observability create if two teams interpreted it differently?
- Which recovery path for MySQL Data Observability is the least understood today?
- How will MySQL Data Observability behave under backfills, reprocessing, or delayed sources?
- What cost pattern in MySQL Data Observability is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside MySQL Data Observability drifted subtly?
...
- A semantic contract note for MySQL Data Observability.
...
- [ ] The meaning of MySQL Data Observability is written in language consumers will actually use.
