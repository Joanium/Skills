---
name: Robotics Fault Recovery Governance
trigger: robotics fault recovery governance, help with robotics fault recovery governance, plan robotics fault recovery governance, improve robotics fault recovery governance, expert robotics fault recovery governance
description: Expert-level guidance for robotics fault recovery governance, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Robotics Fault Recovery Governance is an expert-level specialized systems skill for operating in technical domains where physical, protocol, or computational limits dominate design choices. The emphasis here is on ownership, policy, exception handling, and auditability so the domain does not decay into ad hoc decisions.

## When To Use

- Use this when Robotics Fault Recovery Governance has tight performance, correctness, or hardware constraints.
...
- Who sets the policy for Robotics Fault Recovery Governance, and who is allowed to grant exceptions?
...
- A policy document for Robotics Fault Recovery Governance with explicit scope.
