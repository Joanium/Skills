---
name: Remix Accessibility Flows
trigger: remix accessibility flows, help with remix accessibility flows, plan remix accessibility flows, improve remix accessibility flows, expert remix accessibility flows
description: Expert-level guidance for remix accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Remix Accessibility Flows is an expert frontend skill for Remix. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Remix Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Remix Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in Remix Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Remix Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does Remix Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Remix Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for Remix Accessibility Flows.
...
- [ ] The state model for Remix Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend Remix Accessibility Flows without guessing hidden UI contracts.
