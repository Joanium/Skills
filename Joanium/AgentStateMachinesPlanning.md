---
name: Agent State Machines Planning
trigger: agent state machines planning, help with agent state machines planning, plan agent state machines planning, improve agent state machines planning, expert agent state machines planning
description: Expert-level guidance for agent state machines planning, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Agent State Machines Planning is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is sequencing work, dependencies, and proof points so execution moves quickly without creating blind risk.

## When To Use

- Use this when Agent State Machines Planning influences user-visible output, automation, or safety-sensitive decisions.
...
- What must be true before Agent State Machines Planning can start safely?
...
- A milestone plan for Agent State Machines Planning with explicit gating criteria.
