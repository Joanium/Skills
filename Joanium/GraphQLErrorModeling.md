---
name: GraphQL Error Modeling
trigger: graphql error modeling, help with graphql error modeling, plan graphql error modeling, improve graphql error modeling, expert graphql error modeling
description: Expert-level guidance for graphql error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

GraphQL Error Modeling is an expert backend skill for GraphQL. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when GraphQL Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside GraphQL Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in GraphQL Error Modeling?
- Where does GraphQL Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether GraphQL Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by GraphQL Error Modeling even if it was never written down?
...
- A contract note for GraphQL Error Modeling that includes failure semantics.
...
- [ ] The contract for GraphQL Error Modeling is explicit about success and failure behavior.
