---
name: Service Decomposition Failure Budgets
trigger: service decomposition failure budgets, help with service decomposition failure budgets, plan service decomposition failure budgets, improve service decomposition failure budgets, expert service decomposition failure budgets
description: Expert-level guidance for service decomposition failure budgets with action-oriented workflow, review criteria, and failure patterns.
---

Service Decomposition Failure Budgets is an expert architecture and systems skill for Service Decomposition. The focal concern is Failure Budgets, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Service Decomposition Failure Budgets needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Service Decomposition Failure Budgets becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Service Decomposition Failure Budgets is currently cheap only because the bill has not arrived yet?
- How would Service Decomposition Failure Budgets be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Service Decomposition Failure Budgets too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Service Decomposition Failure Budgets.
...
- [ ] The key boundaries inside Service Decomposition Failure Budgets are explicit and justified.
