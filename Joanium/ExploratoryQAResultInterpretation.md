---
name: Exploratory QA Result Interpretation
trigger: exploratory qa result interpretation, help with exploratory qa result interpretation, plan exploratory qa result interpretation, improve exploratory qa result interpretation, expert exploratory qa result interpretation
description: Expert-level guidance for exploratory qa result interpretation with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Result Interpretation is an expert testing and quality skill for Exploratory QA. The focal concern is Result Interpretation, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Result Interpretation needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Result Interpretation is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Result Interpretation would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Result Interpretation depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Result Interpretation need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Result Interpretation.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Result Interpretation should mean.
...
- [ ] The key behavior protected by Exploratory QA Result Interpretation is explicit.
