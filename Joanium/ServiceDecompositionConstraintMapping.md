---
name: Service Decomposition Constraint Mapping
trigger: service decomposition constraint mapping, help with service decomposition constraint mapping, plan service decomposition constraint mapping, improve service decomposition constraint mapping, expert service decomposition constraint mapping
description: Expert-level guidance for service decomposition constraint mapping with action-oriented workflow, review criteria, and failure patterns.
---

Service Decomposition Constraint Mapping is an expert architecture and systems skill for Service Decomposition. The focal concern is Constraint Mapping, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Service Decomposition Constraint Mapping needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Service Decomposition Constraint Mapping becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Service Decomposition Constraint Mapping is currently cheap only because the bill has not arrived yet?
- How would Service Decomposition Constraint Mapping be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Service Decomposition Constraint Mapping too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Service Decomposition Constraint Mapping.
...
- [ ] The key boundaries inside Service Decomposition Constraint Mapping are explicit and justified.
