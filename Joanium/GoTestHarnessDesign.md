---
name: Go Test Harness Design
trigger: go test harness design, help with go test harness design, plan go test harness design, improve go test harness design, expert go test harness design
description: Expert-level guidance for go test harness design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Go Test Harness Design is an expert programming-language skill for Go. The focal concern is Test Harness Design, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Go Test Harness Design affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Go Test Harness Design?
- Which abstraction in Go Test Harness Design becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Go Test Harness Design first?
- How will Go Test Harness Design behave when multiple contributors evolve it independently?
- Which part of Go Test Harness Design is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Go Test Harness Design.
- A failure-mode note that explains the sharp edges inside Go Test Harness Design.
...
- [ ] The main language-level risk in Go Test Harness Design is named explicitly.
...
- [ ] Future contributors can extend Go Test Harness Design without reverse-engineering hidden contracts.
