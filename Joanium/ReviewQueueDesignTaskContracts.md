---
name: Review Queue Design Task Contracts
trigger: review queue design task contracts, help with review queue design task contracts, plan review queue design task contracts, improve review queue design task contracts, expert review queue design task contracts
description: Expert-level guidance for review queue design task contracts with action-oriented workflow, review criteria, and failure patterns.
---

Review Queue Design Task Contracts is an expert agent and tooling skill for Review Queue Design. The focal concern is Task Contracts, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Review Queue Design Task Contracts affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Review Queue Design Task Contracts is still governed by intuition instead of a contract or label set?
- Which failure in Review Queue Design Task Contracts would be hardest to detect before it compounds?
- How would Review Queue Design Task Contracts behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Review Queue Design Task Contracts?
...
- A contract and failure-label set for Review Queue Design Task Contracts.
...
- [ ] The system behind Review Queue Design Task Contracts is described more clearly than the prompt surface.
