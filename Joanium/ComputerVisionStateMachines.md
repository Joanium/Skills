---
name: Computer Vision State Machines
trigger: computer vision state machines, help with computer vision state machines, plan computer vision state machines, improve computer vision state machines, expert computer vision state machines
description: Expert-level guidance for computer vision state machines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Computer Vision State Machines is an expert systems skill for Computer Vision. The focal concern is State Machines, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Computer Vision State Machines is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Computer Vision State Machines would break first under scale, timing, or hardware stress?
- Which state or resource in Computer Vision State Machines is hardest to observe when behavior goes wrong?
- How does Computer Vision State Machines trade correctness, throughput, and recoverability today?
- What tooling gap makes Computer Vision State Machines harder to reason about than it should be?
- Which optimization target inside Computer Vision State Machines is attractive but strategically secondary?
...
- A system model for Computer Vision State Machines that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Computer Vision State Machines is explicit.
