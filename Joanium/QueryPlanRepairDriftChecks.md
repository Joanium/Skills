---
name: Query Plan Repair Drift Checks
trigger: query plan repair drift checks, help with query plan repair drift checks, plan query plan repair drift checks, improve query plan repair drift checks, expert query plan repair drift checks
description: Expert-level guidance for query plan repair drift checks with action-oriented workflow, review criteria, and failure patterns.
---

Query Plan Repair Drift Checks is an expert database and migration skill for Query Plan Repair. The focal concern is Drift Checks, but the real objective is helping an AI change data structures and access paths without breaking trust, cost, or recovery.

## When To Use

- Use this when Query Plan Repair Drift Checks is changing a data surface that other people or systems depend on semantically, not just structurally.
...
- Which consumer of Query Plan Repair Drift Checks would be harmed first by semantic drift rather than outright failure?
- What recovery move for Query Plan Repair Drift Checks becomes hardest after the cutover begins?
- How will Query Plan Repair Drift Checks be validated if the data looks plausible but is subtly wrong?
- Where could Query Plan Repair Drift Checks create hidden cost spikes while technically succeeding?
- Which ownership boundary in Query Plan Repair Drift Checks is currently too vague for safe change management?
...
- A cutover and recovery plan for Query Plan Repair Drift Checks.
...
- [ ] The meaning of Query Plan Repair Drift Checks is clearer than the implementation steps.
