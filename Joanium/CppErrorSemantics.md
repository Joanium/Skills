---
name: C++ Error Semantics
trigger: c++ error semantics, help with c++ error semantics, plan c++ error semantics, improve c++ error semantics, expert c++ error semantics
description: Expert-level guidance for c++ error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

C++ Error Semantics is an expert programming-language skill for C++. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when C++ Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside C++ Error Semantics?
- Which abstraction in C++ Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in C++ Error Semantics first?
- How will C++ Error Semantics behave when multiple contributors evolve it independently?
- Which part of C++ Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for C++ Error Semantics.
- A failure-mode note that explains the sharp edges inside C++ Error Semantics.
...
- [ ] The main language-level risk in C++ Error Semantics is named explicitly.
...
- [ ] Future contributors can extend C++ Error Semantics without reverse-engineering hidden contracts.
