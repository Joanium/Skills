---
name: React Build Pipelines
trigger: react build pipelines, help with react build pipelines, plan react build pipelines, improve react build pipelines, expert react build pipelines
description: Expert-level guidance for react build pipelines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

React Build Pipelines is an expert frontend skill for React. The focal concern is Build Pipelines, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when React Build Pipelines affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does React Build Pipelines make user state harder to reason about than it needs to be?
- Which interaction in React Build Pipelines degrades first under latency, stale data, or partial failure?
- What accessibility behavior in React Build Pipelines would break if the DOM or state changed unexpectedly?
- How does React Build Pipelines fail when routing, hydration, or rendering order becomes non-ideal?
- What part of React Build Pipelines looks correct in a mock but collapses under production behavior?
...
- A state transition map for React Build Pipelines.
...
- [ ] The state model for React Build Pipelines is clear enough to debug.
...
- [ ] Another engineer can extend React Build Pipelines without guessing hidden UI contracts.
