---
name: Exploratory QA Triage Heuristics
trigger: exploratory qa triage heuristics, help with exploratory qa triage heuristics, plan exploratory qa triage heuristics, improve exploratory qa triage heuristics, expert exploratory qa triage heuristics
description: Expert-level guidance for exploratory qa triage heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Triage Heuristics is an expert testing and quality skill for Exploratory QA. The focal concern is Triage Heuristics, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Triage Heuristics needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Triage Heuristics is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Triage Heuristics would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Triage Heuristics depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Triage Heuristics need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Triage Heuristics.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Triage Heuristics should mean.
...
- [ ] The key behavior protected by Exploratory QA Triage Heuristics is explicit.
