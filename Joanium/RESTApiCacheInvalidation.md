---
name: REST API Cache Invalidation
trigger: rest api cache invalidation, help with rest api cache invalidation, plan rest api cache invalidation, improve rest api cache invalidation, expert rest api cache invalidation
description: Expert-level guidance for rest api cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

REST API Cache Invalidation is an expert backend skill for REST API. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when REST API Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside REST API Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in REST API Cache Invalidation?
- Where does REST API Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether REST API Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by REST API Cache Invalidation even if it was never written down?
...
- A contract note for REST API Cache Invalidation that includes failure semantics.
...
- [ ] The contract for REST API Cache Invalidation is explicit about success and failure behavior.
