---
name: Review Queue Design Quality Gates
trigger: review queue design quality gates, help with review queue design quality gates, plan review queue design quality gates, improve review queue design quality gates, expert review queue design quality gates
description: Expert-level guidance for review queue design quality gates with action-oriented workflow, review criteria, and failure patterns.
---

Review Queue Design Quality Gates is an expert agent and tooling skill for Review Queue Design. The focal concern is Quality Gates, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Review Queue Design Quality Gates affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Review Queue Design Quality Gates is still governed by intuition instead of a contract or label set?
- Which failure in Review Queue Design Quality Gates would be hardest to detect before it compounds?
- How would Review Queue Design Quality Gates behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Review Queue Design Quality Gates?
...
- A contract and failure-label set for Review Queue Design Quality Gates.
...
- [ ] The system behind Review Queue Design Quality Gates is described more clearly than the prompt surface.
