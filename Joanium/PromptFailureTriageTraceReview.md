---
name: Prompt Failure Triage Trace Review
trigger: prompt failure triage trace review, help with prompt failure triage trace review, plan prompt failure triage trace review, improve prompt failure triage trace review, expert prompt failure triage trace review
description: Expert-level guidance for prompt failure triage trace review with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Trace Review is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Trace Review, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Trace Review affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Trace Review is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Trace Review would be hardest to detect before it compounds?
- How would Prompt Failure Triage Trace Review behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Trace Review?
...
- A contract and failure-label set for Prompt Failure Triage Trace Review.
...
- [ ] The system behind Prompt Failure Triage Trace Review is described more clearly than the prompt surface.
