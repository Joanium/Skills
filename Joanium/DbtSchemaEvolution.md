---
name: dbt Schema Evolution
trigger: dbt schema evolution, help with dbt schema evolution, plan dbt schema evolution, improve dbt schema evolution, expert dbt schema evolution
description: Expert-level guidance for dbt schema evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

dbt Schema Evolution is an expert data-systems skill for dbt. The focal concern is Schema Evolution, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when dbt Schema Evolution affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would dbt Schema Evolution create if two teams interpreted it differently?
- Which recovery path for dbt Schema Evolution is the least understood today?
- How will dbt Schema Evolution behave under backfills, reprocessing, or delayed sources?
- What cost pattern in dbt Schema Evolution is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside dbt Schema Evolution drifted subtly?
...
- A semantic contract note for dbt Schema Evolution.
...
- [ ] The meaning of dbt Schema Evolution is written in language consumers will actually use.
