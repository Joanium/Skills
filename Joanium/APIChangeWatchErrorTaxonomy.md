---
name: API Change Watch Error Taxonomy
trigger: api change watch error taxonomy, help with api change watch error taxonomy, plan api change watch error taxonomy, improve api change watch error taxonomy, expert api change watch error taxonomy
description: Expert-level guidance for api change watch error taxonomy with action-oriented workflow, review criteria, and failure patterns.
---

API Change Watch Error Taxonomy is an expert API and integration skill for API Change Watch. The focal concern is Error Taxonomy, but the real objective is helping an AI make external integrations reliable, reviewable, and survivable when contracts, auth, and partners drift.

## When To Use

- Use this when API Change Watch Error Taxonomy affects whether external integrations remain trustworthy after partner behavior changes.
...
- What partner or contract assumption inside API Change Watch Error Taxonomy is least likely to hold over time?
- How would API Change Watch Error Taxonomy fail if the partner changed validation, latency, or auth behavior without notice?
- What evidence proves sandbox behavior is close enough to production for API Change Watch Error Taxonomy?
- Which part of API Change Watch Error Taxonomy needs stronger replay, traceability, or escalation support?
- How will the AI explain the safety model of API Change Watch Error Taxonomy to an integration reviewer?
...
- A contract and drift note for API Change Watch Error Taxonomy.
...
- [ ] The external contract and drift risk behind API Change Watch Error Taxonomy are explicit.
