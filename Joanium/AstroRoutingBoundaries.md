---
name: Astro Routing Boundaries
trigger: astro routing boundaries, help with astro routing boundaries, plan astro routing boundaries, improve astro routing boundaries, expert astro routing boundaries
description: Expert-level guidance for astro routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Astro Routing Boundaries is an expert frontend skill for Astro. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Astro Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Astro Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in Astro Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Astro Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does Astro Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Astro Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for Astro Routing Boundaries.
...
- [ ] The state model for Astro Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend Astro Routing Boundaries without guessing hidden UI contracts.
