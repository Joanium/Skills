---
name: ASP.NET Core Error Modeling
trigger: asp.net core error modeling, help with asp.net core error modeling, plan asp.net core error modeling, improve asp.net core error modeling, expert asp.net core error modeling
description: Expert-level guidance for asp.net core error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

ASP.NET Core Error Modeling is an expert backend skill for ASP.NET Core. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when ASP.NET Core Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside ASP.NET Core Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in ASP.NET Core Error Modeling?
- Where does ASP.NET Core Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether ASP.NET Core Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by ASP.NET Core Error Modeling even if it was never written down?
...
- A contract note for ASP.NET Core Error Modeling that includes failure semantics.
...
- [ ] The contract for ASP.NET Core Error Modeling is explicit about success and failure behavior.
