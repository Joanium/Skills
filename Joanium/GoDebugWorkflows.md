---
name: Go Debug Workflows
trigger: go debug workflows, help with go debug workflows, plan go debug workflows, improve go debug workflows, expert go debug workflows
description: Expert-level guidance for go debug workflows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Go Debug Workflows is an expert programming-language skill for Go. The focal concern is Debug Workflows, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Go Debug Workflows affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Go Debug Workflows?
- Which abstraction in Go Debug Workflows becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Go Debug Workflows first?
- How will Go Debug Workflows behave when multiple contributors evolve it independently?
- Which part of Go Debug Workflows is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Go Debug Workflows.
- A failure-mode note that explains the sharp edges inside Go Debug Workflows.
...
- [ ] The main language-level risk in Go Debug Workflows is named explicitly.
...
- [ ] Future contributors can extend Go Debug Workflows without reverse-engineering hidden contracts.
