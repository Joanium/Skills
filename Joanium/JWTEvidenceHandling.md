---
name: JWT Evidence Handling
trigger: jwt evidence handling, help with jwt evidence handling, plan jwt evidence handling, improve jwt evidence handling, expert jwt evidence handling
description: Expert-level guidance for jwt evidence handling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

JWT Evidence Handling is an expert security skill for JWT. The focal concern is Evidence Handling, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when JWT Evidence Handling influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in JWT Evidence Handling first?
- Which exception or emergency path makes JWT Evidence Handling less true in practice than in policy?
- What evidence would be missing if JWT Evidence Handling failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does JWT Evidence Handling degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for JWT Evidence Handling.
...
- [ ] The attacker model for JWT Evidence Handling is realistic and current.
...
- [ ] The team can explain how JWT Evidence Handling fails and how it recovers.
