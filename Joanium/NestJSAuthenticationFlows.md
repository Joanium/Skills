---
name: NestJS Authentication Flows
trigger: nestjs authentication flows, help with nestjs authentication flows, plan nestjs authentication flows, improve nestjs authentication flows, expert nestjs authentication flows
description: Expert-level guidance for nestjs authentication flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

NestJS Authentication Flows is an expert backend skill for NestJS. The focal concern is Authentication Flows, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when NestJS Authentication Flows shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside NestJS Authentication Flows breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in NestJS Authentication Flows?
- Where does NestJS Authentication Flows need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether NestJS Authentication Flows is unhealthy before customers do?
- What compatibility promise is implied by NestJS Authentication Flows even if it was never written down?
...
- A contract note for NestJS Authentication Flows that includes failure semantics.
...
- [ ] The contract for NestJS Authentication Flows is explicit about success and failure behavior.
