---
name: Technical Debt Burn Down Diff Compression
trigger: technical debt burn down diff compression, help with technical debt burn down diff compression, plan technical debt burn down diff compression, improve technical debt burn down diff compression, expert technical debt burn down diff compression
description: Expert-level guidance for technical debt burn down diff compression with action-oriented workflow, review criteria, and failure patterns.
---

Technical Debt Burn Down Diff Compression is an expert repository and delivery skill for Technical Debt Burn Down. The focal concern is Diff Compression, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Technical Debt Burn Down Diff Compression determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Technical Debt Burn Down Diff Compression riskier than the diff itself suggests?
- Which reviewer would challenge Technical Debt Burn Down Diff Compression first, and what evidence would they need?
- How could Technical Debt Burn Down Diff Compression fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Technical Debt Burn Down Diff Compression would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Technical Debt Burn Down Diff Compression concretely?
...
- A blast-radius note for Technical Debt Burn Down Diff Compression.
...
- [ ] The repository-level blast radius of Technical Debt Burn Down Diff Compression is explicit.
