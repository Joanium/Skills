---
name: Code Search Operations Dependency Blast Radius
trigger: code search operations dependency blast radius, help with code search operations dependency blast radius, plan code search operations dependency blast radius, improve code search operations dependency blast radius, expert code search operations dependency blast radius
description: Expert-level guidance for code search operations dependency blast radius with action-oriented workflow, review criteria, and failure patterns.
---

Code Search Operations Dependency Blast Radius is an expert repository and delivery skill for Code Search Operations. The focal concern is Dependency Blast Radius, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Code Search Operations Dependency Blast Radius determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Code Search Operations Dependency Blast Radius riskier than the diff itself suggests?
- Which reviewer would challenge Code Search Operations Dependency Blast Radius first, and what evidence would they need?
- How could Code Search Operations Dependency Blast Radius fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Code Search Operations Dependency Blast Radius would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Code Search Operations Dependency Blast Radius concretely?
...
- A blast-radius note for Code Search Operations Dependency Blast Radius.
...
- [ ] The repository-level blast radius of Code Search Operations Dependency Blast Radius is explicit.
