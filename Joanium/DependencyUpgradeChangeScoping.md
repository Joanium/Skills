---
name: Dependency Upgrade Change Scoping
trigger: dependency upgrade change scoping, help with dependency upgrade change scoping, plan dependency upgrade change scoping, improve dependency upgrade change scoping, expert dependency upgrade change scoping
description: Expert-level guidance for dependency upgrade change scoping with action-oriented workflow, review criteria, and failure patterns.
---

Dependency Upgrade Change Scoping is an expert coding workflow skill for Dependency Upgrade. The focal concern is Change Scoping, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Dependency Upgrade Change Scoping is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Dependency Upgrade Change Scoping is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Dependency Upgrade Change Scoping would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Dependency Upgrade Change Scoping create a silent regression that tests may miss?
- How would the AI explain the safety of Dependency Upgrade Change Scoping to a skeptical reviewer?
...
- A concise evidence note for Dependency Upgrade Change Scoping.
...
- [ ] The evidence supporting Dependency Upgrade Change Scoping is stronger than the assumptions.
