---
name: Bug Reproduction Edge Case Sweeps
trigger: bug reproduction edge case sweeps, help with bug reproduction edge case sweeps, plan bug reproduction edge case sweeps, improve bug reproduction edge case sweeps, expert bug reproduction edge case sweeps
description: Expert-level guidance for bug reproduction edge case sweeps with action-oriented workflow, review criteria, and failure patterns.
---

Bug Reproduction Edge Case Sweeps is an expert coding workflow skill for Bug Reproduction. The focal concern is Edge Case Sweeps, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Bug Reproduction Edge Case Sweeps is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Bug Reproduction Edge Case Sweeps is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Bug Reproduction Edge Case Sweeps would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Bug Reproduction Edge Case Sweeps create a silent regression that tests may miss?
- How would the AI explain the safety of Bug Reproduction Edge Case Sweeps to a skeptical reviewer?
...
- A concise evidence note for Bug Reproduction Edge Case Sweeps.
...
- [ ] The evidence supporting Bug Reproduction Edge Case Sweeps is stronger than the assumptions.
