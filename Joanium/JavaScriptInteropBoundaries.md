---
name: JavaScript Interop Boundaries
trigger: javascript interop boundaries, help with javascript interop boundaries, plan javascript interop boundaries, improve javascript interop boundaries, expert javascript interop boundaries
description: Expert-level guidance for javascript interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

JavaScript Interop Boundaries is an expert programming-language skill for JavaScript. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when JavaScript Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside JavaScript Interop Boundaries?
- Which abstraction in JavaScript Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in JavaScript Interop Boundaries first?
- How will JavaScript Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of JavaScript Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for JavaScript Interop Boundaries.
- A failure-mode note that explains the sharp edges inside JavaScript Interop Boundaries.
...
- [ ] The main language-level risk in JavaScript Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend JavaScript Interop Boundaries without reverse-engineering hidden contracts.
