---
name: Feature Flag Operations Drift Detection
trigger: feature flag operations drift detection, help with feature flag operations drift detection, plan feature flag operations drift detection, improve feature flag operations drift detection, expert feature flag operations drift detection
description: Expert-level guidance for feature flag operations drift detection with action-oriented workflow, review criteria, and failure patterns.
---

Feature Flag Operations Drift Detection is an expert platform and automation skill for Feature Flag Operations. The focal concern is Drift Detection, but the real objective is helping an AI make infrastructure and operational change without creating invisible blast radius.

## When To Use

- Use this when Feature Flag Operations Drift Detection can affect deployment safety, service recovery, or operator trust in automation.
...
- What part of Feature Flag Operations Drift Detection could fail silently while the automation still reports success?
- Which rollback or recovery step inside Feature Flag Operations Drift Detection depends too much on tribal memory?
- How will operators know when Feature Flag Operations Drift Detection should stop, wait, or reverse course?
- Where does Feature Flag Operations Drift Detection blur access, policy, and execution boundaries dangerously?
...
- A change-and-recovery map for Feature Flag Operations Drift Detection.
...
- [ ] The rollback model for Feature Flag Operations Drift Detection is as real as the forward path.
