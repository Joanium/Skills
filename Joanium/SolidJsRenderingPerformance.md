---
name: SolidJS Rendering Performance
trigger: solidjs rendering performance, help with solidjs rendering performance, plan solidjs rendering performance, improve solidjs rendering performance, expert solidjs rendering performance
description: Expert-level guidance for solidjs rendering performance with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

SolidJS Rendering Performance is an expert frontend skill for SolidJS. The focal concern is Rendering Performance, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when SolidJS Rendering Performance affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does SolidJS Rendering Performance make user state harder to reason about than it needs to be?
- Which interaction in SolidJS Rendering Performance degrades first under latency, stale data, or partial failure?
- What accessibility behavior in SolidJS Rendering Performance would break if the DOM or state changed unexpectedly?
- How does SolidJS Rendering Performance fail when routing, hydration, or rendering order becomes non-ideal?
- What part of SolidJS Rendering Performance looks correct in a mock but collapses under production behavior?
...
- A state transition map for SolidJS Rendering Performance.
...
- [ ] The state model for SolidJS Rendering Performance is clear enough to debug.
...
- [ ] Another engineer can extend SolidJS Rendering Performance without guessing hidden UI contracts.
