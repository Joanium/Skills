---
name: Astro Build Pipelines
trigger: astro build pipelines, help with astro build pipelines, plan astro build pipelines, improve astro build pipelines, expert astro build pipelines
description: Expert-level guidance for astro build pipelines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Astro Build Pipelines is an expert frontend skill for Astro. The focal concern is Build Pipelines, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Astro Build Pipelines affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Astro Build Pipelines make user state harder to reason about than it needs to be?
- Which interaction in Astro Build Pipelines degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Astro Build Pipelines would break if the DOM or state changed unexpectedly?
- How does Astro Build Pipelines fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Astro Build Pipelines looks correct in a mock but collapses under production behavior?
...
- A state transition map for Astro Build Pipelines.
...
- [ ] The state model for Astro Build Pipelines is clear enough to debug.
...
- [ ] Another engineer can extend Astro Build Pipelines without guessing hidden UI contracts.
