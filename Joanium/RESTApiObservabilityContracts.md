---
name: REST API Observability Contracts
trigger: rest api observability contracts, help with rest api observability contracts, plan rest api observability contracts, improve rest api observability contracts, expert rest api observability contracts
description: Expert-level guidance for rest api observability contracts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

REST API Observability Contracts is an expert backend skill for REST API. The focal concern is Observability Contracts, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when REST API Observability Contracts shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside REST API Observability Contracts breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in REST API Observability Contracts?
- Where does REST API Observability Contracts need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether REST API Observability Contracts is unhealthy before customers do?
- What compatibility promise is implied by REST API Observability Contracts even if it was never written down?
...
- A contract note for REST API Observability Contracts that includes failure semantics.
...
- [ ] The contract for REST API Observability Contracts is explicit about success and failure behavior.
