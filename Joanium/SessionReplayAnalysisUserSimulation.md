---
name: Session Replay Analysis User Simulation
trigger: session replay analysis user simulation, help with session replay analysis user simulation, plan session replay analysis user simulation, improve session replay analysis user simulation, expert session replay analysis user simulation
description: Expert-level guidance for session replay analysis user simulation with action-oriented workflow, review criteria, and failure patterns.
---

Session Replay Analysis User Simulation is an expert browser automation skill for Session Replay Analysis. The focal concern is User Simulation, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Session Replay Analysis User Simulation needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Session Replay Analysis User Simulation unreliable across runs?
- Which timing or selector assumption in Session Replay Analysis User Simulation is least stable under real conditions?
- How would Session Replay Analysis User Simulation behave differently across browsers, profiles, or CI environments?
- What artifacts should Session Replay Analysis User Simulation capture so the next failure is diagnosable instead of mysterious?
- Which part of Session Replay Analysis User Simulation is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Session Replay Analysis User Simulation.
...
- [ ] The state model behind Session Replay Analysis User Simulation is explicit.
