...
trigger: fastapi cache invalidation, help with fastapi cache invalidation, plan fastapi cache invalidation, improve fastapi cache invalidation, expert fastapi cache invalidation
description: Expert-level guidance for fastapi cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

FastAPI Cache Invalidation is an expert backend skill for FastAPI. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when FastAPI Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside FastAPI Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in FastAPI Cache Invalidation?
- Where does FastAPI Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether FastAPI Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by FastAPI Cache Invalidation even if it was never written down?
...
- A contract note for FastAPI Cache Invalidation that includes failure semantics.
...
- [ ] The contract for FastAPI Cache Invalidation is explicit about success and failure behavior.
