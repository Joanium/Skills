---
name: Model Routing Latency Budgets
trigger: model routing latency budgets, help with model routing latency budgets, plan model routing latency budgets, improve model routing latency budgets, expert model routing latency budgets
description: Expert-level guidance for model routing latency budgets with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Model Routing Latency Budgets is an expert applied-AI skill for Model Routing. The focal concern is Latency Budgets, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Model Routing Latency Budgets affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Model Routing Latency Budgets would look acceptable in a demo but fail under production ambiguity?
- Which part of Model Routing Latency Budgets lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Model Routing Latency Budgets fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Model Routing Latency Budgets is getting better rather than merely changing?
- Where should humans intervene in Model Routing Latency Budgets to add leverage instead of toil?
...
- A behavior contract for Model Routing Latency Budgets.
...
- [ ] The behavior contract for Model Routing Latency Budgets is explicit.
