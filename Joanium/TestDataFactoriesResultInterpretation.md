---
name: Test Data Factories Result Interpretation
trigger: test data factories result interpretation, help with test data factories result interpretation, plan test data factories result interpretation, improve test data factories result interpretation, expert test data factories result interpretation
description: Expert-level guidance for test data factories result interpretation with action-oriented workflow, review criteria, and failure patterns.
---

Test Data Factories Result Interpretation is an expert testing and quality skill for Test Data Factories. The focal concern is Result Interpretation, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Test Data Factories Result Interpretation needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Test Data Factories Result Interpretation is still untested even if coverage looks healthy?
- Which assertion in Test Data Factories Result Interpretation would stay green while the user-visible behavior regressed?
- How much of Test Data Factories Result Interpretation depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Test Data Factories Result Interpretation need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Test Data Factories Result Interpretation.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Test Data Factories Result Interpretation should mean.
...
- [ ] The key behavior protected by Test Data Factories Result Interpretation is explicit.
