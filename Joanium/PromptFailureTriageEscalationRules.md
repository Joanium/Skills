---
name: Prompt Failure Triage Escalation Rules
trigger: prompt failure triage escalation rules, help with prompt failure triage escalation rules, plan prompt failure triage escalation rules, improve prompt failure triage escalation rules, expert prompt failure triage escalation rules
description: Expert-level guidance for prompt failure triage escalation rules with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Escalation Rules is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Escalation Rules, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Escalation Rules affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Escalation Rules is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Escalation Rules would be hardest to detect before it compounds?
- How would Prompt Failure Triage Escalation Rules behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Escalation Rules?
...
- A contract and failure-label set for Prompt Failure Triage Escalation Rules.
...
- [ ] The system behind Prompt Failure Triage Escalation Rules is described more clearly than the prompt surface.
