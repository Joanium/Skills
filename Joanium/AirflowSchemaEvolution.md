---
name: Airflow Schema Evolution
trigger: airflow schema evolution, help with airflow schema evolution, plan airflow schema evolution, improve airflow schema evolution, expert airflow schema evolution
description: Expert-level guidance for airflow schema evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Airflow Schema Evolution is an expert data-systems skill for Airflow. The focal concern is Schema Evolution, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Airflow Schema Evolution affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Airflow Schema Evolution create if two teams interpreted it differently?
- Which recovery path for Airflow Schema Evolution is the least understood today?
- How will Airflow Schema Evolution behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Airflow Schema Evolution is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Airflow Schema Evolution drifted subtly?
...
- A semantic contract note for Airflow Schema Evolution.
...
- [ ] The meaning of Airflow Schema Evolution is written in language consumers will actually use.
