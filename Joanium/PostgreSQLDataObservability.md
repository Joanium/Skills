---
name: PostgreSQL Data Observability
trigger: postgresql data observability, help with postgresql data observability, plan postgresql data observability, improve postgresql data observability, expert postgresql data observability
description: Expert-level guidance for postgresql data observability with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

PostgreSQL Data Observability is an expert data-systems skill for PostgreSQL. The focal concern is Data Observability, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when PostgreSQL Data Observability affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would PostgreSQL Data Observability create if two teams interpreted it differently?
- Which recovery path for PostgreSQL Data Observability is the least understood today?
- How will PostgreSQL Data Observability behave under backfills, reprocessing, or delayed sources?
- What cost pattern in PostgreSQL Data Observability is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside PostgreSQL Data Observability drifted subtly?
...
- A semantic contract note for PostgreSQL Data Observability.
...
- [ ] The meaning of PostgreSQL Data Observability is written in language consumers will actually use.
