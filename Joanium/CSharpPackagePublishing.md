---
name: C# Package Publishing
trigger: c# package publishing, help with c# package publishing, plan c# package publishing, improve c# package publishing, expert c# package publishing
description: Expert-level guidance for c# package publishing with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

C# Package Publishing is an expert programming-language skill for C#. The focal concern is Package Publishing, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when C# Package Publishing affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside C# Package Publishing?
- Which abstraction in C# Package Publishing becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in C# Package Publishing first?
- How will C# Package Publishing behave when multiple contributors evolve it independently?
- Which part of C# Package Publishing is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for C# Package Publishing.
- A failure-mode note that explains the sharp edges inside C# Package Publishing.
...
- [ ] The main language-level risk in C# Package Publishing is named explicitly.
...
- [ ] Future contributors can extend C# Package Publishing without reverse-engineering hidden contracts.
