...
trigger: tool failure recovery strategy, help with tool failure recovery strategy, plan tool failure recovery strategy, improve tool failure recovery strategy, expert tool failure recovery strategy
description: Expert-level guidance for tool failure recovery strategy, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Tool Failure Recovery Strategy is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is on choosing the operating model, decision boundaries, and success criteria before local optimizations become policy by accident.

## When To Use

- Use this when Tool Failure Recovery Strategy influences user-visible output, automation, or safety-sensitive decisions.
...
- What decision is Tool Failure Recovery Strategy supposed to improve over the next planning horizon?
- Which constraints are fixed, and which are merely habits that can be challenged?
- Where does failure in Tool Failure Recovery Strategy create the largest technical, business, or trust cost?
...
- A written decision statement for Tool Failure Recovery Strategy.
