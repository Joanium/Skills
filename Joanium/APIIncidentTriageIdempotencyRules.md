---
name: API Incident Triage Idempotency Rules
trigger: api incident triage idempotency rules, help with api incident triage idempotency rules, plan api incident triage idempotency rules, improve api incident triage idempotency rules, expert api incident triage idempotency rules
description: Expert-level guidance for api incident triage idempotency rules with action-oriented workflow, review criteria, and failure patterns.
---

API Incident Triage Idempotency Rules is an expert backend execution skill for API Incident Triage. The focal concern is Idempotency Rules, but the real objective is helping an AI design service behavior that stays coherent under retries, queues, partial failure, and operational pressure.

## When To Use

- Use this when API Incident Triage Idempotency Rules affects whether backend changes remain safe once real production behavior kicks in.
...
- Which failure ordering inside API Incident Triage Idempotency Rules would create the most misleading symptom?
- Where does API Incident Triage Idempotency Rules need stronger idempotency or retry discipline?
- What dependency in API Incident Triage Idempotency Rules is currently hiding behind timeout or queue noise?
- How would an operator explain the health of API Incident Triage Idempotency Rules in the first five minutes of an incident?
- What contract assumption in API Incident Triage Idempotency Rules is least defensible under partial failure?
...
- A failure-sequence note for API Incident Triage Idempotency Rules.
...
- [ ] The real behavior of API Incident Triage Idempotency Rules under retries and failures is explicit.
