---
name: Form Autofill Recovery Artifact Capture
trigger: form autofill recovery artifact capture, help with form autofill recovery artifact capture, plan form autofill recovery artifact capture, improve form autofill recovery artifact capture, expert form autofill recovery artifact capture
description: Expert-level guidance for form autofill recovery artifact capture with action-oriented workflow, review criteria, and failure patterns.
---

Form Autofill Recovery Artifact Capture is an expert browser automation skill for Form Autofill Recovery. The focal concern is Artifact Capture, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Form Autofill Recovery Artifact Capture needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Form Autofill Recovery Artifact Capture unreliable across runs?
- Which timing or selector assumption in Form Autofill Recovery Artifact Capture is least stable under real conditions?
- How would Form Autofill Recovery Artifact Capture behave differently across browsers, profiles, or CI environments?
- What artifacts should Form Autofill Recovery Artifact Capture capture so the next failure is diagnosable instead of mysterious?
- Which part of Form Autofill Recovery Artifact Capture is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Form Autofill Recovery Artifact Capture.
...
- [ ] The state model behind Form Autofill Recovery Artifact Capture is explicit.
