---
name: React Error Recovery UX
trigger: react error recovery ux, help with react error recovery ux, plan react error recovery ux, improve react error recovery ux, expert react error recovery ux
description: Expert-level guidance for react error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

React Error Recovery UX is an expert frontend skill for React. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when React Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does React Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in React Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in React Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does React Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of React Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for React Error Recovery UX.
...
- [ ] The state model for React Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend React Error Recovery UX without guessing hidden UI contracts.
