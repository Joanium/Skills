---
name: Build Failure Response Dependency Blast Radius
trigger: build failure response dependency blast radius, help with build failure response dependency blast radius, plan build failure response dependency blast radius, improve build failure response dependency blast radius, expert build failure response dependency blast radius
description: Expert-level guidance for build failure response dependency blast radius with action-oriented workflow, review criteria, and failure patterns.
---

Build Failure Response Dependency Blast Radius is an expert repository and delivery skill for Build Failure Response. The focal concern is Dependency Blast Radius, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Build Failure Response Dependency Blast Radius determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Build Failure Response Dependency Blast Radius riskier than the diff itself suggests?
- Which reviewer would challenge Build Failure Response Dependency Blast Radius first, and what evidence would they need?
- How could Build Failure Response Dependency Blast Radius fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Build Failure Response Dependency Blast Radius would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Build Failure Response Dependency Blast Radius concretely?
...
- A blast-radius note for Build Failure Response Dependency Blast Radius.
...
- [ ] The repository-level blast radius of Build Failure Response Dependency Blast Radius is explicit.
