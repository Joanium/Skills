---
name: GitHub Actions Failure Recovery
trigger: github actions failure recovery, help with github actions failure recovery, plan github actions failure recovery, improve github actions failure recovery, expert github actions failure recovery
description: Expert-level guidance for github actions failure recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

GitHub Actions Failure Recovery is an expert cloud and platform skill for GitHub Actions. The focal concern is Failure Recovery, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when GitHub Actions Failure Recovery changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside GitHub Actions Failure Recovery would fail first during a rushed rollout or incident?
- Which control in GitHub Actions Failure Recovery is automated but not truly reversible?
- How will operators know whether GitHub Actions Failure Recovery is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in GitHub Actions Failure Recovery is invisible until it becomes expensive?
- Which dependency makes GitHub Actions Failure Recovery less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for GitHub Actions Failure Recovery.
...
- [ ] The rollback path for GitHub Actions Failure Recovery is explicit and tested.
