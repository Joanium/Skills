---
name: Performance Triage Regression Guards
trigger: performance triage regression guards, help with performance triage regression guards, plan performance triage regression guards, improve performance triage regression guards, expert performance triage regression guards
description: Expert-level guidance for performance triage regression guards with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Regression Guards is an expert coding workflow skill for Performance Triage. The focal concern is Regression Guards, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Regression Guards is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Regression Guards is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Regression Guards would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Regression Guards create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Regression Guards to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Regression Guards.
...
- [ ] The evidence supporting Performance Triage Regression Guards is stronger than the assumptions.
