---
name: PostgreSQL Query Profiling
trigger: postgresql query profiling, help with postgresql query profiling, plan postgresql query profiling, improve postgresql query profiling, expert postgresql query profiling
description: Expert-level guidance for postgresql query profiling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

PostgreSQL Query Profiling is an expert data-systems skill for PostgreSQL. The focal concern is Query Profiling, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when PostgreSQL Query Profiling affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would PostgreSQL Query Profiling create if two teams interpreted it differently?
- Which recovery path for PostgreSQL Query Profiling is the least understood today?
- How will PostgreSQL Query Profiling behave under backfills, reprocessing, or delayed sources?
- What cost pattern in PostgreSQL Query Profiling is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside PostgreSQL Query Profiling drifted subtly?
...
- A semantic contract note for PostgreSQL Query Profiling.
...
- [ ] The meaning of PostgreSQL Query Profiling is written in language consumers will actually use.
