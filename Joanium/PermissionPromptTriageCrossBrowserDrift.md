---
name: Permission Prompt Triage Cross-Browser Drift
trigger: permission prompt triage cross-browser drift, help with permission prompt triage cross-browser drift, plan permission prompt triage cross-browser drift, improve permission prompt triage cross-browser drift, expert permission prompt triage cross-browser drift
description: Expert-level guidance for permission prompt triage cross-browser drift with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage Cross-Browser Drift is an expert browser automation skill for Permission Prompt Triage. The focal concern is Cross-Browser Drift, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage Cross-Browser Drift needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage Cross-Browser Drift unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage Cross-Browser Drift is least stable under real conditions?
- How would Permission Prompt Triage Cross-Browser Drift behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage Cross-Browser Drift capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage Cross-Browser Drift is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage Cross-Browser Drift.
...
- [ ] The state model behind Permission Prompt Triage Cross-Browser Drift is explicit.
