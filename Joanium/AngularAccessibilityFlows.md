---
name: Angular Accessibility Flows
trigger: angular accessibility flows, help with angular accessibility flows, plan angular accessibility flows, improve angular accessibility flows, expert angular accessibility flows
description: Expert-level guidance for angular accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Angular Accessibility Flows is an expert frontend skill for Angular. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Angular Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Angular Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in Angular Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Angular Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does Angular Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Angular Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for Angular Accessibility Flows.
...
- [ ] The state model for Angular Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend Angular Accessibility Flows without guessing hidden UI contracts.
