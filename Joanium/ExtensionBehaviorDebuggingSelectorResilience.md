---
name: Extension Behavior Debugging Selector Resilience
trigger: extension behavior debugging selector resilience, help with extension behavior debugging selector resilience, plan extension behavior debugging selector resilience, improve extension behavior debugging selector resilience, expert extension behavior debugging selector resilience
description: Expert-level guidance for extension behavior debugging selector resilience with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Selector Resilience is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Selector Resilience, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Selector Resilience needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Selector Resilience unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Selector Resilience is least stable under real conditions?
- How would Extension Behavior Debugging Selector Resilience behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Selector Resilience capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Selector Resilience is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Selector Resilience.
...
- [ ] The state model behind Extension Behavior Debugging Selector Resilience is explicit.
