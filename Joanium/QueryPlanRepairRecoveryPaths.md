---
name: Query Plan Repair Recovery Paths
trigger: query plan repair recovery paths, help with query plan repair recovery paths, plan query plan repair recovery paths, improve query plan repair recovery paths, expert query plan repair recovery paths
description: Expert-level guidance for query plan repair recovery paths with action-oriented workflow, review criteria, and failure patterns.
---

Query Plan Repair Recovery Paths is an expert database and migration skill for Query Plan Repair. The focal concern is Recovery Paths, but the real objective is helping an AI change data structures and access paths without breaking trust, cost, or recovery.

## When To Use

- Use this when Query Plan Repair Recovery Paths is changing a data surface that other people or systems depend on semantically, not just structurally.
...
- Which consumer of Query Plan Repair Recovery Paths would be harmed first by semantic drift rather than outright failure?
- What recovery move for Query Plan Repair Recovery Paths becomes hardest after the cutover begins?
- How will Query Plan Repair Recovery Paths be validated if the data looks plausible but is subtly wrong?
- Where could Query Plan Repair Recovery Paths create hidden cost spikes while technically succeeding?
- Which ownership boundary in Query Plan Repair Recovery Paths is currently too vague for safe change management?
...
- A cutover and recovery plan for Query Plan Repair Recovery Paths.
...
- [ ] The meaning of Query Plan Repair Recovery Paths is clearer than the implementation steps.
