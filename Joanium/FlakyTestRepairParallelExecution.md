---
name: Flaky Test Repair Parallel Execution
trigger: flaky test repair parallel execution, help with flaky test repair parallel execution, plan flaky test repair parallel execution, improve flaky test repair parallel execution, expert flaky test repair parallel execution
description: Expert-level guidance for flaky test repair parallel execution with action-oriented workflow, review criteria, and failure patterns.
---

Flaky Test Repair Parallel Execution is an expert testing and quality skill for Flaky Test Repair. The focal concern is Parallel Execution, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Flaky Test Repair Parallel Execution needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Flaky Test Repair Parallel Execution is still untested even if coverage looks healthy?
- Which assertion in Flaky Test Repair Parallel Execution would stay green while the user-visible behavior regressed?
- How much of Flaky Test Repair Parallel Execution depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Flaky Test Repair Parallel Execution need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Flaky Test Repair Parallel Execution.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Flaky Test Repair Parallel Execution should mean.
...
- [ ] The key behavior protected by Flaky Test Repair Parallel Execution is explicit.
