---
name: Failure Injection Lab Oracle Design
trigger: failure injection lab oracle design, help with failure injection lab oracle design, plan failure injection lab oracle design, improve failure injection lab oracle design, expert failure injection lab oracle design
description: Expert-level guidance for failure injection lab oracle design with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Oracle Design is an expert testing and quality skill for Failure Injection Lab. The focal concern is Oracle Design, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Oracle Design needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Oracle Design is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Oracle Design would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Oracle Design depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Oracle Design need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Oracle Design.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Oracle Design should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Oracle Design is explicit.
