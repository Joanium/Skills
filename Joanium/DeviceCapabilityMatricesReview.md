---
name: Device Capability Matrices Review
trigger: device capability matrices review, help with device capability matrices review, plan device capability matrices review, improve device capability matrices review, expert device capability matrices review
description: Expert-level guidance for device capability matrices review, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Device Capability Matrices Review is an expert-level client platform skill for shipping resilient mobile or desktop behavior across devices, stores, networks, and operating-system constraints. The emphasis here is challenging assumptions, proving readiness, and deciding whether the work is actually safe to adopt.

## When To Use

- Use this when Device Capability Matrices Review affects release risk, device behavior, or user trust during degraded conditions.
...
- What claims about Device Capability Matrices Review are supported by evidence, and which are still assertions?
...
- A review summary for Device Capability Matrices Review with clear approval status.
