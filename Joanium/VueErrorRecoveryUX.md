---
name: Vue Error Recovery UX
trigger: vue error recovery ux, help with vue error recovery ux, plan vue error recovery ux, improve vue error recovery ux, expert vue error recovery ux
description: Expert-level guidance for vue error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Vue Error Recovery UX is an expert frontend skill for Vue. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Vue Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Vue Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in Vue Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Vue Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does Vue Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Vue Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for Vue Error Recovery UX.
...
- [ ] The state model for Vue Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend Vue Error Recovery UX without guessing hidden UI contracts.
