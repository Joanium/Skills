---
name: Performance Triage Trace Isolation
trigger: performance triage trace isolation, help with performance triage trace isolation, plan performance triage trace isolation, improve performance triage trace isolation, expert performance triage trace isolation
description: Expert-level guidance for performance triage trace isolation with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Trace Isolation is an expert coding workflow skill for Performance Triage. The focal concern is Trace Isolation, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Trace Isolation is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Trace Isolation is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Trace Isolation would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Trace Isolation create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Trace Isolation to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Trace Isolation.
...
- [ ] The evidence supporting Performance Triage Trace Isolation is stronger than the assumptions.
