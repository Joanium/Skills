---
name: Next.js Accessibility Flows
trigger: next.js accessibility flows, help with next.js accessibility flows, plan next.js accessibility flows, improve next.js accessibility flows, expert next.js accessibility flows
description: Expert-level guidance for next.js accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Next.js Accessibility Flows is an expert frontend skill for Next.js. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Next.js Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Next.js Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in Next.js Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Next.js Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does Next.js Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Next.js Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for Next.js Accessibility Flows.
...
- [ ] The state model for Next.js Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend Next.js Accessibility Flows without guessing hidden UI contracts.
