---
name: Threat Modeling Secure Defaults
trigger: threat modeling secure defaults, help with threat modeling secure defaults, plan threat modeling secure defaults, improve threat modeling secure defaults, expert threat modeling secure defaults
description: Expert-level guidance for threat modeling secure defaults with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Threat Modeling Secure Defaults is an expert security skill for Threat Modeling. The focal concern is Secure Defaults, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when Threat Modeling Secure Defaults influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in Threat Modeling Secure Defaults first?
- Which exception or emergency path makes Threat Modeling Secure Defaults less true in practice than in policy?
- What evidence would be missing if Threat Modeling Secure Defaults failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does Threat Modeling Secure Defaults degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for Threat Modeling Secure Defaults.
...
- [ ] The attacker model for Threat Modeling Secure Defaults is realistic and current.
...
- [ ] The team can explain how Threat Modeling Secure Defaults fails and how it recovers.
