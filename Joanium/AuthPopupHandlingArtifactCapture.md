---
name: Auth Popup Handling Artifact Capture
trigger: auth popup handling artifact capture, help with auth popup handling artifact capture, plan auth popup handling artifact capture, improve auth popup handling artifact capture, expert auth popup handling artifact capture
description: Expert-level guidance for auth popup handling artifact capture with action-oriented workflow, review criteria, and failure patterns.
---

Auth Popup Handling Artifact Capture is an expert browser automation skill for Auth Popup Handling. The focal concern is Artifact Capture, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Auth Popup Handling Artifact Capture needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Auth Popup Handling Artifact Capture unreliable across runs?
- Which timing or selector assumption in Auth Popup Handling Artifact Capture is least stable under real conditions?
- How would Auth Popup Handling Artifact Capture behave differently across browsers, profiles, or CI environments?
- What artifacts should Auth Popup Handling Artifact Capture capture so the next failure is diagnosable instead of mysterious?
- Which part of Auth Popup Handling Artifact Capture is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Auth Popup Handling Artifact Capture.
...
- [ ] The state model behind Auth Popup Handling Artifact Capture is explicit.
