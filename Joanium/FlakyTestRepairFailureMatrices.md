---
name: Flaky Test Repair Failure Matrices
trigger: flaky test repair failure matrices, help with flaky test repair failure matrices, plan flaky test repair failure matrices, improve flaky test repair failure matrices, expert flaky test repair failure matrices
description: Expert-level guidance for flaky test repair failure matrices with action-oriented workflow, review criteria, and failure patterns.
---

Flaky Test Repair Failure Matrices is an expert testing and quality skill for Flaky Test Repair. The focal concern is Failure Matrices, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Flaky Test Repair Failure Matrices needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Flaky Test Repair Failure Matrices is still untested even if coverage looks healthy?
- Which assertion in Flaky Test Repair Failure Matrices would stay green while the user-visible behavior regressed?
- How much of Flaky Test Repair Failure Matrices depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Flaky Test Repair Failure Matrices need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Flaky Test Repair Failure Matrices.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Flaky Test Repair Failure Matrices should mean.
...
- [ ] The key behavior protected by Flaky Test Repair Failure Matrices is explicit.
