---
name: Form Autofill Recovery Cross-Browser Drift
trigger: form autofill recovery cross-browser drift, help with form autofill recovery cross-browser drift, plan form autofill recovery cross-browser drift, improve form autofill recovery cross-browser drift, expert form autofill recovery cross-browser drift
description: Expert-level guidance for form autofill recovery cross-browser drift with action-oriented workflow, review criteria, and failure patterns.
---

Form Autofill Recovery Cross-Browser Drift is an expert browser automation skill for Form Autofill Recovery. The focal concern is Cross-Browser Drift, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Form Autofill Recovery Cross-Browser Drift needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Form Autofill Recovery Cross-Browser Drift unreliable across runs?
- Which timing or selector assumption in Form Autofill Recovery Cross-Browser Drift is least stable under real conditions?
- How would Form Autofill Recovery Cross-Browser Drift behave differently across browsers, profiles, or CI environments?
- What artifacts should Form Autofill Recovery Cross-Browser Drift capture so the next failure is diagnosable instead of mysterious?
- Which part of Form Autofill Recovery Cross-Browser Drift is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Form Autofill Recovery Cross-Browser Drift.
...
- [ ] The state model behind Form Autofill Recovery Cross-Browser Drift is explicit.
