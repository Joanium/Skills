---
name: Svelte Hydration Recovery
trigger: svelte hydration recovery, help with svelte hydration recovery, plan svelte hydration recovery, improve svelte hydration recovery, expert svelte hydration recovery
description: Expert-level guidance for svelte hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte Hydration Recovery is an expert frontend skill for Svelte. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in Svelte Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does Svelte Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte Hydration Recovery.
...
- [ ] The state model for Svelte Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend Svelte Hydration Recovery without guessing hidden UI contracts.
