---
name: SolidJS Accessibility Flows
trigger: solidjs accessibility flows, help with solidjs accessibility flows, plan solidjs accessibility flows, improve solidjs accessibility flows, expert solidjs accessibility flows
description: Expert-level guidance for solidjs accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

SolidJS Accessibility Flows is an expert frontend skill for SolidJS. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when SolidJS Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does SolidJS Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in SolidJS Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in SolidJS Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does SolidJS Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of SolidJS Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for SolidJS Accessibility Flows.
...
- [ ] The state model for SolidJS Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend SolidJS Accessibility Flows without guessing hidden UI contracts.
