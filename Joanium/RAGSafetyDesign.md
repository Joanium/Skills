---
name: RAG Safety Design
trigger: rag safety design, help with rag safety design, plan rag safety design, improve rag safety design, expert rag safety design
description: Expert-level guidance for rag safety design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

RAG Safety Design is an expert applied-AI skill for RAG. The focal concern is Safety Design, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when RAG Safety Design affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in RAG Safety Design would look acceptable in a demo but fail under production ambiguity?
- Which part of RAG Safety Design lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will RAG Safety Design fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that RAG Safety Design is getting better rather than merely changing?
- Where should humans intervene in RAG Safety Design to add leverage instead of toil?
...
- A behavior contract for RAG Safety Design.
...
- [ ] The behavior contract for RAG Safety Design is explicit.
