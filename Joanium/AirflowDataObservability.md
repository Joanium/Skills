---
name: Airflow Data Observability
trigger: airflow data observability, help with airflow data observability, plan airflow data observability, improve airflow data observability, expert airflow data observability
description: Expert-level guidance for airflow data observability with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Airflow Data Observability is an expert data-systems skill for Airflow. The focal concern is Data Observability, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Airflow Data Observability affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Airflow Data Observability create if two teams interpreted it differently?
- Which recovery path for Airflow Data Observability is the least understood today?
- How will Airflow Data Observability behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Airflow Data Observability is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Airflow Data Observability drifted subtly?
...
- A semantic contract note for Airflow Data Observability.
...
- [ ] The meaning of Airflow Data Observability is written in language consumers will actually use.
