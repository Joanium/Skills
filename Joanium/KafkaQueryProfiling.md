---
name: Kafka Query Profiling
trigger: kafka query profiling, help with kafka query profiling, plan kafka query profiling, improve kafka query profiling, expert kafka query profiling
description: Expert-level guidance for kafka query profiling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kafka Query Profiling is an expert data-systems skill for Kafka. The focal concern is Query Profiling, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Kafka Query Profiling affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Kafka Query Profiling create if two teams interpreted it differently?
- Which recovery path for Kafka Query Profiling is the least understood today?
- How will Kafka Query Profiling behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Kafka Query Profiling is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Kafka Query Profiling drifted subtly?
...
- A semantic contract note for Kafka Query Profiling.
...
- [ ] The meaning of Kafka Query Profiling is written in language consumers will actually use.
