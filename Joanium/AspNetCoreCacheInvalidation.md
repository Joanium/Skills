---
name: ASP.NET Core Cache Invalidation
trigger: asp.net core cache invalidation, help with asp.net core cache invalidation, plan asp.net core cache invalidation, improve asp.net core cache invalidation, expert asp.net core cache invalidation
description: Expert-level guidance for asp.net core cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

ASP.NET Core Cache Invalidation is an expert backend skill for ASP.NET Core. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when ASP.NET Core Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside ASP.NET Core Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in ASP.NET Core Cache Invalidation?
- Where does ASP.NET Core Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether ASP.NET Core Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by ASP.NET Core Cache Invalidation even if it was never written down?
...
- A contract note for ASP.NET Core Cache Invalidation that includes failure semantics.
...
- [ ] The contract for ASP.NET Core Cache Invalidation is explicit about success and failure behavior.
