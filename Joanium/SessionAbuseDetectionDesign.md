---
name: Session Abuse Detection Design
trigger: session abuse detection design, help with session abuse detection design, plan session abuse detection design, improve session abuse detection design, expert session abuse detection design
description: Expert-level guidance for session abuse detection design, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Session Abuse Detection Design is an expert-level security skill for reducing exploitable gaps while keeping controls practical for the team that must operate them. The emphasis here is on interfaces, invariants, failure handling, and why one structure is safer than its alternatives.

## When To Use

- Use this when Session Abuse Detection Design affects trust boundaries, privileged actions, or evidence needed after an incident.
...
- What are the core boundaries, contracts, and invariants in Session Abuse Detection Design?
...
- An architecture or boundary diagram for Session Abuse Detection Design.
