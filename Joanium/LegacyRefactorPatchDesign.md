---
name: Legacy Refactor Patch Design
trigger: legacy refactor patch design, help with legacy refactor patch design, plan legacy refactor patch design, improve legacy refactor patch design, expert legacy refactor patch design
description: Expert-level guidance for legacy refactor patch design with action-oriented workflow, review criteria, and failure patterns.
---

Legacy Refactor Patch Design is an expert coding workflow skill for Legacy Refactor. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Legacy Refactor Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Legacy Refactor Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Legacy Refactor Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Legacy Refactor Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Legacy Refactor Patch Design to a skeptical reviewer?
...
- A concise evidence note for Legacy Refactor Patch Design.
...
- [ ] The evidence supporting Legacy Refactor Patch Design is stronger than the assumptions.
