---
name: Tool Selection Design Comparison Runs
trigger: tool selection design comparison runs, help with tool selection design comparison runs, plan tool selection design comparison runs, improve tool selection design comparison runs, expert tool selection design comparison runs
description: Expert-level guidance for tool selection design comparison runs with action-oriented workflow, review criteria, and failure patterns.
---

Tool Selection Design Comparison Runs is an expert agent and tooling skill for Tool Selection Design. The focal concern is Comparison Runs, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Tool Selection Design Comparison Runs affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Tool Selection Design Comparison Runs is still governed by intuition instead of a contract or label set?
- Which failure in Tool Selection Design Comparison Runs would be hardest to detect before it compounds?
- How would Tool Selection Design Comparison Runs behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Tool Selection Design Comparison Runs?
...
- A contract and failure-label set for Tool Selection Design Comparison Runs.
...
- [ ] The system behind Tool Selection Design Comparison Runs is described more clearly than the prompt surface.
