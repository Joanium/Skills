---
name: Flaky Test Repair Oracle Design
trigger: flaky test repair oracle design, help with flaky test repair oracle design, plan flaky test repair oracle design, improve flaky test repair oracle design, expert flaky test repair oracle design
description: Expert-level guidance for flaky test repair oracle design with action-oriented workflow, review criteria, and failure patterns.
---

Flaky Test Repair Oracle Design is an expert testing and quality skill for Flaky Test Repair. The focal concern is Oracle Design, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Flaky Test Repair Oracle Design needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Flaky Test Repair Oracle Design is still untested even if coverage looks healthy?
- Which assertion in Flaky Test Repair Oracle Design would stay green while the user-visible behavior regressed?
- How much of Flaky Test Repair Oracle Design depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Flaky Test Repair Oracle Design need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Flaky Test Repair Oracle Design.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Flaky Test Repair Oracle Design should mean.
...
- [ ] The key behavior protected by Flaky Test Repair Oracle Design is explicit.
