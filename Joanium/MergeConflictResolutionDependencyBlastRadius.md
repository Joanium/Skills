---
name: Merge Conflict Resolution Dependency Blast Radius
trigger: merge conflict resolution dependency blast radius, help with merge conflict resolution dependency blast radius, plan merge conflict resolution dependency blast radius, improve merge conflict resolution dependency blast radius, expert merge conflict resolution dependency blast radius
description: Expert-level guidance for merge conflict resolution dependency blast radius with action-oriented workflow, review criteria, and failure patterns.
---

Merge Conflict Resolution Dependency Blast Radius is an expert repository and delivery skill for Merge Conflict Resolution. The focal concern is Dependency Blast Radius, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Merge Conflict Resolution Dependency Blast Radius determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Merge Conflict Resolution Dependency Blast Radius riskier than the diff itself suggests?
- Which reviewer would challenge Merge Conflict Resolution Dependency Blast Radius first, and what evidence would they need?
- How could Merge Conflict Resolution Dependency Blast Radius fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Merge Conflict Resolution Dependency Blast Radius would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Merge Conflict Resolution Dependency Blast Radius concretely?
...
- A blast-radius note for Merge Conflict Resolution Dependency Blast Radius.
...
- [ ] The repository-level blast radius of Merge Conflict Resolution Dependency Blast Radius is explicit.
