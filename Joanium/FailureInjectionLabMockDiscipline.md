---
name: Failure Injection Lab Mock Discipline
trigger: failure injection lab mock discipline, help with failure injection lab mock discipline, plan failure injection lab mock discipline, improve failure injection lab mock discipline, expert failure injection lab mock discipline
description: Expert-level guidance for failure injection lab mock discipline with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Mock Discipline is an expert testing and quality skill for Failure Injection Lab. The focal concern is Mock Discipline, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Mock Discipline needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Mock Discipline is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Mock Discipline would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Mock Discipline depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Mock Discipline need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Mock Discipline.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Mock Discipline should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Mock Discipline is explicit.
