---
name: Swift Performance Profiling
trigger: swift performance profiling, help with swift performance profiling, plan swift performance profiling, improve swift performance profiling, expert swift performance profiling
description: Expert-level guidance for swift performance profiling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Swift Performance Profiling is an expert programming-language skill for Swift. The focal concern is Performance Profiling, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Swift Performance Profiling affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Swift Performance Profiling?
- Which abstraction in Swift Performance Profiling becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Swift Performance Profiling first?
- How will Swift Performance Profiling behave when multiple contributors evolve it independently?
- Which part of Swift Performance Profiling is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Swift Performance Profiling.
- A failure-mode note that explains the sharp edges inside Swift Performance Profiling.
...
- [ ] The main language-level risk in Swift Performance Profiling is named explicitly.
...
- [ ] Future contributors can extend Swift Performance Profiling without reverse-engineering hidden contracts.
