---
name: Extension Behavior Debugging Environment Parity
trigger: extension behavior debugging environment parity, help with extension behavior debugging environment parity, plan extension behavior debugging environment parity, improve extension behavior debugging environment parity, expert extension behavior debugging environment parity
description: Expert-level guidance for extension behavior debugging environment parity with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Environment Parity is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Environment Parity, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Environment Parity needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Environment Parity unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Environment Parity is least stable under real conditions?
- How would Extension Behavior Debugging Environment Parity behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Environment Parity capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Environment Parity is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Environment Parity.
...
- [ ] The state model behind Extension Behavior Debugging Environment Parity is explicit.
