---
name: FastAPI Contract Evolution
trigger: fastapi contract evolution, help with fastapi contract evolution, plan fastapi contract evolution, improve fastapi contract evolution, expert fastapi contract evolution
description: Expert-level guidance for fastapi contract evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

FastAPI Contract Evolution is an expert backend skill for FastAPI. The focal concern is Contract Evolution, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when FastAPI Contract Evolution shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside FastAPI Contract Evolution breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in FastAPI Contract Evolution?
- Where does FastAPI Contract Evolution need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether FastAPI Contract Evolution is unhealthy before customers do?
- What compatibility promise is implied by FastAPI Contract Evolution even if it was never written down?
...
- A contract note for FastAPI Contract Evolution that includes failure semantics.
...
- [ ] The contract for FastAPI Contract Evolution is explicit about success and failure behavior.
