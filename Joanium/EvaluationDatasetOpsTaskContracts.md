---
name: Evaluation Dataset Operations Task Contracts
trigger: evaluation dataset operations task contracts, help with evaluation dataset operations task contracts, plan evaluation dataset operations task contracts, improve evaluation dataset operations task contracts, expert evaluation dataset operations task contracts
description: Expert-level guidance for evaluation dataset operations task contracts with action-oriented workflow, review criteria, and failure patterns.
---

Evaluation Dataset Operations Task Contracts is an expert agent and tooling skill for Evaluation Dataset Operations. The focal concern is Task Contracts, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Evaluation Dataset Operations Task Contracts affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Evaluation Dataset Operations Task Contracts is still governed by intuition instead of a contract or label set?
- Which failure in Evaluation Dataset Operations Task Contracts would be hardest to detect before it compounds?
- How would Evaluation Dataset Operations Task Contracts behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Evaluation Dataset Operations Task Contracts?
...
- A contract and failure-label set for Evaluation Dataset Operations Task Contracts.
...
- [ ] The system behind Evaluation Dataset Operations Task Contracts is described more clearly than the prompt surface.
