---
name: Patch Backporting Dependency Blast Radius
trigger: patch backporting dependency blast radius, help with patch backporting dependency blast radius, plan patch backporting dependency blast radius, improve patch backporting dependency blast radius, expert patch backporting dependency blast radius
description: Expert-level guidance for patch backporting dependency blast radius with action-oriented workflow, review criteria, and failure patterns.
---

Patch Backporting Dependency Blast Radius is an expert repository and delivery skill for Patch Backporting. The focal concern is Dependency Blast Radius, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Patch Backporting Dependency Blast Radius determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Patch Backporting Dependency Blast Radius riskier than the diff itself suggests?
- Which reviewer would challenge Patch Backporting Dependency Blast Radius first, and what evidence would they need?
- How could Patch Backporting Dependency Blast Radius fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Patch Backporting Dependency Blast Radius would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Patch Backporting Dependency Blast Radius concretely?
...
- A blast-radius note for Patch Backporting Dependency Blast Radius.
...
- [ ] The repository-level blast radius of Patch Backporting Dependency Blast Radius is explicit.
