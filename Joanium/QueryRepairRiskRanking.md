---
name: Query Repair Risk Ranking
trigger: query repair risk ranking, help with query repair risk ranking, plan query repair risk ranking, improve query repair risk ranking, expert query repair risk ranking
description: Expert-level guidance for query repair risk ranking with action-oriented workflow, review criteria, and failure patterns.
---

Query Repair Risk Ranking is an expert coding workflow skill for Query Repair. The focal concern is Risk Ranking, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Query Repair Risk Ranking is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Query Repair Risk Ranking is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Query Repair Risk Ranking would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Query Repair Risk Ranking create a silent regression that tests may miss?
- How would the AI explain the safety of Query Repair Risk Ranking to a skeptical reviewer?
...
- A concise evidence note for Query Repair Risk Ranking.
...
- [ ] The evidence supporting Query Repair Risk Ranking is stronger than the assumptions.
