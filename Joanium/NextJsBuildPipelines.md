---
name: Next.js Build Pipelines
trigger: next.js build pipelines, help with next.js build pipelines, plan next.js build pipelines, improve next.js build pipelines, expert next.js build pipelines
description: Expert-level guidance for next.js build pipelines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Next.js Build Pipelines is an expert frontend skill for Next.js. The focal concern is Build Pipelines, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Next.js Build Pipelines affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Next.js Build Pipelines make user state harder to reason about than it needs to be?
- Which interaction in Next.js Build Pipelines degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Next.js Build Pipelines would break if the DOM or state changed unexpectedly?
- How does Next.js Build Pipelines fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Next.js Build Pipelines looks correct in a mock but collapses under production behavior?
...
- A state transition map for Next.js Build Pipelines.
...
- [ ] The state model for Next.js Build Pipelines is clear enough to debug.
...
- [ ] Another engineer can extend Next.js Build Pipelines without guessing hidden UI contracts.
