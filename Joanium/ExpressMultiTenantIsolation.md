---
name: Express Multi-Tenant Isolation
trigger: express multi-tenant isolation, help with express multi-tenant isolation, plan express multi-tenant isolation, improve express multi-tenant isolation, expert express multi-tenant isolation
description: Expert-level guidance for express multi-tenant isolation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Express Multi-Tenant Isolation is an expert backend skill for Express. The focal concern is Multi-Tenant Isolation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Express Multi-Tenant Isolation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Express Multi-Tenant Isolation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Express Multi-Tenant Isolation?
- Where does Express Multi-Tenant Isolation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Express Multi-Tenant Isolation is unhealthy before customers do?
- What compatibility promise is implied by Express Multi-Tenant Isolation even if it was never written down?
...
- A contract note for Express Multi-Tenant Isolation that includes failure semantics.
...
- [ ] The contract for Express Multi-Tenant Isolation is explicit about success and failure behavior.
