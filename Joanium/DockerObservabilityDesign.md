---
name: Docker Observability Design
trigger: docker observability design, help with docker observability design, plan docker observability design, improve docker observability design, expert docker observability design
description: Expert-level guidance for docker observability design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Docker Observability Design is an expert cloud and platform skill for Docker. The focal concern is Observability Design, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Docker Observability Design changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Docker Observability Design would fail first during a rushed rollout or incident?
- Which control in Docker Observability Design is automated but not truly reversible?
- How will operators know whether Docker Observability Design is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Docker Observability Design is invisible until it becomes expensive?
- Which dependency makes Docker Observability Design less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Docker Observability Design.
...
- [ ] The rollback path for Docker Observability Design is explicit and tested.
