---
name: Model Fallback Routing Trace Review
trigger: model fallback routing trace review, help with model fallback routing trace review, plan model fallback routing trace review, improve model fallback routing trace review, expert model fallback routing trace review
description: Expert-level guidance for model fallback routing trace review with action-oriented workflow, review criteria, and failure patterns.
---

Model Fallback Routing Trace Review is an expert agent and tooling skill for Model Fallback Routing. The focal concern is Trace Review, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Model Fallback Routing Trace Review affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Model Fallback Routing Trace Review is still governed by intuition instead of a contract or label set?
- Which failure in Model Fallback Routing Trace Review would be hardest to detect before it compounds?
- How would Model Fallback Routing Trace Review behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Model Fallback Routing Trace Review?
...
- A contract and failure-label set for Model Fallback Routing Trace Review.
...
- [ ] The system behind Model Fallback Routing Trace Review is described more clearly than the prompt surface.
