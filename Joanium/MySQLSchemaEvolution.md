---
name: MySQL Schema Evolution
trigger: mysql schema evolution, help with mysql schema evolution, plan mysql schema evolution, improve mysql schema evolution, expert mysql schema evolution
description: Expert-level guidance for mysql schema evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

MySQL Schema Evolution is an expert data-systems skill for MySQL. The focal concern is Schema Evolution, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when MySQL Schema Evolution affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would MySQL Schema Evolution create if two teams interpreted it differently?
- Which recovery path for MySQL Schema Evolution is the least understood today?
- How will MySQL Schema Evolution behave under backfills, reprocessing, or delayed sources?
- What cost pattern in MySQL Schema Evolution is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside MySQL Schema Evolution drifted subtly?
...
- A semantic contract note for MySQL Schema Evolution.
...
- [ ] The meaning of MySQL Schema Evolution is written in language consumers will actually use.
