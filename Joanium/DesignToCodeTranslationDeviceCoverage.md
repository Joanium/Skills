---
name: Design to Code Translation Device Coverage
trigger: design to code translation device coverage, help with design to code translation device coverage, plan design to code translation device coverage, improve design to code translation device coverage, expert design to code translation device coverage
description: Expert-level guidance for design to code translation device coverage with action-oriented workflow, review criteria, and failure patterns.
---

Design to Code Translation Device Coverage is an expert frontend implementation skill for Design to Code Translation. The focal concern is Device Coverage, but the real objective is helping an AI create UI behavior that stays understandable when state, rendering, latency, and devices stop cooperating.

## When To Use

- Use this when Design to Code Translation Device Coverage affects whether a frontend change will remain coherent outside the happy path.
...
- What part of Design to Code Translation Device Coverage becomes hardest to reason about when the UI is slow or inconsistent?
- Which state transition inside Design to Code Translation Device Coverage would fail silently without stronger instrumentation?
- How does Design to Code Translation Device Coverage behave when the browser, network, or input method stops being ideal?
- What implementation choice in Design to Code Translation Device Coverage most threatens the original design intent?
- Which regression in Design to Code Translation Device Coverage would users notice long before the team does?
...
- A state and edge-case map for Design to Code Translation Device Coverage.
...
- [ ] The behavioral contract behind Design to Code Translation Device Coverage is clearer than the implementation shortcuts.
