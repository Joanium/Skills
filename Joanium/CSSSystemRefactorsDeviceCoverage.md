---
name: CSS System Refactors Device Coverage
trigger: css system refactors device coverage, help with css system refactors device coverage, plan css system refactors device coverage, improve css system refactors device coverage, expert css system refactors device coverage
description: Expert-level guidance for css system refactors device coverage with action-oriented workflow, review criteria, and failure patterns.
---

CSS System Refactors Device Coverage is an expert frontend implementation skill for CSS System Refactors. The focal concern is Device Coverage, but the real objective is helping an AI create UI behavior that stays understandable when state, rendering, latency, and devices stop cooperating.

## When To Use

- Use this when CSS System Refactors Device Coverage affects whether a frontend change will remain coherent outside the happy path.
...
- What part of CSS System Refactors Device Coverage becomes hardest to reason about when the UI is slow or inconsistent?
- Which state transition inside CSS System Refactors Device Coverage would fail silently without stronger instrumentation?
- How does CSS System Refactors Device Coverage behave when the browser, network, or input method stops being ideal?
- What implementation choice in CSS System Refactors Device Coverage most threatens the original design intent?
- Which regression in CSS System Refactors Device Coverage would users notice long before the team does?
...
- A state and edge-case map for CSS System Refactors Device Coverage.
...
- [ ] The behavioral contract behind CSS System Refactors Device Coverage is clearer than the implementation shortcuts.
