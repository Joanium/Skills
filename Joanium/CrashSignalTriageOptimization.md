---
name: Crash Signal Triage Optimization
trigger: crash signal triage optimization, help with crash signal triage optimization, plan crash signal triage optimization, improve crash signal triage optimization, expert crash signal triage optimization
description: Expert-level guidance for crash signal triage optimization, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Crash Signal Triage Optimization is an expert-level client platform skill for shipping resilient mobile or desktop behavior across devices, stores, networks, and operating-system constraints. The emphasis here is improving the limiting resource without merely moving cost, fragility, or risk somewhere else.

## When To Use

- Use this when Crash Signal Triage Optimization affects release risk, device behavior, or user trust during degraded conditions.
...
- What is the actual bottleneck in Crash Signal Triage Optimization: CPU, memory, latency, network, human time, or policy friction?
...
- A baseline report for Crash Signal Triage Optimization with representative workloads.
