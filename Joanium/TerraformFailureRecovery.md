---
name: Terraform Failure Recovery
trigger: terraform failure recovery, help with terraform failure recovery, plan terraform failure recovery, improve terraform failure recovery, expert terraform failure recovery
description: Expert-level guidance for terraform failure recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Terraform Failure Recovery is an expert cloud and platform skill for Terraform. The focal concern is Failure Recovery, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Terraform Failure Recovery changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Terraform Failure Recovery would fail first during a rushed rollout or incident?
- Which control in Terraform Failure Recovery is automated but not truly reversible?
- How will operators know whether Terraform Failure Recovery is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Terraform Failure Recovery is invisible until it becomes expensive?
- Which dependency makes Terraform Failure Recovery less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Terraform Failure Recovery.
...
- [ ] The rollback path for Terraform Failure Recovery is explicit and tested.
