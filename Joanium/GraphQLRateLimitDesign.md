---
name: GraphQL Rate Limit Design
trigger: graphql rate limit design, help with graphql rate limit design, plan graphql rate limit design, improve graphql rate limit design, expert graphql rate limit design
description: Expert-level guidance for graphql rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

GraphQL Rate Limit Design is an expert backend skill for GraphQL. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when GraphQL Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside GraphQL Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in GraphQL Rate Limit Design?
- Where does GraphQL Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether GraphQL Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by GraphQL Rate Limit Design even if it was never written down?
...
- A contract note for GraphQL Rate Limit Design that includes failure semantics.
...
- [ ] The contract for GraphQL Rate Limit Design is explicit about success and failure behavior.
