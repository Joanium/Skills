---
name: Unit Testing Systems Oracle Design
trigger: unit testing systems oracle design, help with unit testing systems oracle design, plan unit testing systems oracle design, improve unit testing systems oracle design, expert unit testing systems oracle design
description: Expert-level guidance for unit testing systems oracle design with action-oriented workflow, review criteria, and failure patterns.
---

Unit Testing Systems Oracle Design is an expert testing and quality skill for Unit Testing Systems. The focal concern is Oracle Design, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Unit Testing Systems Oracle Design needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Unit Testing Systems Oracle Design is still untested even if coverage looks healthy?
- Which assertion in Unit Testing Systems Oracle Design would stay green while the user-visible behavior regressed?
- How much of Unit Testing Systems Oracle Design depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Unit Testing Systems Oracle Design need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Unit Testing Systems Oracle Design.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Unit Testing Systems Oracle Design should mean.
...
- [ ] The key behavior protected by Unit Testing Systems Oracle Design is explicit.
