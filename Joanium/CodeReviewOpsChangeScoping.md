---
name: Code Review Operations Change Scoping
trigger: code review operations change scoping, help with code review operations change scoping, plan code review operations change scoping, improve code review operations change scoping, expert code review operations change scoping
description: Expert-level guidance for code review operations change scoping with action-oriented workflow, review criteria, and failure patterns.
---

Code Review Operations Change Scoping is an expert coding workflow skill for Code Review Operations. The focal concern is Change Scoping, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Code Review Operations Change Scoping is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Code Review Operations Change Scoping is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Code Review Operations Change Scoping would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Code Review Operations Change Scoping create a silent regression that tests may miss?
- How would the AI explain the safety of Code Review Operations Change Scoping to a skeptical reviewer?
...
- A concise evidence note for Code Review Operations Change Scoping.
...
- [ ] The evidence supporting Code Review Operations Change Scoping is stronger than the assumptions.
