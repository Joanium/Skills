---
name: Visual Diff Triage State Reset
trigger: visual diff triage state reset, help with visual diff triage state reset, plan visual diff triage state reset, improve visual diff triage state reset, expert visual diff triage state reset
description: Expert-level guidance for visual diff triage state reset with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage State Reset is an expert browser automation skill for Visual Diff Triage. The focal concern is State Reset, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage State Reset needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage State Reset unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage State Reset is least stable under real conditions?
- How would Visual Diff Triage State Reset behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage State Reset capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage State Reset is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage State Reset.
...
- [ ] The state model behind Visual Diff Triage State Reset is explicit.
