---
name: Exploratory QA Fixture Hygiene
trigger: exploratory qa fixture hygiene, help with exploratory qa fixture hygiene, plan exploratory qa fixture hygiene, improve exploratory qa fixture hygiene, expert exploratory qa fixture hygiene
description: Expert-level guidance for exploratory qa fixture hygiene with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Fixture Hygiene is an expert testing and quality skill for Exploratory QA. The focal concern is Fixture Hygiene, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Fixture Hygiene needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Fixture Hygiene is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Fixture Hygiene would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Fixture Hygiene depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Fixture Hygiene need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Fixture Hygiene.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Fixture Hygiene should mean.
...
- [ ] The key behavior protected by Exploratory QA Fixture Hygiene is explicit.
