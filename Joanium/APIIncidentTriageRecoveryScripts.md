---
name: API Incident Triage Recovery Scripts
trigger: api incident triage recovery scripts, help with api incident triage recovery scripts, plan api incident triage recovery scripts, improve api incident triage recovery scripts, expert api incident triage recovery scripts
description: Expert-level guidance for api incident triage recovery scripts with action-oriented workflow, review criteria, and failure patterns.
---

API Incident Triage Recovery Scripts is an expert backend execution skill for API Incident Triage. The focal concern is Recovery Scripts, but the real objective is helping an AI design service behavior that stays coherent under retries, queues, partial failure, and operational pressure.

## When To Use

- Use this when API Incident Triage Recovery Scripts affects whether backend changes remain safe once real production behavior kicks in.
...
- Which failure ordering inside API Incident Triage Recovery Scripts would create the most misleading symptom?
- Where does API Incident Triage Recovery Scripts need stronger idempotency or retry discipline?
- What dependency in API Incident Triage Recovery Scripts is currently hiding behind timeout or queue noise?
- How would an operator explain the health of API Incident Triage Recovery Scripts in the first five minutes of an incident?
- What contract assumption in API Incident Triage Recovery Scripts is least defensible under partial failure?
...
- A failure-sequence note for API Incident Triage Recovery Scripts.
...
- [ ] The real behavior of API Incident Triage Recovery Scripts under retries and failures is explicit.
