---
name: dbt Query Profiling
trigger: dbt query profiling, help with dbt query profiling, plan dbt query profiling, improve dbt query profiling, expert dbt query profiling
description: Expert-level guidance for dbt query profiling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

dbt Query Profiling is an expert data-systems skill for dbt. The focal concern is Query Profiling, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when dbt Query Profiling affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would dbt Query Profiling create if two teams interpreted it differently?
- Which recovery path for dbt Query Profiling is the least understood today?
- How will dbt Query Profiling behave under backfills, reprocessing, or delayed sources?
- What cost pattern in dbt Query Profiling is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside dbt Query Profiling drifted subtly?
...
- A semantic contract note for dbt Query Profiling.
...
- [ ] The meaning of dbt Query Profiling is written in language consumers will actually use.
