---
name: Architecture Fitness Functions Strategy
trigger: architecture fitness functions strategy, help with architecture fitness functions strategy, plan architecture fitness functions strategy, improve architecture fitness functions strategy, expert architecture fitness functions strategy
description: Expert-level guidance for architecture fitness functions strategy, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Architecture Fitness Functions Strategy is an expert-level engineering systems skill for making architecture, standards, and change management explicit enough to scale beyond individual memory. The emphasis here is on choosing the operating model, decision boundaries, and success criteria before local optimizations become policy by accident.

## When To Use

- Use this when Architecture Fitness Functions Strategy affects codebase shape, team interfaces, or long-term delivery speed.
...
- What decision is Architecture Fitness Functions Strategy supposed to improve over the next planning horizon?
- Which constraints are fixed, and which are merely habits that can be challenged?
- Where does failure in Architecture Fitness Functions Strategy create the largest technical, business, or trust cost?
...
- A written decision statement for Architecture Fitness Functions Strategy.
