---
name: Visual Diff Triage Selector Resilience
trigger: visual diff triage selector resilience, help with visual diff triage selector resilience, plan visual diff triage selector resilience, improve visual diff triage selector resilience, expert visual diff triage selector resilience
description: Expert-level guidance for visual diff triage selector resilience with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage Selector Resilience is an expert browser automation skill for Visual Diff Triage. The focal concern is Selector Resilience, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage Selector Resilience needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage Selector Resilience unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage Selector Resilience is least stable under real conditions?
- How would Visual Diff Triage Selector Resilience behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage Selector Resilience capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage Selector Resilience is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage Selector Resilience.
...
- [ ] The state model behind Visual Diff Triage Selector Resilience is explicit.
