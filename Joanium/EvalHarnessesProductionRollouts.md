---
name: Evaluation Harnesses Production Rollouts
trigger: evaluation harnesses production rollouts, help with evaluation harnesses production rollouts, plan evaluation harnesses production rollouts, improve evaluation harnesses production rollouts, expert evaluation harnesses production rollouts
description: Expert-level guidance for evaluation harnesses production rollouts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Evaluation Harnesses Production Rollouts is an expert applied-AI skill for Evaluation Harnesses. The focal concern is Production Rollouts, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Evaluation Harnesses Production Rollouts affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Evaluation Harnesses Production Rollouts would look acceptable in a demo but fail under production ambiguity?
- Which part of Evaluation Harnesses Production Rollouts lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Evaluation Harnesses Production Rollouts fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Evaluation Harnesses Production Rollouts is getting better rather than merely changing?
- Where should humans intervene in Evaluation Harnesses Production Rollouts to add leverage instead of toil?
...
- A behavior contract for Evaluation Harnesses Production Rollouts.
...
- [ ] The behavior contract for Evaluation Harnesses Production Rollouts is explicit.
