---
name: Test Data Factories Parallel Execution
trigger: test data factories parallel execution, help with test data factories parallel execution, plan test data factories parallel execution, improve test data factories parallel execution, expert test data factories parallel execution
description: Expert-level guidance for test data factories parallel execution with action-oriented workflow, review criteria, and failure patterns.
---

Test Data Factories Parallel Execution is an expert testing and quality skill for Test Data Factories. The focal concern is Parallel Execution, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Test Data Factories Parallel Execution needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Test Data Factories Parallel Execution is still untested even if coverage looks healthy?
- Which assertion in Test Data Factories Parallel Execution would stay green while the user-visible behavior regressed?
- How much of Test Data Factories Parallel Execution depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Test Data Factories Parallel Execution need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Test Data Factories Parallel Execution.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Test Data Factories Parallel Execution should mean.
...
- [ ] The key behavior protected by Test Data Factories Parallel Execution is explicit.
