---
name: Visual Diff Triage User Simulation
trigger: visual diff triage user simulation, help with visual diff triage user simulation, plan visual diff triage user simulation, improve visual diff triage user simulation, expert visual diff triage user simulation
description: Expert-level guidance for visual diff triage user simulation with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage User Simulation is an expert browser automation skill for Visual Diff Triage. The focal concern is User Simulation, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage User Simulation needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage User Simulation unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage User Simulation is least stable under real conditions?
- How would Visual Diff Triage User Simulation behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage User Simulation capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage User Simulation is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage User Simulation.
...
- [ ] The state model behind Visual Diff Triage User Simulation is explicit.
