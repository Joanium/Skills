---
name: Swift Type Architecture
trigger: swift type architecture, help with swift type architecture, plan swift type architecture, improve swift type architecture, expert swift type architecture
description: Expert-level guidance for swift type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Swift Type Architecture is an expert programming-language skill for Swift. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Swift Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Swift Type Architecture?
- Which abstraction in Swift Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Swift Type Architecture first?
- How will Swift Type Architecture behave when multiple contributors evolve it independently?
- Which part of Swift Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Swift Type Architecture.
- A failure-mode note that explains the sharp edges inside Swift Type Architecture.
...
- [ ] The main language-level risk in Swift Type Architecture is named explicitly.
...
- [ ] Future contributors can extend Swift Type Architecture without reverse-engineering hidden contracts.
