---
name: RAG Systems Trace Analysis
trigger: rag systems trace analysis, help with rag systems trace analysis, plan rag systems trace analysis, improve rag systems trace analysis, expert rag systems trace analysis
description: Expert-level guidance for rag systems trace analysis with action-oriented workflow, review criteria, and failure patterns.
---

RAG Systems Trace Analysis is an expert applied AI skill for RAG Systems. The focal concern is Trace Analysis, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when RAG Systems Trace Analysis affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of RAG Systems Trace Analysis is currently measured by vibes rather than a real contract?
- Which failure in RAG Systems Trace Analysis would be hardest to notice before it harms users or operators?
- How will RAG Systems Trace Analysis behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside RAG Systems Trace Analysis?
...
- A behavior contract for RAG Systems Trace Analysis.
...
- [ ] The contract behind RAG Systems Trace Analysis is explicit enough to evaluate.
