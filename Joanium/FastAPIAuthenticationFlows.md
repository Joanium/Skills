---
name: FastAPI Authentication Flows
trigger: fastapi authentication flows, help with fastapi authentication flows, plan fastapi authentication flows, improve fastapi authentication flows, expert fastapi authentication flows
description: Expert-level guidance for fastapi authentication flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

FastAPI Authentication Flows is an expert backend skill for FastAPI. The focal concern is Authentication Flows, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when FastAPI Authentication Flows shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside FastAPI Authentication Flows breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in FastAPI Authentication Flows?
- Where does FastAPI Authentication Flows need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether FastAPI Authentication Flows is unhealthy before customers do?
- What compatibility promise is implied by FastAPI Authentication Flows even if it was never written down?
...
- A contract note for FastAPI Authentication Flows that includes failure semantics.
...
- [ ] The contract for FastAPI Authentication Flows is explicit about success and failure behavior.
