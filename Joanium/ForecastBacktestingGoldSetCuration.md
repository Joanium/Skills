---
name: Forecast Backtesting Gold Set Curation
trigger: forecast backtesting gold set curation, help with forecast backtesting gold set curation, plan forecast backtesting gold set curation, improve forecast backtesting gold set curation, expert forecast backtesting gold set curation
description: Expert-level guidance for forecast backtesting gold set curation with action-oriented workflow, review criteria, and failure patterns.
---

Forecast Backtesting Gold Set Curation is an expert machine learning operations skill for Forecast Backtesting. The focal concern is Gold Set Curation, but the real objective is helping an AI create ML systems that stay measurable, diagnosable, and useful beyond one successful training run.

## When To Use

- Use this when Forecast Backtesting Gold Set Curation determines whether an ML system remains interpretable after training and deployment.
...
- Which slice or label assumption inside Forecast Backtesting Gold Set Curation is least likely to hold in production?
- How will Forecast Backtesting Gold Set Curation distinguish drift, bad labels, and threshold error?
- What human disagreement inside Forecast Backtesting Gold Set Curation should become a calibration event instead of noise?
- What deployment gate should stop a model that looks globally good but locally harmful?
- How quickly should Forecast Backtesting Gold Set Curation be reviewed again after the environment changes?
...
- A slice, label, and threshold note for Forecast Backtesting Gold Set Curation.
...
- [ ] The task, slices, and wrong-answer cost behind Forecast Backtesting Gold Set Curation are explicit.
