---
name: gRPC Authentication Flows
trigger: grpc authentication flows, help with grpc authentication flows, plan grpc authentication flows, improve grpc authentication flows, expert grpc authentication flows
description: Expert-level guidance for grpc authentication flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

gRPC Authentication Flows is an expert backend skill for gRPC. The focal concern is Authentication Flows, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when gRPC Authentication Flows shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside gRPC Authentication Flows breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in gRPC Authentication Flows?
- Where does gRPC Authentication Flows need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether gRPC Authentication Flows is unhealthy before customers do?
- What compatibility promise is implied by gRPC Authentication Flows even if it was never written down?
...
- A contract note for gRPC Authentication Flows that includes failure semantics.
...
- [ ] The contract for gRPC Authentication Flows is explicit about success and failure behavior.
