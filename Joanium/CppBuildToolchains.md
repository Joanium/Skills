---
name: C++ Build Toolchains
trigger: c++ build toolchains, help with c++ build toolchains, plan c++ build toolchains, improve c++ build toolchains, expert c++ build toolchains
description: Expert-level guidance for c++ build toolchains with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

C++ Build Toolchains is an expert programming-language skill for C++. The focal concern is Build Toolchains, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when C++ Build Toolchains affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside C++ Build Toolchains?
- Which abstraction in C++ Build Toolchains becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in C++ Build Toolchains first?
- How will C++ Build Toolchains behave when multiple contributors evolve it independently?
- Which part of C++ Build Toolchains is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for C++ Build Toolchains.
- A failure-mode note that explains the sharp edges inside C++ Build Toolchains.
...
- [ ] The main language-level risk in C++ Build Toolchains is named explicitly.
...
- [ ] Future contributors can extend C++ Build Toolchains without reverse-engineering hidden contracts.
