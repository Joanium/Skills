---
name: Review Queue Design Trace Review
trigger: review queue design trace review, help with review queue design trace review, plan review queue design trace review, improve review queue design trace review, expert review queue design trace review
description: Expert-level guidance for review queue design trace review with action-oriented workflow, review criteria, and failure patterns.
---

Review Queue Design Trace Review is an expert agent and tooling skill for Review Queue Design. The focal concern is Trace Review, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Review Queue Design Trace Review affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Review Queue Design Trace Review is still governed by intuition instead of a contract or label set?
- Which failure in Review Queue Design Trace Review would be hardest to detect before it compounds?
- How would Review Queue Design Trace Review behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Review Queue Design Trace Review?
...
- A contract and failure-label set for Review Queue Design Trace Review.
...
- [ ] The system behind Review Queue Design Trace Review is described more clearly than the prompt surface.
