---
name: Terraform Observability Design
trigger: terraform observability design, help with terraform observability design, plan terraform observability design, improve terraform observability design, expert terraform observability design
description: Expert-level guidance for terraform observability design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Terraform Observability Design is an expert cloud and platform skill for Terraform. The focal concern is Observability Design, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Terraform Observability Design changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Terraform Observability Design would fail first during a rushed rollout or incident?
- Which control in Terraform Observability Design is automated but not truly reversible?
- How will operators know whether Terraform Observability Design is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Terraform Observability Design is invisible until it becomes expensive?
- Which dependency makes Terraform Observability Design less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Terraform Observability Design.
...
- [ ] The rollback path for Terraform Observability Design is explicit and tested.
