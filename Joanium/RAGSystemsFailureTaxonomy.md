---
name: RAG Systems Failure Taxonomy
trigger: rag systems failure taxonomy, help with rag systems failure taxonomy, plan rag systems failure taxonomy, improve rag systems failure taxonomy, expert rag systems failure taxonomy
description: Expert-level guidance for rag systems failure taxonomy with action-oriented workflow, review criteria, and failure patterns.
---

RAG Systems Failure Taxonomy is an expert applied AI skill for RAG Systems. The focal concern is Failure Taxonomy, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when RAG Systems Failure Taxonomy affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of RAG Systems Failure Taxonomy is currently measured by vibes rather than a real contract?
- Which failure in RAG Systems Failure Taxonomy would be hardest to notice before it harms users or operators?
- How will RAG Systems Failure Taxonomy behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside RAG Systems Failure Taxonomy?
...
- A behavior contract for RAG Systems Failure Taxonomy.
...
- [ ] The contract behind RAG Systems Failure Taxonomy is explicit enough to evaluate.
