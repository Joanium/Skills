---
name: NestJS Observability Contracts
trigger: nestjs observability contracts, help with nestjs observability contracts, plan nestjs observability contracts, improve nestjs observability contracts, expert nestjs observability contracts
description: Expert-level guidance for nestjs observability contracts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

NestJS Observability Contracts is an expert backend skill for NestJS. The focal concern is Observability Contracts, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when NestJS Observability Contracts shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside NestJS Observability Contracts breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in NestJS Observability Contracts?
- Where does NestJS Observability Contracts need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether NestJS Observability Contracts is unhealthy before customers do?
- What compatibility promise is implied by NestJS Observability Contracts even if it was never written down?
...
- A contract note for NestJS Observability Contracts that includes failure semantics.
...
- [ ] The contract for NestJS Observability Contracts is explicit about success and failure behavior.
