---
name: Next.js Routing Boundaries
trigger: next.js routing boundaries, help with next.js routing boundaries, plan next.js routing boundaries, improve next.js routing boundaries, expert next.js routing boundaries
description: Expert-level guidance for next.js routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Next.js Routing Boundaries is an expert frontend skill for Next.js. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Next.js Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Next.js Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in Next.js Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Next.js Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does Next.js Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Next.js Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for Next.js Routing Boundaries.
...
- [ ] The state model for Next.js Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend Next.js Routing Boundaries without guessing hidden UI contracts.
