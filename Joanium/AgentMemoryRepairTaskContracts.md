---
name: Agent Memory Repair Task Contracts
trigger: agent memory repair task contracts, help with agent memory repair task contracts, plan agent memory repair task contracts, improve agent memory repair task contracts, expert agent memory repair task contracts
description: Expert-level guidance for agent memory repair task contracts with action-oriented workflow, review criteria, and failure patterns.
---

Agent Memory Repair Task Contracts is an expert agent and tooling skill for Agent Memory Repair. The focal concern is Task Contracts, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Agent Memory Repair Task Contracts affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Agent Memory Repair Task Contracts is still governed by intuition instead of a contract or label set?
- Which failure in Agent Memory Repair Task Contracts would be hardest to detect before it compounds?
- How would Agent Memory Repair Task Contracts behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Agent Memory Repair Task Contracts?
...
- A contract and failure-label set for Agent Memory Repair Task Contracts.
...
- [ ] The system behind Agent Memory Repair Task Contracts is described more clearly than the prompt surface.
