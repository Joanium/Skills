---
name: C++ Test Harness Design
trigger: c++ test harness design, help with c++ test harness design, plan c++ test harness design, improve c++ test harness design, expert c++ test harness design
description: Expert-level guidance for c++ test harness design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

C++ Test Harness Design is an expert programming-language skill for C++. The focal concern is Test Harness Design, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when C++ Test Harness Design affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside C++ Test Harness Design?
- Which abstraction in C++ Test Harness Design becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in C++ Test Harness Design first?
- How will C++ Test Harness Design behave when multiple contributors evolve it independently?
- Which part of C++ Test Harness Design is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for C++ Test Harness Design.
- A failure-mode note that explains the sharp edges inside C++ Test Harness Design.
...
- [ ] The main language-level risk in C++ Test Harness Design is named explicitly.
...
- [ ] Future contributors can extend C++ Test Harness Design without reverse-engineering hidden contracts.
