---
name: Express Rate Limit Design
trigger: express rate limit design, help with express rate limit design, plan express rate limit design, improve express rate limit design, expert express rate limit design
description: Expert-level guidance for express rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Express Rate Limit Design is an expert backend skill for Express. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Express Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Express Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Express Rate Limit Design?
- Where does Express Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Express Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by Express Rate Limit Design even if it was never written down?
...
- A contract note for Express Rate Limit Design that includes failure semantics.
...
- [ ] The contract for Express Rate Limit Design is explicit about success and failure behavior.
