---
name: Test Data Factories Fixture Hygiene
trigger: test data factories fixture hygiene, help with test data factories fixture hygiene, plan test data factories fixture hygiene, improve test data factories fixture hygiene, expert test data factories fixture hygiene
description: Expert-level guidance for test data factories fixture hygiene with action-oriented workflow, review criteria, and failure patterns.
---

Test Data Factories Fixture Hygiene is an expert testing and quality skill for Test Data Factories. The focal concern is Fixture Hygiene, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Test Data Factories Fixture Hygiene needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Test Data Factories Fixture Hygiene is still untested even if coverage looks healthy?
- Which assertion in Test Data Factories Fixture Hygiene would stay green while the user-visible behavior regressed?
- How much of Test Data Factories Fixture Hygiene depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Test Data Factories Fixture Hygiene need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Test Data Factories Fixture Hygiene.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Test Data Factories Fixture Hygiene should mean.
...
- [ ] The key behavior protected by Test Data Factories Fixture Hygiene is explicit.
