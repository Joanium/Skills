---
name: FastAPI Multi-Tenant Isolation
trigger: fastapi multi-tenant isolation, help with fastapi multi-tenant isolation, plan fastapi multi-tenant isolation, improve fastapi multi-tenant isolation, expert fastapi multi-tenant isolation
description: Expert-level guidance for fastapi multi-tenant isolation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

FastAPI Multi-Tenant Isolation is an expert backend skill for FastAPI. The focal concern is Multi-Tenant Isolation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when FastAPI Multi-Tenant Isolation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside FastAPI Multi-Tenant Isolation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in FastAPI Multi-Tenant Isolation?
- Where does FastAPI Multi-Tenant Isolation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether FastAPI Multi-Tenant Isolation is unhealthy before customers do?
- What compatibility promise is implied by FastAPI Multi-Tenant Isolation even if it was never written down?
...
- A contract note for FastAPI Multi-Tenant Isolation that includes failure semantics.
...
- [ ] The contract for FastAPI Multi-Tenant Isolation is explicit about success and failure behavior.
