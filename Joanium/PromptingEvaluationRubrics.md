---
name: Prompting Evaluation Rubrics
trigger: prompting evaluation rubrics, help with prompting evaluation rubrics, plan prompting evaluation rubrics, improve prompting evaluation rubrics, expert prompting evaluation rubrics
description: Expert-level guidance for prompting evaluation rubrics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Prompting Evaluation Rubrics is an expert applied-AI skill for Prompting. The focal concern is Evaluation Rubrics, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Prompting Evaluation Rubrics affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Prompting Evaluation Rubrics would look acceptable in a demo but fail under production ambiguity?
- Which part of Prompting Evaluation Rubrics lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Prompting Evaluation Rubrics fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Prompting Evaluation Rubrics is getting better rather than merely changing?
- Where should humans intervene in Prompting Evaluation Rubrics to add leverage instead of toil?
...
- A behavior contract for Prompting Evaluation Rubrics.
...
- [ ] The behavior contract for Prompting Evaluation Rubrics is explicit.
