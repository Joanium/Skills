---
name: Dependency Upgrade Edge Case Sweeps
trigger: dependency upgrade edge case sweeps, help with dependency upgrade edge case sweeps, plan dependency upgrade edge case sweeps, improve dependency upgrade edge case sweeps, expert dependency upgrade edge case sweeps
description: Expert-level guidance for dependency upgrade edge case sweeps with action-oriented workflow, review criteria, and failure patterns.
---

Dependency Upgrade Edge Case Sweeps is an expert coding workflow skill for Dependency Upgrade. The focal concern is Edge Case Sweeps, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Dependency Upgrade Edge Case Sweeps is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Dependency Upgrade Edge Case Sweeps is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Dependency Upgrade Edge Case Sweeps would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Dependency Upgrade Edge Case Sweeps create a silent regression that tests may miss?
- How would the AI explain the safety of Dependency Upgrade Edge Case Sweeps to a skeptical reviewer?
...
- A concise evidence note for Dependency Upgrade Edge Case Sweeps.
...
- [ ] The evidence supporting Dependency Upgrade Edge Case Sweeps is stronger than the assumptions.
