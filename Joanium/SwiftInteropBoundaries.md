---
name: Swift Interop Boundaries
trigger: swift interop boundaries, help with swift interop boundaries, plan swift interop boundaries, improve swift interop boundaries, expert swift interop boundaries
description: Expert-level guidance for swift interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Swift Interop Boundaries is an expert programming-language skill for Swift. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Swift Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Swift Interop Boundaries?
- Which abstraction in Swift Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Swift Interop Boundaries first?
- How will Swift Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of Swift Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Swift Interop Boundaries.
- A failure-mode note that explains the sharp edges inside Swift Interop Boundaries.
...
- [ ] The main language-level risk in Swift Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend Swift Interop Boundaries without reverse-engineering hidden contracts.
