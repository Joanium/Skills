...
trigger: typescript type architecture, help with typescript type architecture, plan typescript type architecture, improve typescript type architecture, expert typescript type architecture
description: Expert-level guidance for typescript type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

TypeScript Type Architecture is an expert programming-language skill for TypeScript. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when TypeScript Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside TypeScript Type Architecture?
- Which abstraction in TypeScript Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in TypeScript Type Architecture first?
- How will TypeScript Type Architecture behave when multiple contributors evolve it independently?
- Which part of TypeScript Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for TypeScript Type Architecture.
- A failure-mode note that explains the sharp edges inside TypeScript Type Architecture.
...
- [ ] The main language-level risk in TypeScript Type Architecture is named explicitly.
...
- [ ] Future contributors can extend TypeScript Type Architecture without reverse-engineering hidden contracts.
