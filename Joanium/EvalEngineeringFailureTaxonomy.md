---
name: Evaluation Engineering Failure Taxonomy
trigger: evaluation engineering failure taxonomy, help with evaluation engineering failure taxonomy, plan evaluation engineering failure taxonomy, improve evaluation engineering failure taxonomy, expert evaluation engineering failure taxonomy
description: Expert-level guidance for evaluation engineering failure taxonomy with action-oriented workflow, review criteria, and failure patterns.
---

Evaluation Engineering Failure Taxonomy is an expert applied AI skill for Evaluation Engineering. The focal concern is Failure Taxonomy, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Evaluation Engineering Failure Taxonomy affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Evaluation Engineering Failure Taxonomy is currently measured by vibes rather than a real contract?
- Which failure in Evaluation Engineering Failure Taxonomy would be hardest to notice before it harms users or operators?
- How will Evaluation Engineering Failure Taxonomy behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Evaluation Engineering Failure Taxonomy?
...
- A behavior contract for Evaluation Engineering Failure Taxonomy.
...
- [ ] The contract behind Evaluation Engineering Failure Taxonomy is explicit enough to evaluate.
