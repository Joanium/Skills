---
name: Pull Request Triage Staging Plans
trigger: pull request triage staging plans, help with pull request triage staging plans, plan pull request triage staging plans, improve pull request triage staging plans, expert pull request triage staging plans
description: Expert-level guidance for pull request triage staging plans with action-oriented workflow, review criteria, and failure patterns.
---

Pull Request Triage Staging Plans is an expert repository and delivery skill for Pull Request Triage. The focal concern is Staging Plans, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Pull Request Triage Staging Plans determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Pull Request Triage Staging Plans riskier than the diff itself suggests?
- Which reviewer would challenge Pull Request Triage Staging Plans first, and what evidence would they need?
- How could Pull Request Triage Staging Plans fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Pull Request Triage Staging Plans would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Pull Request Triage Staging Plans concretely?
...
- A blast-radius note for Pull Request Triage Staging Plans.
...
- [ ] The repository-level blast radius of Pull Request Triage Staging Plans is explicit.
