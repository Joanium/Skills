---
name: Next.js Error Recovery UX
trigger: next.js error recovery ux, help with next.js error recovery ux, plan next.js error recovery ux, improve next.js error recovery ux, expert next.js error recovery ux
description: Expert-level guidance for next.js error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Next.js Error Recovery UX is an expert frontend skill for Next.js. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Next.js Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Next.js Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in Next.js Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Next.js Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does Next.js Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Next.js Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for Next.js Error Recovery UX.
...
- [ ] The state model for Next.js Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend Next.js Error Recovery UX without guessing hidden UI contracts.
