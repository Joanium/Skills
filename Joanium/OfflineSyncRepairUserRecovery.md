---
name: Offline Sync Repair User Recovery
trigger: offline sync repair user recovery, help with offline sync repair user recovery, plan offline sync repair user recovery, improve offline sync repair user recovery, expert offline sync repair user recovery
description: Expert-level guidance for offline sync repair user recovery with client delivery workflow, review criteria, and failure patterns.
---

Offline Sync Repair User Recovery is an expert mobile and desktop delivery skill for Offline Sync Repair. The focal concern is User Recovery, but the real objective is helping an AI ship client software that survives device constraints, stores, upgrades, and real user recovery paths.

## When To Use

- Use this when Offline Sync Repair User Recovery affects whether client software remains trustworthy across devices, upgrades, and degraded conditions.
...
- Which user or device condition would expose the weakest assumption in Offline Sync Repair User Recovery first?
- How will Offline Sync Repair User Recovery behave during upgrade, rollback, or stale local state?
- What store, signing, or permission dependency could block or confuse Offline Sync Repair User Recovery unexpectedly?
- How would support or operations diagnose Offline Sync Repair User Recovery from telemetry instead of guesswork?
- What path inside Offline Sync Repair User Recovery should help the user recover rather than merely fail loudly?
...
- A device and release risk note for Offline Sync Repair User Recovery.
...
- [ ] The device, store, and upgrade assumptions behind Offline Sync Repair User Recovery are explicit.
