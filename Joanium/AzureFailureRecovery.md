---
name: Azure Failure Recovery
trigger: azure failure recovery, help with azure failure recovery, plan azure failure recovery, improve azure failure recovery, expert azure failure recovery
description: Expert-level guidance for azure failure recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Azure Failure Recovery is an expert cloud and platform skill for Azure. The focal concern is Failure Recovery, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Azure Failure Recovery changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Azure Failure Recovery would fail first during a rushed rollout or incident?
- Which control in Azure Failure Recovery is automated but not truly reversible?
- How will operators know whether Azure Failure Recovery is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Azure Failure Recovery is invisible until it becomes expensive?
- Which dependency makes Azure Failure Recovery less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Azure Failure Recovery.
...
- [ ] The rollback path for Azure Failure Recovery is explicit and tested.
