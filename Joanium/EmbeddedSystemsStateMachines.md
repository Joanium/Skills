---
name: Embedded Systems State Machines
trigger: embedded systems state machines, help with embedded systems state machines, plan embedded systems state machines, improve embedded systems state machines, expert embedded systems state machines
description: Expert-level guidance for embedded systems state machines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Embedded Systems State Machines is an expert systems skill for Embedded Systems. The focal concern is State Machines, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Embedded Systems State Machines is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Embedded Systems State Machines would break first under scale, timing, or hardware stress?
- Which state or resource in Embedded Systems State Machines is hardest to observe when behavior goes wrong?
- How does Embedded Systems State Machines trade correctness, throughput, and recoverability today?
- What tooling gap makes Embedded Systems State Machines harder to reason about than it should be?
- Which optimization target inside Embedded Systems State Machines is attractive but strategically secondary?
...
- A system model for Embedded Systems State Machines that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Embedded Systems State Machines is explicit.
