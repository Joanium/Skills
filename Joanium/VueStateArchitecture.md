---
name: Vue State Architecture
trigger: vue state architecture, help with vue state architecture, plan vue state architecture, improve vue state architecture, expert vue state architecture
description: Expert-level guidance for vue state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Vue State Architecture is an expert frontend skill for Vue. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Vue State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Vue State Architecture make user state harder to reason about than it needs to be?
- Which interaction in Vue State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Vue State Architecture would break if the DOM or state changed unexpectedly?
- How does Vue State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Vue State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for Vue State Architecture.
...
- [ ] The state model for Vue State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend Vue State Architecture without guessing hidden UI contracts.
