---
name: Flaky Test Repair Coverage Strategy
trigger: flaky test repair coverage strategy, help with flaky test repair coverage strategy, plan flaky test repair coverage strategy, improve flaky test repair coverage strategy, expert flaky test repair coverage strategy
description: Expert-level guidance for flaky test repair coverage strategy with action-oriented workflow, review criteria, and failure patterns.
---

Flaky Test Repair Coverage Strategy is an expert testing and quality skill for Flaky Test Repair. The focal concern is Coverage Strategy, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Flaky Test Repair Coverage Strategy needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Flaky Test Repair Coverage Strategy is still untested even if coverage looks healthy?
- Which assertion in Flaky Test Repair Coverage Strategy would stay green while the user-visible behavior regressed?
- How much of Flaky Test Repair Coverage Strategy depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Flaky Test Repair Coverage Strategy need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Flaky Test Repair Coverage Strategy.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Flaky Test Repair Coverage Strategy should mean.
...
- [ ] The key behavior protected by Flaky Test Repair Coverage Strategy is explicit.
