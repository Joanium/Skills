---
name: Dependency Upgrade Risk Ranking
trigger: dependency upgrade risk ranking, help with dependency upgrade risk ranking, plan dependency upgrade risk ranking, improve dependency upgrade risk ranking, expert dependency upgrade risk ranking
description: Expert-level guidance for dependency upgrade risk ranking with action-oriented workflow, review criteria, and failure patterns.
---

Dependency Upgrade Risk Ranking is an expert coding workflow skill for Dependency Upgrade. The focal concern is Risk Ranking, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Dependency Upgrade Risk Ranking is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Dependency Upgrade Risk Ranking is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Dependency Upgrade Risk Ranking would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Dependency Upgrade Risk Ranking create a silent regression that tests may miss?
- How would the AI explain the safety of Dependency Upgrade Risk Ranking to a skeptical reviewer?
...
- A concise evidence note for Dependency Upgrade Risk Ranking.
...
- [ ] The evidence supporting Dependency Upgrade Risk Ranking is stronger than the assumptions.
