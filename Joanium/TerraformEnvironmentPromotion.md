...
trigger: terraform environment promotion, help with terraform environment promotion, plan terraform environment promotion, improve terraform environment promotion, expert terraform environment promotion
description: Expert-level guidance for terraform environment promotion with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Terraform Environment Promotion is an expert cloud and platform skill for Terraform. The focal concern is Environment Promotion, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Terraform Environment Promotion changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Terraform Environment Promotion would fail first during a rushed rollout or incident?
- Which control in Terraform Environment Promotion is automated but not truly reversible?
- How will operators know whether Terraform Environment Promotion is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Terraform Environment Promotion is invisible until it becomes expensive?
- Which dependency makes Terraform Environment Promotion less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Terraform Environment Promotion.
...
- [ ] The rollback path for Terraform Environment Promotion is explicit and tested.
