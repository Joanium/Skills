---
name: SolidJS Hydration Recovery
trigger: solidjs hydration recovery, help with solidjs hydration recovery, plan solidjs hydration recovery, improve solidjs hydration recovery, expert solidjs hydration recovery
description: Expert-level guidance for solidjs hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

SolidJS Hydration Recovery is an expert frontend skill for SolidJS. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when SolidJS Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does SolidJS Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in SolidJS Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in SolidJS Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does SolidJS Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of SolidJS Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for SolidJS Hydration Recovery.
...
- [ ] The state model for SolidJS Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend SolidJS Hydration Recovery without guessing hidden UI contracts.
