---
name: Test Data Factories Triage Heuristics
trigger: test data factories triage heuristics, help with test data factories triage heuristics, plan test data factories triage heuristics, improve test data factories triage heuristics, expert test data factories triage heuristics
description: Expert-level guidance for test data factories triage heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Test Data Factories Triage Heuristics is an expert testing and quality skill for Test Data Factories. The focal concern is Triage Heuristics, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Test Data Factories Triage Heuristics needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Test Data Factories Triage Heuristics is still untested even if coverage looks healthy?
- Which assertion in Test Data Factories Triage Heuristics would stay green while the user-visible behavior regressed?
- How much of Test Data Factories Triage Heuristics depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Test Data Factories Triage Heuristics need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Test Data Factories Triage Heuristics.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Test Data Factories Triage Heuristics should mean.
...
- [ ] The key behavior protected by Test Data Factories Triage Heuristics is explicit.
