---
name: Flaky Test Repair Boundary Scenarios
trigger: flaky test repair boundary scenarios, help with flaky test repair boundary scenarios, plan flaky test repair boundary scenarios, improve flaky test repair boundary scenarios, expert flaky test repair boundary scenarios
description: Expert-level guidance for flaky test repair boundary scenarios with action-oriented workflow, review criteria, and failure patterns.
---

Flaky Test Repair Boundary Scenarios is an expert testing and quality skill for Flaky Test Repair. The focal concern is Boundary Scenarios, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Flaky Test Repair Boundary Scenarios needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Flaky Test Repair Boundary Scenarios is still untested even if coverage looks healthy?
- Which assertion in Flaky Test Repair Boundary Scenarios would stay green while the user-visible behavior regressed?
- How much of Flaky Test Repair Boundary Scenarios depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Flaky Test Repair Boundary Scenarios need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Flaky Test Repair Boundary Scenarios.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Flaky Test Repair Boundary Scenarios should mean.
...
- [ ] The key behavior protected by Flaky Test Repair Boundary Scenarios is explicit.
