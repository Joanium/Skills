---
name: Session Replay Analysis Flake Triage
trigger: session replay analysis flake triage, help with session replay analysis flake triage, plan session replay analysis flake triage, improve session replay analysis flake triage, expert session replay analysis flake triage
description: Expert-level guidance for session replay analysis flake triage with action-oriented workflow, review criteria, and failure patterns.
---

Session Replay Analysis Flake Triage is an expert browser automation skill for Session Replay Analysis. The focal concern is Flake Triage, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Session Replay Analysis Flake Triage needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Session Replay Analysis Flake Triage unreliable across runs?
- Which timing or selector assumption in Session Replay Analysis Flake Triage is least stable under real conditions?
- How would Session Replay Analysis Flake Triage behave differently across browsers, profiles, or CI environments?
- What artifacts should Session Replay Analysis Flake Triage capture so the next failure is diagnosable instead of mysterious?
- Which part of Session Replay Analysis Flake Triage is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Session Replay Analysis Flake Triage.
...
- [ ] The state model behind Session Replay Analysis Flake Triage is explicit.
