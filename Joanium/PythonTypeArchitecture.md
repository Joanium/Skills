---
name: Python Type Architecture
trigger: python type architecture, help with python type architecture, plan python type architecture, improve python type architecture, expert python type architecture
description: Expert-level guidance for python type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Python Type Architecture is an expert programming-language skill for Python. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Python Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Python Type Architecture?
- Which abstraction in Python Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Python Type Architecture first?
- How will Python Type Architecture behave when multiple contributors evolve it independently?
- Which part of Python Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Python Type Architecture.
- A failure-mode note that explains the sharp edges inside Python Type Architecture.
...
- [ ] The main language-level risk in Python Type Architecture is named explicitly.
...
- [ ] Future contributors can extend Python Type Architecture without reverse-engineering hidden contracts.
