---
name: Visual Diff Triage Timing Stability
trigger: visual diff triage timing stability, help with visual diff triage timing stability, plan visual diff triage timing stability, improve visual diff triage timing stability, expert visual diff triage timing stability
description: Expert-level guidance for visual diff triage timing stability with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage Timing Stability is an expert browser automation skill for Visual Diff Triage. The focal concern is Timing Stability, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage Timing Stability needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage Timing Stability unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage Timing Stability is least stable under real conditions?
- How would Visual Diff Triage Timing Stability behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage Timing Stability capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage Timing Stability is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage Timing Stability.
...
- [ ] The state model behind Visual Diff Triage Timing Stability is explicit.
