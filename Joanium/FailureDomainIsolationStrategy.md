---
name: Failure Domain Isolation Strategy
trigger: failure domain isolation strategy, help with failure domain isolation strategy, plan failure domain isolation strategy, improve failure domain isolation strategy, expert failure domain isolation strategy
description: Expert-level guidance for failure domain isolation strategy, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Failure Domain Isolation Strategy is an expert-level infrastructure skill for reducing operational risk while keeping rollout control, recovery, and ownership explicit. The emphasis here is on choosing the operating model, decision boundaries, and success criteria before local optimizations become policy by accident.

## When To Use

- Use this when Failure Domain Isolation Strategy changes service availability, deployment safety, or recovery behavior.
...
- What decision is Failure Domain Isolation Strategy supposed to improve over the next planning horizon?
- Which constraints are fixed, and which are merely habits that can be challenged?
- Where does failure in Failure Domain Isolation Strategy create the largest technical, business, or trust cost?
...
- A written decision statement for Failure Domain Isolation Strategy.
