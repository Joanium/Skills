---
name: Tool Selection Design Prompt Versioning
trigger: tool selection design prompt versioning, help with tool selection design prompt versioning, plan tool selection design prompt versioning, improve tool selection design prompt versioning, expert tool selection design prompt versioning
description: Expert-level guidance for tool selection design prompt versioning with action-oriented workflow, review criteria, and failure patterns.
---

Tool Selection Design Prompt Versioning is an expert agent and tooling skill for Tool Selection Design. The focal concern is Prompt Versioning, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Tool Selection Design Prompt Versioning affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Tool Selection Design Prompt Versioning is still governed by intuition instead of a contract or label set?
- Which failure in Tool Selection Design Prompt Versioning would be hardest to detect before it compounds?
- How would Tool Selection Design Prompt Versioning behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Tool Selection Design Prompt Versioning?
...
- A contract and failure-label set for Tool Selection Design Prompt Versioning.
...
- [ ] The system behind Tool Selection Design Prompt Versioning is described more clearly than the prompt surface.
