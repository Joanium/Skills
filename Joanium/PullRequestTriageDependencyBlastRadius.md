---
name: Pull Request Triage Dependency Blast Radius
trigger: pull request triage dependency blast radius, help with pull request triage dependency blast radius, plan pull request triage dependency blast radius, improve pull request triage dependency blast radius, expert pull request triage dependency blast radius
description: Expert-level guidance for pull request triage dependency blast radius with action-oriented workflow, review criteria, and failure patterns.
---

Pull Request Triage Dependency Blast Radius is an expert repository and delivery skill for Pull Request Triage. The focal concern is Dependency Blast Radius, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Pull Request Triage Dependency Blast Radius determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Pull Request Triage Dependency Blast Radius riskier than the diff itself suggests?
- Which reviewer would challenge Pull Request Triage Dependency Blast Radius first, and what evidence would they need?
- How could Pull Request Triage Dependency Blast Radius fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Pull Request Triage Dependency Blast Radius would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Pull Request Triage Dependency Blast Radius concretely?
...
- A blast-radius note for Pull Request Triage Dependency Blast Radius.
...
- [ ] The repository-level blast radius of Pull Request Triage Dependency Blast Radius is explicit.
