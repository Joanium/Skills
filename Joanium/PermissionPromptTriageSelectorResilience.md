---
name: Permission Prompt Triage Selector Resilience
trigger: permission prompt triage selector resilience, help with permission prompt triage selector resilience, plan permission prompt triage selector resilience, improve permission prompt triage selector resilience, expert permission prompt triage selector resilience
description: Expert-level guidance for permission prompt triage selector resilience with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage Selector Resilience is an expert browser automation skill for Permission Prompt Triage. The focal concern is Selector Resilience, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage Selector Resilience needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage Selector Resilience unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage Selector Resilience is least stable under real conditions?
- How would Permission Prompt Triage Selector Resilience behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage Selector Resilience capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage Selector Resilience is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage Selector Resilience.
...
- [ ] The state model behind Permission Prompt Triage Selector Resilience is explicit.
