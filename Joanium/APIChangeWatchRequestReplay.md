---
name: API Change Watch Request Replay
trigger: api change watch request replay, help with api change watch request replay, plan api change watch request replay, improve api change watch request replay, expert api change watch request replay
description: Expert-level guidance for api change watch request replay with action-oriented workflow, review criteria, and failure patterns.
---

API Change Watch Request Replay is an expert API and integration skill for API Change Watch. The focal concern is Request Replay, but the real objective is helping an AI make external integrations reliable, reviewable, and survivable when contracts, auth, and partners drift.

## When To Use

- Use this when API Change Watch Request Replay affects whether external integrations remain trustworthy after partner behavior changes.
...
- What partner or contract assumption inside API Change Watch Request Replay is least likely to hold over time?
- How would API Change Watch Request Replay fail if the partner changed validation, latency, or auth behavior without notice?
- What evidence proves sandbox behavior is close enough to production for API Change Watch Request Replay?
- Which part of API Change Watch Request Replay needs stronger replay, traceability, or escalation support?
- How will the AI explain the safety model of API Change Watch Request Replay to an integration reviewer?
...
- A contract and drift note for API Change Watch Request Replay.
...
- [ ] The external contract and drift risk behind API Change Watch Request Replay are explicit.
