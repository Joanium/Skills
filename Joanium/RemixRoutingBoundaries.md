---
name: Remix Routing Boundaries
trigger: remix routing boundaries, help with remix routing boundaries, plan remix routing boundaries, improve remix routing boundaries, expert remix routing boundaries
description: Expert-level guidance for remix routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Remix Routing Boundaries is an expert frontend skill for Remix. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Remix Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Remix Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in Remix Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Remix Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does Remix Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Remix Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for Remix Routing Boundaries.
...
- [ ] The state model for Remix Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend Remix Routing Boundaries without guessing hidden UI contracts.
