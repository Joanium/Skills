---
name: Unit Testing Systems Boundary Scenarios
trigger: unit testing systems boundary scenarios, help with unit testing systems boundary scenarios, plan unit testing systems boundary scenarios, improve unit testing systems boundary scenarios, expert unit testing systems boundary scenarios
description: Expert-level guidance for unit testing systems boundary scenarios with action-oriented workflow, review criteria, and failure patterns.
---

Unit Testing Systems Boundary Scenarios is an expert testing and quality skill for Unit Testing Systems. The focal concern is Boundary Scenarios, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Unit Testing Systems Boundary Scenarios needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Unit Testing Systems Boundary Scenarios is still untested even if coverage looks healthy?
- Which assertion in Unit Testing Systems Boundary Scenarios would stay green while the user-visible behavior regressed?
- How much of Unit Testing Systems Boundary Scenarios depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Unit Testing Systems Boundary Scenarios need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Unit Testing Systems Boundary Scenarios.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Unit Testing Systems Boundary Scenarios should mean.
...
- [ ] The key behavior protected by Unit Testing Systems Boundary Scenarios is explicit.
