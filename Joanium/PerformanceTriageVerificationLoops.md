---
name: Performance Triage Verification Loops
trigger: performance triage verification loops, help with performance triage verification loops, plan performance triage verification loops, improve performance triage verification loops, expert performance triage verification loops
description: Expert-level guidance for performance triage verification loops with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Verification Loops is an expert coding workflow skill for Performance Triage. The focal concern is Verification Loops, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Verification Loops is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Verification Loops is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Verification Loops would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Verification Loops create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Verification Loops to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Verification Loops.
...
- [ ] The evidence supporting Performance Triage Verification Loops is stronger than the assumptions.
