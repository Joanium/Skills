---
name: Agents Latency Budgets
trigger: agents latency budgets, help with agents latency budgets, plan agents latency budgets, improve agents latency budgets, expert agents latency budgets
description: Expert-level guidance for agents latency budgets with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Agents Latency Budgets is an expert applied-AI skill for Agents. The focal concern is Latency Budgets, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Agents Latency Budgets affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Agents Latency Budgets would look acceptable in a demo but fail under production ambiguity?
- Which part of Agents Latency Budgets lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Agents Latency Budgets fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Agents Latency Budgets is getting better rather than merely changing?
- Where should humans intervene in Agents Latency Budgets to add leverage instead of toil?
...
- A behavior contract for Agents Latency Budgets.
...
- [ ] The behavior contract for Agents Latency Budgets is explicit.
