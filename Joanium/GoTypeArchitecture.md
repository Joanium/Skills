---
name: Go Type Architecture
trigger: go type architecture, help with go type architecture, plan go type architecture, improve go type architecture, expert go type architecture
description: Expert-level guidance for go type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Go Type Architecture is an expert programming-language skill for Go. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Go Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Go Type Architecture?
- Which abstraction in Go Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Go Type Architecture first?
- How will Go Type Architecture behave when multiple contributors evolve it independently?
- Which part of Go Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Go Type Architecture.
- A failure-mode note that explains the sharp edges inside Go Type Architecture.
...
- [ ] The main language-level risk in Go Type Architecture is named explicitly.
...
- [ ] Future contributors can extend Go Type Architecture without reverse-engineering hidden contracts.
