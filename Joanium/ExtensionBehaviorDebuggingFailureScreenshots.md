---
name: Extension Behavior Debugging Failure Screenshots
trigger: extension behavior debugging failure screenshots, help with extension behavior debugging failure screenshots, plan extension behavior debugging failure screenshots, improve extension behavior debugging failure screenshots, expert extension behavior debugging failure screenshots
description: Expert-level guidance for extension behavior debugging failure screenshots with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Failure Screenshots is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Failure Screenshots, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Failure Screenshots needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Failure Screenshots unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Failure Screenshots is least stable under real conditions?
- How would Extension Behavior Debugging Failure Screenshots behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Failure Screenshots capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Failure Screenshots is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Failure Screenshots.
...
- [ ] The state model behind Extension Behavior Debugging Failure Screenshots is explicit.
