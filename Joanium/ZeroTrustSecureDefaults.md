---
name: Zero Trust Secure Defaults
trigger: zero trust secure defaults, help with zero trust secure defaults, plan zero trust secure defaults, improve zero trust secure defaults, expert zero trust secure defaults
description: Expert-level guidance for zero trust secure defaults with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Zero Trust Secure Defaults is an expert security skill for Zero Trust. The focal concern is Secure Defaults, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when Zero Trust Secure Defaults influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in Zero Trust Secure Defaults first?
- Which exception or emergency path makes Zero Trust Secure Defaults less true in practice than in policy?
- What evidence would be missing if Zero Trust Secure Defaults failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does Zero Trust Secure Defaults degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for Zero Trust Secure Defaults.
...
- [ ] The attacker model for Zero Trust Secure Defaults is realistic and current.
...
- [ ] The team can explain how Zero Trust Secure Defaults fails and how it recovers.
