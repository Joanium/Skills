---
name: Distributed Systems State Machines
trigger: distributed systems state machines, help with distributed systems state machines, plan distributed systems state machines, improve distributed systems state machines, expert distributed systems state machines
description: Expert-level guidance for distributed systems state machines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Distributed Systems State Machines is an expert systems skill for Distributed Systems. The focal concern is State Machines, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Distributed Systems State Machines is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Distributed Systems State Machines would break first under scale, timing, or hardware stress?
- Which state or resource in Distributed Systems State Machines is hardest to observe when behavior goes wrong?
- How does Distributed Systems State Machines trade correctness, throughput, and recoverability today?
- What tooling gap makes Distributed Systems State Machines harder to reason about than it should be?
- Which optimization target inside Distributed Systems State Machines is attractive but strategically secondary?
...
- A system model for Distributed Systems State Machines that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Distributed Systems State Machines is explicit.
