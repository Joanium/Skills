---
name: Svelte Build Pipelines
trigger: svelte build pipelines, help with svelte build pipelines, plan svelte build pipelines, improve svelte build pipelines, expert svelte build pipelines
description: Expert-level guidance for svelte build pipelines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte Build Pipelines is an expert frontend skill for Svelte. The focal concern is Build Pipelines, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte Build Pipelines affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte Build Pipelines make user state harder to reason about than it needs to be?
- Which interaction in Svelte Build Pipelines degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte Build Pipelines would break if the DOM or state changed unexpectedly?
- How does Svelte Build Pipelines fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte Build Pipelines looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte Build Pipelines.
...
- [ ] The state model for Svelte Build Pipelines is clear enough to debug.
...
- [ ] Another engineer can extend Svelte Build Pipelines without guessing hidden UI contracts.
