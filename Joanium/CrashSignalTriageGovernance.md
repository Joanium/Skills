---
name: Crash Signal Triage Governance
trigger: crash signal triage governance, help with crash signal triage governance, plan crash signal triage governance, improve crash signal triage governance, expert crash signal triage governance
description: Expert-level guidance for crash signal triage governance, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Crash Signal Triage Governance is an expert-level client platform skill for shipping resilient mobile or desktop behavior across devices, stores, networks, and operating-system constraints. The emphasis here is on ownership, policy, exception handling, and auditability so the domain does not decay into ad hoc decisions.

## When To Use

- Use this when Crash Signal Triage Governance affects release risk, device behavior, or user trust during degraded conditions.
...
- Who sets the policy for Crash Signal Triage Governance, and who is allowed to grant exceptions?
...
- A policy document for Crash Signal Triage Governance with explicit scope.
