---
name: Synthetic Dataset Design Task Contracts
trigger: synthetic dataset design task contracts, help with synthetic dataset design task contracts, plan synthetic dataset design task contracts, improve synthetic dataset design task contracts, expert synthetic dataset design task contracts
description: Expert-level guidance for synthetic dataset design task contracts with action-oriented workflow, review criteria, and failure patterns.
---

Synthetic Dataset Design Task Contracts is an expert applied AI skill for Synthetic Dataset Design. The focal concern is Task Contracts, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Synthetic Dataset Design Task Contracts affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Synthetic Dataset Design Task Contracts is currently measured by vibes rather than a real contract?
- Which failure in Synthetic Dataset Design Task Contracts would be hardest to notice before it harms users or operators?
- How will Synthetic Dataset Design Task Contracts behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Synthetic Dataset Design Task Contracts?
...
- A behavior contract for Synthetic Dataset Design Task Contracts.
...
- [ ] The contract behind Synthetic Dataset Design Task Contracts is explicit enough to evaluate.
