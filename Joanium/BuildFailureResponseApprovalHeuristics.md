---
name: Build Failure Response Approval Heuristics
trigger: build failure response approval heuristics, help with build failure response approval heuristics, plan build failure response approval heuristics, improve build failure response approval heuristics, expert build failure response approval heuristics
description: Expert-level guidance for build failure response approval heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Build Failure Response Approval Heuristics is an expert repository and delivery skill for Build Failure Response. The focal concern is Approval Heuristics, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Build Failure Response Approval Heuristics determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Build Failure Response Approval Heuristics riskier than the diff itself suggests?
- Which reviewer would challenge Build Failure Response Approval Heuristics first, and what evidence would they need?
- How could Build Failure Response Approval Heuristics fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Build Failure Response Approval Heuristics would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Build Failure Response Approval Heuristics concretely?
...
- A blast-radius note for Build Failure Response Approval Heuristics.
...
- [ ] The repository-level blast radius of Build Failure Response Approval Heuristics is explicit.
