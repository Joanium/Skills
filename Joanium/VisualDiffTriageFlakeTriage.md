---
name: Visual Diff Triage Flake Triage
trigger: visual diff triage flake triage, help with visual diff triage flake triage, plan visual diff triage flake triage, improve visual diff triage flake triage, expert visual diff triage flake triage
description: Expert-level guidance for visual diff triage flake triage with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage Flake Triage is an expert browser automation skill for Visual Diff Triage. The focal concern is Flake Triage, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage Flake Triage needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage Flake Triage unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage Flake Triage is least stable under real conditions?
- How would Visual Diff Triage Flake Triage behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage Flake Triage capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage Flake Triage is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage Flake Triage.
...
- [ ] The state model behind Visual Diff Triage Flake Triage is explicit.
