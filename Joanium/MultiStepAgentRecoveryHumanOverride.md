---
name: Multi-Step Agent Recovery Human Override
trigger: multi-step agent recovery human override, help with multi-step agent recovery human override, plan multi-step agent recovery human override, improve multi-step agent recovery human override, expert multi-step agent recovery human override
description: Expert-level guidance for multi-step agent recovery human override with action-oriented workflow, review criteria, and failure patterns.
---

Multi-Step Agent Recovery Human Override is an expert agent and tooling skill for Multi-Step Agent Recovery. The focal concern is Human Override, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Multi-Step Agent Recovery Human Override affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Multi-Step Agent Recovery Human Override is still governed by intuition instead of a contract or label set?
- Which failure in Multi-Step Agent Recovery Human Override would be hardest to detect before it compounds?
- How would Multi-Step Agent Recovery Human Override behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Multi-Step Agent Recovery Human Override?
...
- A contract and failure-label set for Multi-Step Agent Recovery Human Override.
...
- [ ] The system behind Multi-Step Agent Recovery Human Override is described more clearly than the prompt surface.
