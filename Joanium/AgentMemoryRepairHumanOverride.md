---
name: Agent Memory Repair Human Override
trigger: agent memory repair human override, help with agent memory repair human override, plan agent memory repair human override, improve agent memory repair human override, expert agent memory repair human override
description: Expert-level guidance for agent memory repair human override with action-oriented workflow, review criteria, and failure patterns.
---

Agent Memory Repair Human Override is an expert agent and tooling skill for Agent Memory Repair. The focal concern is Human Override, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Agent Memory Repair Human Override affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Agent Memory Repair Human Override is still governed by intuition instead of a contract or label set?
- Which failure in Agent Memory Repair Human Override would be hardest to detect before it compounds?
- How would Agent Memory Repair Human Override behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Agent Memory Repair Human Override?
...
- A contract and failure-label set for Agent Memory Repair Human Override.
...
- [ ] The system behind Agent Memory Repair Human Override is described more clearly than the prompt surface.
