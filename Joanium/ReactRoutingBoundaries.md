---
name: React Routing Boundaries
trigger: react routing boundaries, help with react routing boundaries, plan react routing boundaries, improve react routing boundaries, expert react routing boundaries
description: Expert-level guidance for react routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

React Routing Boundaries is an expert frontend skill for React. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when React Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does React Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in React Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in React Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does React Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of React Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for React Routing Boundaries.
...
- [ ] The state model for React Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend React Routing Boundaries without guessing hidden UI contracts.
