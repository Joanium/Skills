---
name: API Incident Triage Causal Tracing
trigger: api incident triage causal tracing, help with api incident triage causal tracing, plan api incident triage causal tracing, improve api incident triage causal tracing, expert api incident triage causal tracing
description: Expert-level guidance for api incident triage causal tracing with action-oriented workflow, review criteria, and failure patterns.
---

API Incident Triage Causal Tracing is an expert backend execution skill for API Incident Triage. The focal concern is Causal Tracing, but the real objective is helping an AI design service behavior that stays coherent under retries, queues, partial failure, and operational pressure.

## When To Use

- Use this when API Incident Triage Causal Tracing affects whether backend changes remain safe once real production behavior kicks in.
...
- Which failure ordering inside API Incident Triage Causal Tracing would create the most misleading symptom?
- Where does API Incident Triage Causal Tracing need stronger idempotency or retry discipline?
- What dependency in API Incident Triage Causal Tracing is currently hiding behind timeout or queue noise?
- How would an operator explain the health of API Incident Triage Causal Tracing in the first five minutes of an incident?
- What contract assumption in API Incident Triage Causal Tracing is least defensible under partial failure?
...
- A failure-sequence note for API Incident Triage Causal Tracing.
...
- [ ] The real behavior of API Incident Triage Causal Tracing under retries and failures is explicit.
