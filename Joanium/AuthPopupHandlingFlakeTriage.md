---
name: Auth Popup Handling Flake Triage
trigger: auth popup handling flake triage, help with auth popup handling flake triage, plan auth popup handling flake triage, improve auth popup handling flake triage, expert auth popup handling flake triage
description: Expert-level guidance for auth popup handling flake triage with action-oriented workflow, review criteria, and failure patterns.
---

Auth Popup Handling Flake Triage is an expert browser automation skill for Auth Popup Handling. The focal concern is Flake Triage, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Auth Popup Handling Flake Triage needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Auth Popup Handling Flake Triage unreliable across runs?
- Which timing or selector assumption in Auth Popup Handling Flake Triage is least stable under real conditions?
- How would Auth Popup Handling Flake Triage behave differently across browsers, profiles, or CI environments?
- What artifacts should Auth Popup Handling Flake Triage capture so the next failure is diagnosable instead of mysterious?
- Which part of Auth Popup Handling Flake Triage is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Auth Popup Handling Flake Triage.
...
- [ ] The state model behind Auth Popup Handling Flake Triage is explicit.
