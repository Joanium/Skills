---
name: Prompt Workflows Trace Analysis
trigger: prompt workflows trace analysis, help with prompt workflows trace analysis, plan prompt workflows trace analysis, improve prompt workflows trace analysis, expert prompt workflows trace analysis
description: Expert-level guidance for prompt workflows trace analysis with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Workflows Trace Analysis is an expert applied AI skill for Prompt Workflows. The focal concern is Trace Analysis, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Prompt Workflows Trace Analysis affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Prompt Workflows Trace Analysis is currently measured by vibes rather than a real contract?
- Which failure in Prompt Workflows Trace Analysis would be hardest to notice before it harms users or operators?
- How will Prompt Workflows Trace Analysis behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Prompt Workflows Trace Analysis?
...
- A behavior contract for Prompt Workflows Trace Analysis.
...
- [ ] The contract behind Prompt Workflows Trace Analysis is explicit enough to evaluate.
