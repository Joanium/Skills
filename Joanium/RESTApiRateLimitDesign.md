---
name: REST API Rate Limit Design
trigger: rest api rate limit design, help with rest api rate limit design, plan rest api rate limit design, improve rest api rate limit design, expert rest api rate limit design
description: Expert-level guidance for rest api rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

REST API Rate Limit Design is an expert backend skill for REST API. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when REST API Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside REST API Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in REST API Rate Limit Design?
- Where does REST API Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether REST API Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by REST API Rate Limit Design even if it was never written down?
...
- A contract note for REST API Rate Limit Design that includes failure semantics.
...
- [ ] The contract for REST API Rate Limit Design is explicit about success and failure behavior.
