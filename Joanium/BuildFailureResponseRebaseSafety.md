---
name: Build Failure Response Rebase Safety
trigger: build failure response rebase safety, help with build failure response rebase safety, plan build failure response rebase safety, improve build failure response rebase safety, expert build failure response rebase safety
description: Expert-level guidance for build failure response rebase safety with action-oriented workflow, review criteria, and failure patterns.
---

Build Failure Response Rebase Safety is an expert repository and delivery skill for Build Failure Response. The focal concern is Rebase Safety, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Build Failure Response Rebase Safety determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Build Failure Response Rebase Safety riskier than the diff itself suggests?
- Which reviewer would challenge Build Failure Response Rebase Safety first, and what evidence would they need?
- How could Build Failure Response Rebase Safety fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Build Failure Response Rebase Safety would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Build Failure Response Rebase Safety concretely?
...
- A blast-radius note for Build Failure Response Rebase Safety.
...
- [ ] The repository-level blast radius of Build Failure Response Rebase Safety is explicit.
