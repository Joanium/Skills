---
name: Vue Rendering Performance
trigger: vue rendering performance, help with vue rendering performance, plan vue rendering performance, improve vue rendering performance, expert vue rendering performance
description: Expert-level guidance for vue rendering performance with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Vue Rendering Performance is an expert frontend skill for Vue. The focal concern is Rendering Performance, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Vue Rendering Performance affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Vue Rendering Performance make user state harder to reason about than it needs to be?
- Which interaction in Vue Rendering Performance degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Vue Rendering Performance would break if the DOM or state changed unexpectedly?
- How does Vue Rendering Performance fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Vue Rendering Performance looks correct in a mock but collapses under production behavior?
...
- A state transition map for Vue Rendering Performance.
...
- [ ] The state model for Vue Rendering Performance is clear enough to debug.
...
- [ ] Another engineer can extend Vue Rendering Performance without guessing hidden UI contracts.
