---
name: Evaluation Engineering Policy Enforcement
trigger: evaluation engineering policy enforcement, help with evaluation engineering policy enforcement, plan evaluation engineering policy enforcement, improve evaluation engineering policy enforcement, expert evaluation engineering policy enforcement
description: Expert-level guidance for evaluation engineering policy enforcement with action-oriented workflow, review criteria, and failure patterns.
---

Evaluation Engineering Policy Enforcement is an expert applied AI skill for Evaluation Engineering. The focal concern is Policy Enforcement, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Evaluation Engineering Policy Enforcement affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Evaluation Engineering Policy Enforcement is currently measured by vibes rather than a real contract?
- Which failure in Evaluation Engineering Policy Enforcement would be hardest to notice before it harms users or operators?
- How will Evaluation Engineering Policy Enforcement behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Evaluation Engineering Policy Enforcement?
...
- A behavior contract for Evaluation Engineering Policy Enforcement.
...
- [ ] The contract behind Evaluation Engineering Policy Enforcement is explicit enough to evaluate.
