---
name: Code Search Operations Approval Heuristics
trigger: code search operations approval heuristics, help with code search operations approval heuristics, plan code search operations approval heuristics, improve code search operations approval heuristics, expert code search operations approval heuristics
description: Expert-level guidance for code search operations approval heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Code Search Operations Approval Heuristics is an expert repository and delivery skill for Code Search Operations. The focal concern is Approval Heuristics, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Code Search Operations Approval Heuristics determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Code Search Operations Approval Heuristics riskier than the diff itself suggests?
- Which reviewer would challenge Code Search Operations Approval Heuristics first, and what evidence would they need?
- How could Code Search Operations Approval Heuristics fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Code Search Operations Approval Heuristics would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Code Search Operations Approval Heuristics concretely?
...
- A blast-radius note for Code Search Operations Approval Heuristics.
...
- [ ] The repository-level blast radius of Code Search Operations Approval Heuristics is explicit.
