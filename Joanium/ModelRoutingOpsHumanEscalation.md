---
name: Model Routing Operations Human Escalation
trigger: model routing operations human escalation, help with model routing operations human escalation, plan model routing operations human escalation, improve model routing operations human escalation, expert model routing operations human escalation
description: Expert-level guidance for model routing operations human escalation with action-oriented workflow, review criteria, and failure patterns.
---

Model Routing Operations Human Escalation is an expert applied AI skill for Model Routing Operations. The focal concern is Human Escalation, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Model Routing Operations Human Escalation affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Model Routing Operations Human Escalation is currently measured by vibes rather than a real contract?
- Which failure in Model Routing Operations Human Escalation would be hardest to notice before it harms users or operators?
- How will Model Routing Operations Human Escalation behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Model Routing Operations Human Escalation?
...
- A behavior contract for Model Routing Operations Human Escalation.
...
- [ ] The contract behind Model Routing Operations Human Escalation is explicit enough to evaluate.
