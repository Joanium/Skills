---
name: Go Package Publishing
trigger: go package publishing, help with go package publishing, plan go package publishing, improve go package publishing, expert go package publishing
description: Expert-level guidance for go package publishing with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Go Package Publishing is an expert programming-language skill for Go. The focal concern is Package Publishing, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Go Package Publishing affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Go Package Publishing?
- Which abstraction in Go Package Publishing becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Go Package Publishing first?
- How will Go Package Publishing behave when multiple contributors evolve it independently?
- Which part of Go Package Publishing is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Go Package Publishing.
- A failure-mode note that explains the sharp edges inside Go Package Publishing.
...
- [ ] The main language-level risk in Go Package Publishing is named explicitly.
...
- [ ] Future contributors can extend Go Package Publishing without reverse-engineering hidden contracts.
