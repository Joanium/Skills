---
name: Permission Prompt Triage Failure Screenshots
trigger: permission prompt triage failure screenshots, help with permission prompt triage failure screenshots, plan permission prompt triage failure screenshots, improve permission prompt triage failure screenshots, expert permission prompt triage failure screenshots
description: Expert-level guidance for permission prompt triage failure screenshots with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage Failure Screenshots is an expert browser automation skill for Permission Prompt Triage. The focal concern is Failure Screenshots, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage Failure Screenshots needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage Failure Screenshots unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage Failure Screenshots is least stable under real conditions?
- How would Permission Prompt Triage Failure Screenshots behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage Failure Screenshots capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage Failure Screenshots is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage Failure Screenshots.
...
- [ ] The state model behind Permission Prompt Triage Failure Screenshots is explicit.
