---
name: Query Repair Trace Isolation
trigger: query repair trace isolation, help with query repair trace isolation, plan query repair trace isolation, improve query repair trace isolation, expert query repair trace isolation
description: Expert-level guidance for query repair trace isolation with action-oriented workflow, review criteria, and failure patterns.
---

Query Repair Trace Isolation is an expert coding workflow skill for Query Repair. The focal concern is Trace Isolation, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Query Repair Trace Isolation is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Query Repair Trace Isolation is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Query Repair Trace Isolation would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Query Repair Trace Isolation create a silent regression that tests may miss?
- How would the AI explain the safety of Query Repair Trace Isolation to a skeptical reviewer?
...
- A concise evidence note for Query Repair Trace Isolation.
...
- [ ] The evidence supporting Query Repair Trace Isolation is stronger than the assumptions.
