---
name: REST API Error Modeling
trigger: rest api error modeling, help with rest api error modeling, plan rest api error modeling, improve rest api error modeling, expert rest api error modeling
description: Expert-level guidance for rest api error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

REST API Error Modeling is an expert backend skill for REST API. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when REST API Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside REST API Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in REST API Error Modeling?
- Where does REST API Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether REST API Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by REST API Error Modeling even if it was never written down?
...
- A contract note for REST API Error Modeling that includes failure semantics.
...
- [ ] The contract for REST API Error Modeling is explicit about success and failure behavior.
