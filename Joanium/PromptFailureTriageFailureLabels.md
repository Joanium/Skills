---
name: Prompt Failure Triage Failure Labels
trigger: prompt failure triage failure labels, help with prompt failure triage failure labels, plan prompt failure triage failure labels, improve prompt failure triage failure labels, expert prompt failure triage failure labels
description: Expert-level guidance for prompt failure triage failure labels with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Failure Labels is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Failure Labels, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Failure Labels affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Failure Labels is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Failure Labels would be hardest to detect before it compounds?
- How would Prompt Failure Triage Failure Labels behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Failure Labels?
...
- A contract and failure-label set for Prompt Failure Triage Failure Labels.
...
- [ ] The system behind Prompt Failure Triage Failure Labels is described more clearly than the prompt surface.
