---
name: Vue Routing Boundaries
trigger: vue routing boundaries, help with vue routing boundaries, plan vue routing boundaries, improve vue routing boundaries, expert vue routing boundaries
description: Expert-level guidance for vue routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Vue Routing Boundaries is an expert frontend skill for Vue. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Vue Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Vue Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in Vue Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Vue Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does Vue Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Vue Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for Vue Routing Boundaries.
...
- [ ] The state model for Vue Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend Vue Routing Boundaries without guessing hidden UI contracts.
