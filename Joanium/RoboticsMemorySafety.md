---
name: Robotics Memory Safety
trigger: robotics memory safety, help with robotics memory safety, plan robotics memory safety, improve robotics memory safety, expert robotics memory safety
description: Expert-level guidance for robotics memory safety with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Robotics Memory Safety is an expert systems skill for Robotics. The focal concern is Memory Safety, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Robotics Memory Safety is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Robotics Memory Safety would break first under scale, timing, or hardware stress?
- Which state or resource in Robotics Memory Safety is hardest to observe when behavior goes wrong?
- How does Robotics Memory Safety trade correctness, throughput, and recoverability today?
- What tooling gap makes Robotics Memory Safety harder to reason about than it should be?
- Which optimization target inside Robotics Memory Safety is attractive but strategically secondary?
...
- A system model for Robotics Memory Safety that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Robotics Memory Safety is explicit.
