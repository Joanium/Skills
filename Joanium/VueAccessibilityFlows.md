---
name: Vue Accessibility Flows
trigger: vue accessibility flows, help with vue accessibility flows, plan vue accessibility flows, improve vue accessibility flows, expert vue accessibility flows
description: Expert-level guidance for vue accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Vue Accessibility Flows is an expert frontend skill for Vue. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Vue Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Vue Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in Vue Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Vue Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does Vue Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Vue Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for Vue Accessibility Flows.
...
- [ ] The state model for Vue Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend Vue Accessibility Flows without guessing hidden UI contracts.
