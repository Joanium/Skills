---
name: Failure Domain Isolation Planning
trigger: failure domain isolation planning, help with failure domain isolation planning, plan failure domain isolation planning, improve failure domain isolation planning, expert failure domain isolation planning
description: Expert-level guidance for failure domain isolation planning, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Failure Domain Isolation Planning is an expert-level infrastructure skill for reducing operational risk while keeping rollout control, recovery, and ownership explicit. The emphasis here is sequencing work, dependencies, and proof points so execution moves quickly without creating blind risk.

## When To Use

- Use this when Failure Domain Isolation Planning changes service availability, deployment safety, or recovery behavior.
...
- What must be true before Failure Domain Isolation Planning can start safely?
...
- A milestone plan for Failure Domain Isolation Planning with explicit gating criteria.
