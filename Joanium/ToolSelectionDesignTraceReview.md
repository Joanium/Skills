---
name: Tool Selection Design Trace Review
trigger: tool selection design trace review, help with tool selection design trace review, plan tool selection design trace review, improve tool selection design trace review, expert tool selection design trace review
description: Expert-level guidance for tool selection design trace review with action-oriented workflow, review criteria, and failure patterns.
---

Tool Selection Design Trace Review is an expert agent and tooling skill for Tool Selection Design. The focal concern is Trace Review, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Tool Selection Design Trace Review affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Tool Selection Design Trace Review is still governed by intuition instead of a contract or label set?
- Which failure in Tool Selection Design Trace Review would be hardest to detect before it compounds?
- How would Tool Selection Design Trace Review behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Tool Selection Design Trace Review?
...
- A contract and failure-label set for Tool Selection Design Trace Review.
...
- [ ] The system behind Tool Selection Design Trace Review is described more clearly than the prompt surface.
