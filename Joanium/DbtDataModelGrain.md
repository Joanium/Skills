---
name: dbt Data Model Grain
trigger: dbt data model grain, help with dbt data model grain, plan dbt data model grain, improve dbt data model grain, expert dbt data model grain
description: Expert-level guidance for dbt data model grain with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

dbt Data Model Grain is an expert data-systems skill for dbt. The focal concern is Data Model Grain, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when dbt Data Model Grain affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would dbt Data Model Grain create if two teams interpreted it differently?
- Which recovery path for dbt Data Model Grain is the least understood today?
- How will dbt Data Model Grain behave under backfills, reprocessing, or delayed sources?
- What cost pattern in dbt Data Model Grain is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside dbt Data Model Grain drifted subtly?
...
- A semantic contract note for dbt Data Model Grain.
...
- [ ] The meaning of dbt Data Model Grain is written in language consumers will actually use.
