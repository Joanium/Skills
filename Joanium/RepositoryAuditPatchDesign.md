---
name: Repository Audit Patch Design
trigger: repository audit patch design, help with repository audit patch design, plan repository audit patch design, improve repository audit patch design, expert repository audit patch design
description: Expert-level guidance for repository audit patch design with action-oriented workflow, review criteria, and failure patterns.
---

Repository Audit Patch Design is an expert coding workflow skill for Repository Audit. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Repository Audit Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Repository Audit Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Repository Audit Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Repository Audit Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Repository Audit Patch Design to a skeptical reviewer?
...
- A concise evidence note for Repository Audit Patch Design.
...
- [ ] The evidence supporting Repository Audit Patch Design is stronger than the assumptions.
