---
name: API Boundary Design Migration Seams
trigger: api boundary design migration seams, help with api boundary design migration seams, plan api boundary design migration seams, improve api boundary design migration seams, expert api boundary design migration seams
description: Expert-level guidance for api boundary design migration seams with action-oriented workflow, review criteria, and failure patterns.
---

API Boundary Design Migration Seams is an expert architecture and systems skill for API Boundary Design. The focal concern is Migration Seams, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when API Boundary Design Migration Seams needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of API Boundary Design Migration Seams becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in API Boundary Design Migration Seams is currently cheap only because the bill has not arrived yet?
- How would API Boundary Design Migration Seams be migrated if the current assumption proved wrong in six months?
- Where is ownership inside API Boundary Design Migration Seams too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for API Boundary Design Migration Seams.
...
- [ ] The key boundaries inside API Boundary Design Migration Seams are explicit and justified.
