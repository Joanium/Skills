---
name: CI/CD Pipelines Drift Detection
trigger: ci/cd pipelines drift detection, help with ci/cd pipelines drift detection, plan ci/cd pipelines drift detection, improve ci/cd pipelines drift detection, expert ci/cd pipelines drift detection
description: Expert-level guidance for ci/cd pipelines drift detection with action-oriented workflow, review criteria, and failure patterns.
---

CI/CD Pipelines Drift Detection is an expert platform and automation skill for CI/CD Pipelines. The focal concern is Drift Detection, but the real objective is helping an AI make infrastructure and operational change without creating invisible blast radius.

## When To Use

- Use this when CI/CD Pipelines Drift Detection can affect deployment safety, service recovery, or operator trust in automation.
...
- What part of CI/CD Pipelines Drift Detection could fail silently while the automation still reports success?
- Which rollback or recovery step inside CI/CD Pipelines Drift Detection depends too much on tribal memory?
- How will operators know when CI/CD Pipelines Drift Detection should stop, wait, or reverse course?
- Where does CI/CD Pipelines Drift Detection blur access, policy, and execution boundaries dangerously?
...
- A change-and-recovery map for CI/CD Pipelines Drift Detection.
...
- [ ] The rollback model for CI/CD Pipelines Drift Detection is as real as the forward path.
