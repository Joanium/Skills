---
name: Algorithms Failure Analysis
trigger: algorithms failure analysis, help with algorithms failure analysis, plan algorithms failure analysis, improve algorithms failure analysis, expert algorithms failure analysis
description: Expert-level guidance for algorithms failure analysis with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Algorithms Failure Analysis is an expert systems skill for Algorithms. The focal concern is Failure Analysis, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Algorithms Failure Analysis is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Algorithms Failure Analysis would break first under scale, timing, or hardware stress?
- Which state or resource in Algorithms Failure Analysis is hardest to observe when behavior goes wrong?
- How does Algorithms Failure Analysis trade correctness, throughput, and recoverability today?
- What tooling gap makes Algorithms Failure Analysis harder to reason about than it should be?
- Which optimization target inside Algorithms Failure Analysis is attractive but strategically secondary?
...
- A system model for Algorithms Failure Analysis that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Algorithms Failure Analysis is explicit.
