---
name: gRPC Rate Limit Design
trigger: grpc rate limit design, help with grpc rate limit design, plan grpc rate limit design, improve grpc rate limit design, expert grpc rate limit design
description: Expert-level guidance for grpc rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

gRPC Rate Limit Design is an expert backend skill for gRPC. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when gRPC Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside gRPC Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in gRPC Rate Limit Design?
- Where does gRPC Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether gRPC Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by gRPC Rate Limit Design even if it was never written down?
...
- A contract note for gRPC Rate Limit Design that includes failure semantics.
...
- [ ] The contract for gRPC Rate Limit Design is explicit about success and failure behavior.
