---
name: Swift Error Semantics
trigger: swift error semantics, help with swift error semantics, plan swift error semantics, improve swift error semantics, expert swift error semantics
description: Expert-level guidance for swift error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Swift Error Semantics is an expert programming-language skill for Swift. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Swift Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Swift Error Semantics?
- Which abstraction in Swift Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Swift Error Semantics first?
- How will Swift Error Semantics behave when multiple contributors evolve it independently?
- Which part of Swift Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Swift Error Semantics.
- A failure-mode note that explains the sharp edges inside Swift Error Semantics.
...
- [ ] The main language-level risk in Swift Error Semantics is named explicitly.
...
- [ ] Future contributors can extend Swift Error Semantics without reverse-engineering hidden contracts.
