---
name: Robotics Failure Analysis
trigger: robotics failure analysis, help with robotics failure analysis, plan robotics failure analysis, improve robotics failure analysis, expert robotics failure analysis
description: Expert-level guidance for robotics failure analysis with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Robotics Failure Analysis is an expert systems skill for Robotics. The focal concern is Failure Analysis, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Robotics Failure Analysis is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Robotics Failure Analysis would break first under scale, timing, or hardware stress?
- Which state or resource in Robotics Failure Analysis is hardest to observe when behavior goes wrong?
- How does Robotics Failure Analysis trade correctness, throughput, and recoverability today?
- What tooling gap makes Robotics Failure Analysis harder to reason about than it should be?
- Which optimization target inside Robotics Failure Analysis is attractive but strategically secondary?
...
- A system model for Robotics Failure Analysis that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Robotics Failure Analysis is explicit.
