---
name: Abuse Defense Attack Simulation
trigger: abuse defense attack simulation, help with abuse defense attack simulation, plan abuse defense attack simulation, improve abuse defense attack simulation, expert abuse defense attack simulation
description: Expert-level guidance for abuse defense attack simulation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Abuse Defense Attack Simulation is an expert security skill for Abuse Defense. The focal concern is Attack Simulation, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when Abuse Defense Attack Simulation influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in Abuse Defense Attack Simulation first?
- Which exception or emergency path makes Abuse Defense Attack Simulation less true in practice than in policy?
- What evidence would be missing if Abuse Defense Attack Simulation failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does Abuse Defense Attack Simulation degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for Abuse Defense Attack Simulation.
...
- [ ] The attacker model for Abuse Defense Attack Simulation is realistic and current.
...
- [ ] The team can explain how Abuse Defense Attack Simulation fails and how it recovers.
