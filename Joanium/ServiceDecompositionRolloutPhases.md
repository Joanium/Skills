---
name: Service Decomposition Rollout Phases
trigger: service decomposition rollout phases, help with service decomposition rollout phases, plan service decomposition rollout phases, improve service decomposition rollout phases, expert service decomposition rollout phases
description: Expert-level guidance for service decomposition rollout phases with action-oriented workflow, review criteria, and failure patterns.
---

Service Decomposition Rollout Phases is an expert architecture and systems skill for Service Decomposition. The focal concern is Rollout Phases, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Service Decomposition Rollout Phases needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Service Decomposition Rollout Phases becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Service Decomposition Rollout Phases is currently cheap only because the bill has not arrived yet?
- How would Service Decomposition Rollout Phases be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Service Decomposition Rollout Phases too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Service Decomposition Rollout Phases.
...
- [ ] The key boundaries inside Service Decomposition Rollout Phases are explicit and justified.
