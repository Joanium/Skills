---
name: Bug Reproduction Rollback Plans
trigger: bug reproduction rollback plans, help with bug reproduction rollback plans, plan bug reproduction rollback plans, improve bug reproduction rollback plans, expert bug reproduction rollback plans
description: Expert-level guidance for bug reproduction rollback plans with action-oriented workflow, review criteria, and failure patterns.
---

Bug Reproduction Rollback Plans is an expert coding workflow skill for Bug Reproduction. The focal concern is Rollback Plans, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Bug Reproduction Rollback Plans is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Bug Reproduction Rollback Plans is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Bug Reproduction Rollback Plans would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Bug Reproduction Rollback Plans create a silent regression that tests may miss?
- How would the AI explain the safety of Bug Reproduction Rollback Plans to a skeptical reviewer?
...
- A concise evidence note for Bug Reproduction Rollback Plans.
...
- [ ] The evidence supporting Bug Reproduction Rollback Plans is stronger than the assumptions.
