---
name: Config Promotion Flows Design
trigger: config promotion flows design, help with config promotion flows design, plan config promotion flows design, improve config promotion flows design, expert config promotion flows design
description: Expert-level guidance for config promotion flows design, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Config Promotion Flows Design is an expert-level infrastructure skill for reducing operational risk while keeping rollout control, recovery, and ownership explicit. The emphasis here is on interfaces, invariants, failure handling, and why one structure is safer than its alternatives.

## When To Use

- Use this when Config Promotion Flows Design changes service availability, deployment safety, or recovery behavior.
...
- What are the core boundaries, contracts, and invariants in Config Promotion Flows Design?
...
- An architecture or boundary diagram for Config Promotion Flows Design.
