---
name: Browser Test Orchestration Cross-Browser Drift
trigger: browser test orchestration cross-browser drift, help with browser test orchestration cross-browser drift, plan browser test orchestration cross-browser drift, improve browser test orchestration cross-browser drift, expert browser test orchestration cross-browser drift
description: Expert-level guidance for browser test orchestration cross-browser drift with action-oriented workflow, review criteria, and failure patterns.
---

Browser Test Orchestration Cross-Browser Drift is an expert browser automation skill for Browser Test Orchestration. The focal concern is Cross-Browser Drift, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Browser Test Orchestration Cross-Browser Drift needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Browser Test Orchestration Cross-Browser Drift unreliable across runs?
- Which timing or selector assumption in Browser Test Orchestration Cross-Browser Drift is least stable under real conditions?
- How would Browser Test Orchestration Cross-Browser Drift behave differently across browsers, profiles, or CI environments?
- What artifacts should Browser Test Orchestration Cross-Browser Drift capture so the next failure is diagnosable instead of mysterious?
- Which part of Browser Test Orchestration Cross-Browser Drift is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Browser Test Orchestration Cross-Browser Drift.
...
- [ ] The state model behind Browser Test Orchestration Cross-Browser Drift is explicit.
