---
name: Dependency Upgrade Minimal Repros
trigger: dependency upgrade minimal repros, help with dependency upgrade minimal repros, plan dependency upgrade minimal repros, improve dependency upgrade minimal repros, expert dependency upgrade minimal repros
description: Expert-level guidance for dependency upgrade minimal repros with action-oriented workflow, review criteria, and failure patterns.
---

Dependency Upgrade Minimal Repros is an expert coding workflow skill for Dependency Upgrade. The focal concern is Minimal Repros, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Dependency Upgrade Minimal Repros is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Dependency Upgrade Minimal Repros is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Dependency Upgrade Minimal Repros would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Dependency Upgrade Minimal Repros create a silent regression that tests may miss?
- How would the AI explain the safety of Dependency Upgrade Minimal Repros to a skeptical reviewer?
...
- A concise evidence note for Dependency Upgrade Minimal Repros.
...
- [ ] The evidence supporting Dependency Upgrade Minimal Repros is stronger than the assumptions.
