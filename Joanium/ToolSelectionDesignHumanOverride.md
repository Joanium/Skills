---
name: Tool Selection Design Human Override
trigger: tool selection design human override, help with tool selection design human override, plan tool selection design human override, improve tool selection design human override, expert tool selection design human override
description: Expert-level guidance for tool selection design human override with action-oriented workflow, review criteria, and failure patterns.
---

Tool Selection Design Human Override is an expert agent and tooling skill for Tool Selection Design. The focal concern is Human Override, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Tool Selection Design Human Override affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Tool Selection Design Human Override is still governed by intuition instead of a contract or label set?
- Which failure in Tool Selection Design Human Override would be hardest to detect before it compounds?
- How would Tool Selection Design Human Override behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Tool Selection Design Human Override?
...
- A contract and failure-label set for Tool Selection Design Human Override.
...
- [ ] The system behind Tool Selection Design Human Override is described more clearly than the prompt surface.
