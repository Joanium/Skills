---
name: Express Error Modeling
trigger: express error modeling, help with express error modeling, plan express error modeling, improve express error modeling, expert express error modeling
description: Expert-level guidance for express error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Express Error Modeling is an expert backend skill for Express. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Express Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Express Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Express Error Modeling?
- Where does Express Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Express Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by Express Error Modeling even if it was never written down?
...
- A contract note for Express Error Modeling that includes failure semantics.
...
- [ ] The contract for Express Error Modeling is explicit about success and failure behavior.
