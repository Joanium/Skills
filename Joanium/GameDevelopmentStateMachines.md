---
name: Game Development State Machines
trigger: game development state machines, help with game development state machines, plan game development state machines, improve game development state machines, expert game development state machines
description: Expert-level guidance for game development state machines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Game Development State Machines is an expert systems skill for Game Development. The focal concern is State Machines, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Game Development State Machines is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Game Development State Machines would break first under scale, timing, or hardware stress?
- Which state or resource in Game Development State Machines is hardest to observe when behavior goes wrong?
- How does Game Development State Machines trade correctness, throughput, and recoverability today?
- What tooling gap makes Game Development State Machines harder to reason about than it should be?
- Which optimization target inside Game Development State Machines is attractive but strategically secondary?
...
- A system model for Game Development State Machines that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Game Development State Machines is explicit.
