---
name: OAuth Detection Engineering
trigger: oauth detection engineering, help with oauth detection engineering, plan oauth detection engineering, improve oauth detection engineering, expert oauth detection engineering
description: Expert-level guidance for oauth detection engineering with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

OAuth Detection Engineering is an expert security skill for OAuth. The focal concern is Detection Engineering, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when OAuth Detection Engineering influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in OAuth Detection Engineering first?
- Which exception or emergency path makes OAuth Detection Engineering less true in practice than in policy?
- What evidence would be missing if OAuth Detection Engineering failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does OAuth Detection Engineering degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for OAuth Detection Engineering.
...
- [ ] The attacker model for OAuth Detection Engineering is realistic and current.
...
- [ ] The team can explain how OAuth Detection Engineering fails and how it recovers.
