---
name: Legacy Refactor Regression Guards
trigger: legacy refactor regression guards, help with legacy refactor regression guards, plan legacy refactor regression guards, improve legacy refactor regression guards, expert legacy refactor regression guards
description: Expert-level guidance for legacy refactor regression guards with action-oriented workflow, review criteria, and failure patterns.
---

Legacy Refactor Regression Guards is an expert coding workflow skill for Legacy Refactor. The focal concern is Regression Guards, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Legacy Refactor Regression Guards is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Legacy Refactor Regression Guards is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Legacy Refactor Regression Guards would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Legacy Refactor Regression Guards create a silent regression that tests may miss?
- How would the AI explain the safety of Legacy Refactor Regression Guards to a skeptical reviewer?
...
- A concise evidence note for Legacy Refactor Regression Guards.
...
- [ ] The evidence supporting Legacy Refactor Regression Guards is stronger than the assumptions.
