---
name: Svelte State Architecture
trigger: svelte state architecture, help with svelte state architecture, plan svelte state architecture, improve svelte state architecture, expert svelte state architecture
description: Expert-level guidance for svelte state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte State Architecture is an expert frontend skill for Svelte. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte State Architecture make user state harder to reason about than it needs to be?
- Which interaction in Svelte State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte State Architecture would break if the DOM or state changed unexpectedly?
- How does Svelte State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte State Architecture.
...
- [ ] The state model for Svelte State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend Svelte State Architecture without guessing hidden UI contracts.
