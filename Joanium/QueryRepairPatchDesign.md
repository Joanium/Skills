---
name: Query Repair Patch Design
trigger: query repair patch design, help with query repair patch design, plan query repair patch design, improve query repair patch design, expert query repair patch design
description: Expert-level guidance for query repair patch design with action-oriented workflow, review criteria, and failure patterns.
---

Query Repair Patch Design is an expert coding workflow skill for Query Repair. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Query Repair Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Query Repair Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Query Repair Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Query Repair Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Query Repair Patch Design to a skeptical reviewer?
...
- A concise evidence note for Query Repair Patch Design.
...
- [ ] The evidence supporting Query Repair Patch Design is stronger than the assumptions.
