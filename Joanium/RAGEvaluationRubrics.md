---
name: RAG Evaluation Rubrics
trigger: rag evaluation rubrics, help with rag evaluation rubrics, plan rag evaluation rubrics, improve rag evaluation rubrics, expert rag evaluation rubrics
description: Expert-level guidance for rag evaluation rubrics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

RAG Evaluation Rubrics is an expert applied-AI skill for RAG. The focal concern is Evaluation Rubrics, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when RAG Evaluation Rubrics affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in RAG Evaluation Rubrics would look acceptable in a demo but fail under production ambiguity?
- Which part of RAG Evaluation Rubrics lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will RAG Evaluation Rubrics fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that RAG Evaluation Rubrics is getting better rather than merely changing?
- Where should humans intervene in RAG Evaluation Rubrics to add leverage instead of toil?
...
- A behavior contract for RAG Evaluation Rubrics.
...
- [ ] The behavior contract for RAG Evaluation Rubrics is explicit.
