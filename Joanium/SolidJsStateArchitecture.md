---
name: SolidJS State Architecture
trigger: solidjs state architecture, help with solidjs state architecture, plan solidjs state architecture, improve solidjs state architecture, expert solidjs state architecture
description: Expert-level guidance for solidjs state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

SolidJS State Architecture is an expert frontend skill for SolidJS. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when SolidJS State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does SolidJS State Architecture make user state harder to reason about than it needs to be?
- Which interaction in SolidJS State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in SolidJS State Architecture would break if the DOM or state changed unexpectedly?
- How does SolidJS State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of SolidJS State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for SolidJS State Architecture.
...
- [ ] The state model for SolidJS State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend SolidJS State Architecture without guessing hidden UI contracts.
