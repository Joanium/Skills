---
name: Query Repair Rollback Plans
trigger: query repair rollback plans, help with query repair rollback plans, plan query repair rollback plans, improve query repair rollback plans, expert query repair rollback plans
description: Expert-level guidance for query repair rollback plans with action-oriented workflow, review criteria, and failure patterns.
---

Query Repair Rollback Plans is an expert coding workflow skill for Query Repair. The focal concern is Rollback Plans, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Query Repair Rollback Plans is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Query Repair Rollback Plans is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Query Repair Rollback Plans would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Query Repair Rollback Plans create a silent regression that tests may miss?
- How would the AI explain the safety of Query Repair Rollback Plans to a skeptical reviewer?
...
- A concise evidence note for Query Repair Rollback Plans.
...
- [ ] The evidence supporting Query Repair Rollback Plans is stronger than the assumptions.
