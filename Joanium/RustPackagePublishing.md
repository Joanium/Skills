---
name: Rust Package Publishing
trigger: rust package publishing, help with rust package publishing, plan rust package publishing, improve rust package publishing, expert rust package publishing
description: Expert-level guidance for rust package publishing with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rust Package Publishing is an expert programming-language skill for Rust. The focal concern is Package Publishing, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Rust Package Publishing affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Rust Package Publishing?
- Which abstraction in Rust Package Publishing becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Rust Package Publishing first?
- How will Rust Package Publishing behave when multiple contributors evolve it independently?
- Which part of Rust Package Publishing is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Rust Package Publishing.
- A failure-mode note that explains the sharp edges inside Rust Package Publishing.
...
- [ ] The main language-level risk in Rust Package Publishing is named explicitly.
...
- [ ] Future contributors can extend Rust Package Publishing without reverse-engineering hidden contracts.
