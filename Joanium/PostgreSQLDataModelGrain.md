---
name: PostgreSQL Data Model Grain
trigger: postgresql data model grain, help with postgresql data model grain, plan postgresql data model grain, improve postgresql data model grain, expert postgresql data model grain
description: Expert-level guidance for postgresql data model grain with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

PostgreSQL Data Model Grain is an expert data-systems skill for PostgreSQL. The focal concern is Data Model Grain, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when PostgreSQL Data Model Grain affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would PostgreSQL Data Model Grain create if two teams interpreted it differently?
- Which recovery path for PostgreSQL Data Model Grain is the least understood today?
- How will PostgreSQL Data Model Grain behave under backfills, reprocessing, or delayed sources?
- What cost pattern in PostgreSQL Data Model Grain is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside PostgreSQL Data Model Grain drifted subtly?
...
- A semantic contract note for PostgreSQL Data Model Grain.
...
- [ ] The meaning of PostgreSQL Data Model Grain is written in language consumers will actually use.
