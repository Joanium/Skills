---
name: Kotlin Interop Boundaries
trigger: kotlin interop boundaries, help with kotlin interop boundaries, plan kotlin interop boundaries, improve kotlin interop boundaries, expert kotlin interop boundaries
description: Expert-level guidance for kotlin interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kotlin Interop Boundaries is an expert programming-language skill for Kotlin. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Kotlin Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Kotlin Interop Boundaries?
- Which abstraction in Kotlin Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Kotlin Interop Boundaries first?
- How will Kotlin Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of Kotlin Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Kotlin Interop Boundaries.
- A failure-mode note that explains the sharp edges inside Kotlin Interop Boundaries.
...
- [ ] The main language-level risk in Kotlin Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend Kotlin Interop Boundaries without reverse-engineering hidden contracts.
