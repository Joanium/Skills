---
name: Extension Behavior Debugging Artifact Capture
trigger: extension behavior debugging artifact capture, help with extension behavior debugging artifact capture, plan extension behavior debugging artifact capture, improve extension behavior debugging artifact capture, expert extension behavior debugging artifact capture
description: Expert-level guidance for extension behavior debugging artifact capture with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Artifact Capture is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Artifact Capture, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Artifact Capture needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Artifact Capture unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Artifact Capture is least stable under real conditions?
- How would Extension Behavior Debugging Artifact Capture behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Artifact Capture capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Artifact Capture is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Artifact Capture.
...
- [ ] The state model behind Extension Behavior Debugging Artifact Capture is explicit.
