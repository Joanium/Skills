---
name: Kotlin Error Semantics
trigger: kotlin error semantics, help with kotlin error semantics, plan kotlin error semantics, improve kotlin error semantics, expert kotlin error semantics
description: Expert-level guidance for kotlin error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kotlin Error Semantics is an expert programming-language skill for Kotlin. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Kotlin Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Kotlin Error Semantics?
- Which abstraction in Kotlin Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Kotlin Error Semantics first?
- How will Kotlin Error Semantics behave when multiple contributors evolve it independently?
- Which part of Kotlin Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Kotlin Error Semantics.
- A failure-mode note that explains the sharp edges inside Kotlin Error Semantics.
...
- [ ] The main language-level risk in Kotlin Error Semantics is named explicitly.
...
- [ ] Future contributors can extend Kotlin Error Semantics without reverse-engineering hidden contracts.
