...
trigger: react state architecture, help with react state architecture, plan react state architecture, improve react state architecture, expert react state architecture
description: Expert-level guidance for react state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

React State Architecture is an expert frontend skill for React. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when React State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does React State Architecture make user state harder to reason about than it needs to be?
- Which interaction in React State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in React State Architecture would break if the DOM or state changed unexpectedly?
- How does React State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of React State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for React State Architecture.
...
- [ ] The state model for React State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend React State Architecture without guessing hidden UI contracts.
