---
name: Technical Debt Burn Down Dependency Blast Radius
trigger: technical debt burn down dependency blast radius, help with technical debt burn down dependency blast radius, plan technical debt burn down dependency blast radius, improve technical debt burn down dependency blast radius, expert technical debt burn down dependency blast radius
description: Expert-level guidance for technical debt burn down dependency blast radius with action-oriented workflow, review criteria, and failure patterns.
---

Technical Debt Burn Down Dependency Blast Radius is an expert repository and delivery skill for Technical Debt Burn Down. The focal concern is Dependency Blast Radius, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Technical Debt Burn Down Dependency Blast Radius determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Technical Debt Burn Down Dependency Blast Radius riskier than the diff itself suggests?
- Which reviewer would challenge Technical Debt Burn Down Dependency Blast Radius first, and what evidence would they need?
- How could Technical Debt Burn Down Dependency Blast Radius fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Technical Debt Burn Down Dependency Blast Radius would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Technical Debt Burn Down Dependency Blast Radius concretely?
...
- A blast-radius note for Technical Debt Burn Down Dependency Blast Radius.
...
- [ ] The repository-level blast radius of Technical Debt Burn Down Dependency Blast Radius is explicit.
