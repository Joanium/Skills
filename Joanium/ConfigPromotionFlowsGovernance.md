---
name: Config Promotion Flows Governance
trigger: config promotion flows governance, help with config promotion flows governance, plan config promotion flows governance, improve config promotion flows governance, expert config promotion flows governance
description: Expert-level guidance for config promotion flows governance, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Config Promotion Flows Governance is an expert-level infrastructure skill for reducing operational risk while keeping rollout control, recovery, and ownership explicit. The emphasis here is on ownership, policy, exception handling, and auditability so the domain does not decay into ad hoc decisions.

## When To Use

- Use this when Config Promotion Flows Governance changes service availability, deployment safety, or recovery behavior.
...
- Who sets the policy for Config Promotion Flows Governance, and who is allowed to grant exceptions?
...
- A policy document for Config Promotion Flows Governance with explicit scope.
