---
name: Query Repair Verification Loops
trigger: query repair verification loops, help with query repair verification loops, plan query repair verification loops, improve query repair verification loops, expert query repair verification loops
description: Expert-level guidance for query repair verification loops with action-oriented workflow, review criteria, and failure patterns.
---

Query Repair Verification Loops is an expert coding workflow skill for Query Repair. The focal concern is Verification Loops, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Query Repair Verification Loops is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Query Repair Verification Loops is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Query Repair Verification Loops would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Query Repair Verification Loops create a silent regression that tests may miss?
- How would the AI explain the safety of Query Repair Verification Loops to a skeptical reviewer?
...
- A concise evidence note for Query Repair Verification Loops.
...
- [ ] The evidence supporting Query Repair Verification Loops is stronger than the assumptions.
