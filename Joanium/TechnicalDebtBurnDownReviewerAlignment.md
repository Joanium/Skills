---
name: Technical Debt Burn Down Reviewer Alignment
trigger: technical debt burn down reviewer alignment, help with technical debt burn down reviewer alignment, plan technical debt burn down reviewer alignment, improve technical debt burn down reviewer alignment, expert technical debt burn down reviewer alignment
description: Expert-level guidance for technical debt burn down reviewer alignment with action-oriented workflow, review criteria, and failure patterns.
---

Technical Debt Burn Down Reviewer Alignment is an expert repository and delivery skill for Technical Debt Burn Down. The focal concern is Reviewer Alignment, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Technical Debt Burn Down Reviewer Alignment determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Technical Debt Burn Down Reviewer Alignment riskier than the diff itself suggests?
- Which reviewer would challenge Technical Debt Burn Down Reviewer Alignment first, and what evidence would they need?
- How could Technical Debt Burn Down Reviewer Alignment fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Technical Debt Burn Down Reviewer Alignment would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Technical Debt Burn Down Reviewer Alignment concretely?
...
- A blast-radius note for Technical Debt Burn Down Reviewer Alignment.
...
- [ ] The repository-level blast radius of Technical Debt Burn Down Reviewer Alignment is explicit.
