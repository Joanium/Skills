---
name: dbt Consistency Rules
trigger: dbt consistency rules, help with dbt consistency rules, plan dbt consistency rules, improve dbt consistency rules, expert dbt consistency rules
description: Expert-level guidance for dbt consistency rules with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

dbt Consistency Rules is an expert data-systems skill for dbt. The focal concern is Consistency Rules, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when dbt Consistency Rules affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would dbt Consistency Rules create if two teams interpreted it differently?
- Which recovery path for dbt Consistency Rules is the least understood today?
- How will dbt Consistency Rules behave under backfills, reprocessing, or delayed sources?
- What cost pattern in dbt Consistency Rules is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside dbt Consistency Rules drifted subtly?
...
- A semantic contract note for dbt Consistency Rules.
...
- [ ] The meaning of dbt Consistency Rules is written in language consumers will actually use.
