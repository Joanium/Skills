---
name: Extension Behavior Debugging Retry Windows
trigger: extension behavior debugging retry windows, help with extension behavior debugging retry windows, plan extension behavior debugging retry windows, improve extension behavior debugging retry windows, expert extension behavior debugging retry windows
description: Expert-level guidance for extension behavior debugging retry windows with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Retry Windows is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Retry Windows, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Retry Windows needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Retry Windows unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Retry Windows is least stable under real conditions?
- How would Extension Behavior Debugging Retry Windows behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Retry Windows capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Retry Windows is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Retry Windows.
...
- [ ] The state model behind Extension Behavior Debugging Retry Windows is explicit.
