---
name: Identity Platforms Scaling Paths
trigger: identity platforms scaling paths, help with identity platforms scaling paths, plan identity platforms scaling paths, improve identity platforms scaling paths, expert identity platforms scaling paths
description: Expert-level guidance for identity platforms scaling paths with action-oriented workflow, review criteria, and failure patterns.
---

Identity Platforms Scaling Paths is an expert architecture and systems skill for Identity Platforms. The focal concern is Scaling Paths, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Identity Platforms Scaling Paths needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Identity Platforms Scaling Paths becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Identity Platforms Scaling Paths is currently cheap only because the bill has not arrived yet?
- How would Identity Platforms Scaling Paths be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Identity Platforms Scaling Paths too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Identity Platforms Scaling Paths.
...
- [ ] The key boundaries inside Identity Platforms Scaling Paths are explicit and justified.
