---
name: Unit Testing Systems Failure Matrices
trigger: unit testing systems failure matrices, help with unit testing systems failure matrices, plan unit testing systems failure matrices, improve unit testing systems failure matrices, expert unit testing systems failure matrices
description: Expert-level guidance for unit testing systems failure matrices with action-oriented workflow, review criteria, and failure patterns.
---

Unit Testing Systems Failure Matrices is an expert testing and quality skill for Unit Testing Systems. The focal concern is Failure Matrices, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Unit Testing Systems Failure Matrices needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Unit Testing Systems Failure Matrices is still untested even if coverage looks healthy?
- Which assertion in Unit Testing Systems Failure Matrices would stay green while the user-visible behavior regressed?
- How much of Unit Testing Systems Failure Matrices depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Unit Testing Systems Failure Matrices need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Unit Testing Systems Failure Matrices.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Unit Testing Systems Failure Matrices should mean.
...
- [ ] The key behavior protected by Unit Testing Systems Failure Matrices is explicit.
