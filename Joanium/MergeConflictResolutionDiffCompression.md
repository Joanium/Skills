---
name: Merge Conflict Resolution Diff Compression
trigger: merge conflict resolution diff compression, help with merge conflict resolution diff compression, plan merge conflict resolution diff compression, improve merge conflict resolution diff compression, expert merge conflict resolution diff compression
description: Expert-level guidance for merge conflict resolution diff compression with action-oriented workflow, review criteria, and failure patterns.
---

Merge Conflict Resolution Diff Compression is an expert repository and delivery skill for Merge Conflict Resolution. The focal concern is Diff Compression, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Merge Conflict Resolution Diff Compression determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Merge Conflict Resolution Diff Compression riskier than the diff itself suggests?
- Which reviewer would challenge Merge Conflict Resolution Diff Compression first, and what evidence would they need?
- How could Merge Conflict Resolution Diff Compression fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Merge Conflict Resolution Diff Compression would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Merge Conflict Resolution Diff Compression concretely?
...
- A blast-radius note for Merge Conflict Resolution Diff Compression.
...
- [ ] The repository-level blast radius of Merge Conflict Resolution Diff Compression is explicit.
