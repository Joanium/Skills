---
name: Spark Consistency Rules
trigger: spark consistency rules, help with spark consistency rules, plan spark consistency rules, improve spark consistency rules, expert spark consistency rules
description: Expert-level guidance for spark consistency rules with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Spark Consistency Rules is an expert data-systems skill for Spark. The focal concern is Consistency Rules, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Spark Consistency Rules affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Spark Consistency Rules create if two teams interpreted it differently?
- Which recovery path for Spark Consistency Rules is the least understood today?
- How will Spark Consistency Rules behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Spark Consistency Rules is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Spark Consistency Rules drifted subtly?
...
- A semantic contract note for Spark Consistency Rules.
...
- [ ] The meaning of Spark Consistency Rules is written in language consumers will actually use.
