---
name: Logging Instrumentation Patch Design
trigger: logging instrumentation patch design, help with logging instrumentation patch design, plan logging instrumentation patch design, improve logging instrumentation patch design, expert logging instrumentation patch design
description: Expert-level guidance for logging instrumentation patch design with action-oriented workflow, review criteria, and failure patterns.
---

Logging Instrumentation Patch Design is an expert coding workflow skill for Logging Instrumentation. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Logging Instrumentation Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Logging Instrumentation Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Logging Instrumentation Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Logging Instrumentation Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Logging Instrumentation Patch Design to a skeptical reviewer?
...
- A concise evidence note for Logging Instrumentation Patch Design.
...
- [ ] The evidence supporting Logging Instrumentation Patch Design is stronger than the assumptions.
