...
trigger: react component testing, help with react component testing, plan react component testing, improve react component testing, expert react component testing
description: Expert-level guidance for react component testing with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

React Component Testing is an expert frontend skill for React. The focal concern is Component Testing, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when React Component Testing affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does React Component Testing make user state harder to reason about than it needs to be?
- Which interaction in React Component Testing degrades first under latency, stale data, or partial failure?
- What accessibility behavior in React Component Testing would break if the DOM or state changed unexpectedly?
- How does React Component Testing fail when routing, hydration, or rendering order becomes non-ideal?
- What part of React Component Testing looks correct in a mock but collapses under production behavior?
...
- A state transition map for React Component Testing.
...
- [ ] The state model for React Component Testing is clear enough to debug.
...
- [ ] Another engineer can extend React Component Testing without guessing hidden UI contracts.
