---
name: Model Routing Operations Trace Analysis
trigger: model routing operations trace analysis, help with model routing operations trace analysis, plan model routing operations trace analysis, improve model routing operations trace analysis, expert model routing operations trace analysis
description: Expert-level guidance for model routing operations trace analysis with action-oriented workflow, review criteria, and failure patterns.
---

Model Routing Operations Trace Analysis is an expert applied AI skill for Model Routing Operations. The focal concern is Trace Analysis, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Model Routing Operations Trace Analysis affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Model Routing Operations Trace Analysis is currently measured by vibes rather than a real contract?
- Which failure in Model Routing Operations Trace Analysis would be hardest to notice before it harms users or operators?
- How will Model Routing Operations Trace Analysis behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Model Routing Operations Trace Analysis?
...
- A behavior contract for Model Routing Operations Trace Analysis.
...
- [ ] The contract behind Model Routing Operations Trace Analysis is explicit enough to evaluate.
