---
name: Permission Prompt Triage Timing Stability
trigger: permission prompt triage timing stability, help with permission prompt triage timing stability, plan permission prompt triage timing stability, improve permission prompt triage timing stability, expert permission prompt triage timing stability
description: Expert-level guidance for permission prompt triage timing stability with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage Timing Stability is an expert browser automation skill for Permission Prompt Triage. The focal concern is Timing Stability, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage Timing Stability needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage Timing Stability unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage Timing Stability is least stable under real conditions?
- How would Permission Prompt Triage Timing Stability behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage Timing Stability capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage Timing Stability is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage Timing Stability.
...
- [ ] The state model behind Permission Prompt Triage Timing Stability is explicit.
