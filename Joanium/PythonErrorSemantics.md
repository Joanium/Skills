---
name: Python Error Semantics
trigger: python error semantics, help with python error semantics, plan python error semantics, improve python error semantics, expert python error semantics
description: Expert-level guidance for python error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Python Error Semantics is an expert programming-language skill for Python. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Python Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Python Error Semantics?
- Which abstraction in Python Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Python Error Semantics first?
- How will Python Error Semantics behave when multiple contributors evolve it independently?
- Which part of Python Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Python Error Semantics.
- A failure-mode note that explains the sharp edges inside Python Error Semantics.
...
- [ ] The main language-level risk in Python Error Semantics is named explicitly.
...
- [ ] Future contributors can extend Python Error Semantics without reverse-engineering hidden contracts.
