---
name: Svelte Rendering Performance
trigger: svelte rendering performance, help with svelte rendering performance, plan svelte rendering performance, improve svelte rendering performance, expert svelte rendering performance
description: Expert-level guidance for svelte rendering performance with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte Rendering Performance is an expert frontend skill for Svelte. The focal concern is Rendering Performance, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte Rendering Performance affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte Rendering Performance make user state harder to reason about than it needs to be?
- Which interaction in Svelte Rendering Performance degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte Rendering Performance would break if the DOM or state changed unexpectedly?
- How does Svelte Rendering Performance fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte Rendering Performance looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte Rendering Performance.
...
- [ ] The state model for Svelte Rendering Performance is clear enough to debug.
...
- [ ] Another engineer can extend Svelte Rendering Performance without guessing hidden UI contracts.
