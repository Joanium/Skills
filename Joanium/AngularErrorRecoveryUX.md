---
name: Angular Error Recovery UX
trigger: angular error recovery ux, help with angular error recovery ux, plan angular error recovery ux, improve angular error recovery ux, expert angular error recovery ux
description: Expert-level guidance for angular error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Angular Error Recovery UX is an expert frontend skill for Angular. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Angular Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Angular Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in Angular Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Angular Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does Angular Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Angular Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for Angular Error Recovery UX.
...
- [ ] The state model for Angular Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend Angular Error Recovery UX without guessing hidden UI contracts.
