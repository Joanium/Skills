---
name: Embeddings Failure Analysis
trigger: embeddings failure analysis, help with embeddings failure analysis, plan embeddings failure analysis, improve embeddings failure analysis, expert embeddings failure analysis
description: Expert-level guidance for embeddings failure analysis with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Embeddings Failure Analysis is an expert applied-AI skill for Embeddings. The focal concern is Failure Analysis, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Embeddings Failure Analysis affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Embeddings Failure Analysis would look acceptable in a demo but fail under production ambiguity?
- Which part of Embeddings Failure Analysis lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Embeddings Failure Analysis fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Embeddings Failure Analysis is getting better rather than merely changing?
- Where should humans intervene in Embeddings Failure Analysis to add leverage instead of toil?
...
- A behavior contract for Embeddings Failure Analysis.
...
- [ ] The behavior contract for Embeddings Failure Analysis is explicit.
