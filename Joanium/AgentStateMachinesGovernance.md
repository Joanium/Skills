---
name: Agent State Machines Governance
trigger: agent state machines governance, help with agent state machines governance, plan agent state machines governance, improve agent state machines governance, expert agent state machines governance
description: Expert-level guidance for agent state machines governance, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Agent State Machines Governance is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is on ownership, policy, exception handling, and auditability so the domain does not decay into ad hoc decisions.

## When To Use

- Use this when Agent State Machines Governance influences user-visible output, automation, or safety-sensitive decisions.
...
- Who sets the policy for Agent State Machines Governance, and who is allowed to grant exceptions?
...
- A policy document for Agent State Machines Governance with explicit scope.
