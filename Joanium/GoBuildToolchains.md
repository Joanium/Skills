---
name: Go Build Toolchains
trigger: go build toolchains, help with go build toolchains, plan go build toolchains, improve go build toolchains, expert go build toolchains
description: Expert-level guidance for go build toolchains with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Go Build Toolchains is an expert programming-language skill for Go. The focal concern is Build Toolchains, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Go Build Toolchains affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Go Build Toolchains?
- Which abstraction in Go Build Toolchains becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Go Build Toolchains first?
- How will Go Build Toolchains behave when multiple contributors evolve it independently?
- Which part of Go Build Toolchains is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Go Build Toolchains.
- A failure-mode note that explains the sharp edges inside Go Build Toolchains.
...
- [ ] The main language-level risk in Go Build Toolchains is named explicitly.
...
- [ ] Future contributors can extend Go Build Toolchains without reverse-engineering hidden contracts.
