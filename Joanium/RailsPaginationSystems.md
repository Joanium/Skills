---
name: Rails Pagination Systems
trigger: rails pagination systems, help with rails pagination systems, plan rails pagination systems, improve rails pagination systems, expert rails pagination systems
description: Expert-level guidance for rails pagination systems with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rails Pagination Systems is an expert backend skill for Rails. The focal concern is Pagination Systems, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Rails Pagination Systems shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Rails Pagination Systems breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Rails Pagination Systems?
- Where does Rails Pagination Systems need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Rails Pagination Systems is unhealthy before customers do?
- What compatibility promise is implied by Rails Pagination Systems even if it was never written down?
...
- A contract note for Rails Pagination Systems that includes failure semantics.
...
- [ ] The contract for Rails Pagination Systems is explicit about success and failure behavior.
