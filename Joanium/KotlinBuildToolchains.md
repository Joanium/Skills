---
name: Kotlin Build Toolchains
trigger: kotlin build toolchains, help with kotlin build toolchains, plan kotlin build toolchains, improve kotlin build toolchains, expert kotlin build toolchains
description: Expert-level guidance for kotlin build toolchains with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kotlin Build Toolchains is an expert programming-language skill for Kotlin. The focal concern is Build Toolchains, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Kotlin Build Toolchains affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Kotlin Build Toolchains?
- Which abstraction in Kotlin Build Toolchains becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Kotlin Build Toolchains first?
- How will Kotlin Build Toolchains behave when multiple contributors evolve it independently?
- Which part of Kotlin Build Toolchains is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Kotlin Build Toolchains.
- A failure-mode note that explains the sharp edges inside Kotlin Build Toolchains.
...
- [ ] The main language-level risk in Kotlin Build Toolchains is named explicitly.
...
- [ ] Future contributors can extend Kotlin Build Toolchains without reverse-engineering hidden contracts.
