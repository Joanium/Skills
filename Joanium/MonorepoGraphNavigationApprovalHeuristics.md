---
name: Monorepo Graph Navigation Approval Heuristics
trigger: monorepo graph navigation approval heuristics, help with monorepo graph navigation approval heuristics, plan monorepo graph navigation approval heuristics, improve monorepo graph navigation approval heuristics, expert monorepo graph navigation approval heuristics
description: Expert-level guidance for monorepo graph navigation approval heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Monorepo Graph Navigation Approval Heuristics is an expert repository and delivery skill for Monorepo Graph Navigation. The focal concern is Approval Heuristics, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Monorepo Graph Navigation Approval Heuristics determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Monorepo Graph Navigation Approval Heuristics riskier than the diff itself suggests?
- Which reviewer would challenge Monorepo Graph Navigation Approval Heuristics first, and what evidence would they need?
- How could Monorepo Graph Navigation Approval Heuristics fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Monorepo Graph Navigation Approval Heuristics would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Monorepo Graph Navigation Approval Heuristics concretely?
...
- A blast-radius note for Monorepo Graph Navigation Approval Heuristics.
...
- [ ] The repository-level blast radius of Monorepo Graph Navigation Approval Heuristics is explicit.
