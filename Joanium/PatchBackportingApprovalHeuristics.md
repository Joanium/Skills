---
name: Patch Backporting Approval Heuristics
trigger: patch backporting approval heuristics, help with patch backporting approval heuristics, plan patch backporting approval heuristics, improve patch backporting approval heuristics, expert patch backporting approval heuristics
description: Expert-level guidance for patch backporting approval heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Patch Backporting Approval Heuristics is an expert repository and delivery skill for Patch Backporting. The focal concern is Approval Heuristics, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Patch Backporting Approval Heuristics determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Patch Backporting Approval Heuristics riskier than the diff itself suggests?
- Which reviewer would challenge Patch Backporting Approval Heuristics first, and what evidence would they need?
- How could Patch Backporting Approval Heuristics fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Patch Backporting Approval Heuristics would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Patch Backporting Approval Heuristics concretely?
...
- A blast-radius note for Patch Backporting Approval Heuristics.
...
- [ ] The repository-level blast radius of Patch Backporting Approval Heuristics is explicit.
