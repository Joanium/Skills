---
name: Next.js Rendering Performance
trigger: next.js rendering performance, help with next.js rendering performance, plan next.js rendering performance, improve next.js rendering performance, expert next.js rendering performance
description: Expert-level guidance for next.js rendering performance with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Next.js Rendering Performance is an expert frontend skill for Next.js. The focal concern is Rendering Performance, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Next.js Rendering Performance affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Next.js Rendering Performance make user state harder to reason about than it needs to be?
- Which interaction in Next.js Rendering Performance degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Next.js Rendering Performance would break if the DOM or state changed unexpectedly?
- How does Next.js Rendering Performance fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Next.js Rendering Performance looks correct in a mock but collapses under production behavior?
...
- A state transition map for Next.js Rendering Performance.
...
- [ ] The state model for Next.js Rendering Performance is clear enough to debug.
...
- [ ] Another engineer can extend Next.js Rendering Performance without guessing hidden UI contracts.
