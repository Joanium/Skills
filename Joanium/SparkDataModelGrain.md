---
name: Spark Data Model Grain
trigger: spark data model grain, help with spark data model grain, plan spark data model grain, improve spark data model grain, expert spark data model grain
description: Expert-level guidance for spark data model grain with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Spark Data Model Grain is an expert data-systems skill for Spark. The focal concern is Data Model Grain, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Spark Data Model Grain affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Spark Data Model Grain create if two teams interpreted it differently?
- Which recovery path for Spark Data Model Grain is the least understood today?
- How will Spark Data Model Grain behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Spark Data Model Grain is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Spark Data Model Grain drifted subtly?
...
- A semantic contract note for Spark Data Model Grain.
...
- [ ] The meaning of Spark Data Model Grain is written in language consumers will actually use.
