---
name: Build Failure Response Reviewer Alignment
trigger: build failure response reviewer alignment, help with build failure response reviewer alignment, plan build failure response reviewer alignment, improve build failure response reviewer alignment, expert build failure response reviewer alignment
description: Expert-level guidance for build failure response reviewer alignment with action-oriented workflow, review criteria, and failure patterns.
---

Build Failure Response Reviewer Alignment is an expert repository and delivery skill for Build Failure Response. The focal concern is Reviewer Alignment, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Build Failure Response Reviewer Alignment determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Build Failure Response Reviewer Alignment riskier than the diff itself suggests?
- Which reviewer would challenge Build Failure Response Reviewer Alignment first, and what evidence would they need?
- How could Build Failure Response Reviewer Alignment fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Build Failure Response Reviewer Alignment would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Build Failure Response Reviewer Alignment concretely?
...
- A blast-radius note for Build Failure Response Reviewer Alignment.
...
- [ ] The repository-level blast radius of Build Failure Response Reviewer Alignment is explicit.
