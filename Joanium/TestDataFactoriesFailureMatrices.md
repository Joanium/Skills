---
name: Test Data Factories Failure Matrices
trigger: test data factories failure matrices, help with test data factories failure matrices, plan test data factories failure matrices, improve test data factories failure matrices, expert test data factories failure matrices
description: Expert-level guidance for test data factories failure matrices with action-oriented workflow, review criteria, and failure patterns.
---

Test Data Factories Failure Matrices is an expert testing and quality skill for Test Data Factories. The focal concern is Failure Matrices, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Test Data Factories Failure Matrices needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Test Data Factories Failure Matrices is still untested even if coverage looks healthy?
- Which assertion in Test Data Factories Failure Matrices would stay green while the user-visible behavior regressed?
- How much of Test Data Factories Failure Matrices depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Test Data Factories Failure Matrices need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Test Data Factories Failure Matrices.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Test Data Factories Failure Matrices should mean.
...
- [ ] The key behavior protected by Test Data Factories Failure Matrices is explicit.
