---
name: Permission Prompt Triage Environment Parity
trigger: permission prompt triage environment parity, help with permission prompt triage environment parity, plan permission prompt triage environment parity, improve permission prompt triage environment parity, expert permission prompt triage environment parity
description: Expert-level guidance for permission prompt triage environment parity with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage Environment Parity is an expert browser automation skill for Permission Prompt Triage. The focal concern is Environment Parity, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage Environment Parity needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage Environment Parity unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage Environment Parity is least stable under real conditions?
- How would Permission Prompt Triage Environment Parity behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage Environment Parity capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage Environment Parity is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage Environment Parity.
...
- [ ] The state model behind Permission Prompt Triage Environment Parity is explicit.
