---
name: Nuxt Error Recovery UX
trigger: nuxt error recovery ux, help with nuxt error recovery ux, plan nuxt error recovery ux, improve nuxt error recovery ux, expert nuxt error recovery ux
description: Expert-level guidance for nuxt error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Nuxt Error Recovery UX is an expert frontend skill for Nuxt. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Nuxt Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Nuxt Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in Nuxt Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Nuxt Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does Nuxt Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Nuxt Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for Nuxt Error Recovery UX.
...
- [ ] The state model for Nuxt Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend Nuxt Error Recovery UX without guessing hidden UI contracts.
