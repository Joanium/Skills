---
name: Bug Reproduction Risk Ranking
trigger: bug reproduction risk ranking, help with bug reproduction risk ranking, plan bug reproduction risk ranking, improve bug reproduction risk ranking, expert bug reproduction risk ranking
description: Expert-level guidance for bug reproduction risk ranking with action-oriented workflow, review criteria, and failure patterns.
---

Bug Reproduction Risk Ranking is an expert coding workflow skill for Bug Reproduction. The focal concern is Risk Ranking, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Bug Reproduction Risk Ranking is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Bug Reproduction Risk Ranking is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Bug Reproduction Risk Ranking would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Bug Reproduction Risk Ranking create a silent regression that tests may miss?
- How would the AI explain the safety of Bug Reproduction Risk Ranking to a skeptical reviewer?
...
- A concise evidence note for Bug Reproduction Risk Ranking.
...
- [ ] The evidence supporting Bug Reproduction Risk Ranking is stronger than the assumptions.
