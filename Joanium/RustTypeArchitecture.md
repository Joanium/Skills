---
name: Rust Type Architecture
trigger: rust type architecture, help with rust type architecture, plan rust type architecture, improve rust type architecture, expert rust type architecture
description: Expert-level guidance for rust type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rust Type Architecture is an expert programming-language skill for Rust. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Rust Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Rust Type Architecture?
- Which abstraction in Rust Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Rust Type Architecture first?
- How will Rust Type Architecture behave when multiple contributors evolve it independently?
- Which part of Rust Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Rust Type Architecture.
- A failure-mode note that explains the sharp edges inside Rust Type Architecture.
...
- [ ] The main language-level risk in Rust Type Architecture is named explicitly.
...
- [ ] Future contributors can extend Rust Type Architecture without reverse-engineering hidden contracts.
