---
name: Search Architectures Migration Seams
trigger: search architectures migration seams, help with search architectures migration seams, plan search architectures migration seams, improve search architectures migration seams, expert search architectures migration seams
description: Expert-level guidance for search architectures migration seams with action-oriented workflow, review criteria, and failure patterns.
---

Search Architectures Migration Seams is an expert architecture and systems skill for Search Architectures. The focal concern is Migration Seams, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Search Architectures Migration Seams needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Search Architectures Migration Seams becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Search Architectures Migration Seams is currently cheap only because the bill has not arrived yet?
- How would Search Architectures Migration Seams be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Search Architectures Migration Seams too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Search Architectures Migration Seams.
...
- [ ] The key boundaries inside Search Architectures Migration Seams are explicit and justified.
