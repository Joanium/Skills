---
name: Visual Diff Triage Retry Windows
trigger: visual diff triage retry windows, help with visual diff triage retry windows, plan visual diff triage retry windows, improve visual diff triage retry windows, expert visual diff triage retry windows
description: Expert-level guidance for visual diff triage retry windows with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage Retry Windows is an expert browser automation skill for Visual Diff Triage. The focal concern is Retry Windows, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage Retry Windows needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage Retry Windows unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage Retry Windows is least stable under real conditions?
- How would Visual Diff Triage Retry Windows behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage Retry Windows capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage Retry Windows is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage Retry Windows.
...
- [ ] The state model behind Visual Diff Triage Retry Windows is explicit.
