---
name: Extension Behavior Debugging State Reset
trigger: extension behavior debugging state reset, help with extension behavior debugging state reset, plan extension behavior debugging state reset, improve extension behavior debugging state reset, expert extension behavior debugging state reset
description: Expert-level guidance for extension behavior debugging state reset with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging State Reset is an expert browser automation skill for Extension Behavior Debugging. The focal concern is State Reset, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging State Reset needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging State Reset unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging State Reset is least stable under real conditions?
- How would Extension Behavior Debugging State Reset behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging State Reset capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging State Reset is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging State Reset.
...
- [ ] The state model behind Extension Behavior Debugging State Reset is explicit.
