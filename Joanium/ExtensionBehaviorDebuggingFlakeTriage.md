---
name: Extension Behavior Debugging Flake Triage
trigger: extension behavior debugging flake triage, help with extension behavior debugging flake triage, plan extension behavior debugging flake triage, improve extension behavior debugging flake triage, expert extension behavior debugging flake triage
description: Expert-level guidance for extension behavior debugging flake triage with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Flake Triage is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Flake Triage, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Flake Triage needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Flake Triage unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Flake Triage is least stable under real conditions?
- How would Extension Behavior Debugging Flake Triage behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Flake Triage capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Flake Triage is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Flake Triage.
...
- [ ] The state model behind Extension Behavior Debugging Flake Triage is explicit.
