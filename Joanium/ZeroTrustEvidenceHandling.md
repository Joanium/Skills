---
name: Zero Trust Evidence Handling
trigger: zero trust evidence handling, help with zero trust evidence handling, plan zero trust evidence handling, improve zero trust evidence handling, expert zero trust evidence handling
description: Expert-level guidance for zero trust evidence handling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Zero Trust Evidence Handling is an expert security skill for Zero Trust. The focal concern is Evidence Handling, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when Zero Trust Evidence Handling influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in Zero Trust Evidence Handling first?
- Which exception or emergency path makes Zero Trust Evidence Handling less true in practice than in policy?
- What evidence would be missing if Zero Trust Evidence Handling failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does Zero Trust Evidence Handling degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for Zero Trust Evidence Handling.
...
- [ ] The attacker model for Zero Trust Evidence Handling is realistic and current.
...
- [ ] The team can explain how Zero Trust Evidence Handling fails and how it recovers.
