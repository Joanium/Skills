---
name: Exploratory QA Boundary Scenarios
trigger: exploratory qa boundary scenarios, help with exploratory qa boundary scenarios, plan exploratory qa boundary scenarios, improve exploratory qa boundary scenarios, expert exploratory qa boundary scenarios
description: Expert-level guidance for exploratory qa boundary scenarios with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Boundary Scenarios is an expert testing and quality skill for Exploratory QA. The focal concern is Boundary Scenarios, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Boundary Scenarios needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Boundary Scenarios is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Boundary Scenarios would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Boundary Scenarios depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Boundary Scenarios need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Boundary Scenarios.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Boundary Scenarios should mean.
...
- [ ] The key behavior protected by Exploratory QA Boundary Scenarios is explicit.
