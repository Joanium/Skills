---
name: Bug Reproduction Patch Design
trigger: bug reproduction patch design, help with bug reproduction patch design, plan bug reproduction patch design, improve bug reproduction patch design, expert bug reproduction patch design
description: Expert-level guidance for bug reproduction patch design with action-oriented workflow, review criteria, and failure patterns.
---

Bug Reproduction Patch Design is an expert coding workflow skill for Bug Reproduction. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Bug Reproduction Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Bug Reproduction Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Bug Reproduction Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Bug Reproduction Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Bug Reproduction Patch Design to a skeptical reviewer?
...
- A concise evidence note for Bug Reproduction Patch Design.
...
- [ ] The evidence supporting Bug Reproduction Patch Design is stronger than the assumptions.
