---
name: Merge Conflict Resolution Staging Plans
trigger: merge conflict resolution staging plans, help with merge conflict resolution staging plans, plan merge conflict resolution staging plans, improve merge conflict resolution staging plans, expert merge conflict resolution staging plans
description: Expert-level guidance for merge conflict resolution staging plans with action-oriented workflow, review criteria, and failure patterns.
---

Merge Conflict Resolution Staging Plans is an expert repository and delivery skill for Merge Conflict Resolution. The focal concern is Staging Plans, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Merge Conflict Resolution Staging Plans determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Merge Conflict Resolution Staging Plans riskier than the diff itself suggests?
- Which reviewer would challenge Merge Conflict Resolution Staging Plans first, and what evidence would they need?
- How could Merge Conflict Resolution Staging Plans fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Merge Conflict Resolution Staging Plans would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Merge Conflict Resolution Staging Plans concretely?
...
- A blast-radius note for Merge Conflict Resolution Staging Plans.
...
- [ ] The repository-level blast radius of Merge Conflict Resolution Staging Plans is explicit.
