---
name: Express Cache Invalidation
trigger: express cache invalidation, help with express cache invalidation, plan express cache invalidation, improve express cache invalidation, expert express cache invalidation
description: Expert-level guidance for express cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Express Cache Invalidation is an expert backend skill for Express. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Express Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Express Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Express Cache Invalidation?
- Where does Express Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Express Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by Express Cache Invalidation even if it was never written down?
...
- A contract note for Express Cache Invalidation that includes failure semantics.
...
- [ ] The contract for Express Cache Invalidation is explicit about success and failure behavior.
