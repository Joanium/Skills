---
name: RAG Cost Control
trigger: rag cost control, help with rag cost control, plan rag cost control, improve rag cost control, expert rag cost control
description: Expert-level guidance for rag cost control with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

RAG Cost Control is an expert applied-AI skill for RAG. The focal concern is Cost Control, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when RAG Cost Control affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in RAG Cost Control would look acceptable in a demo but fail under production ambiguity?
- Which part of RAG Cost Control lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will RAG Cost Control fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that RAG Cost Control is getting better rather than merely changing?
- Where should humans intervene in RAG Cost Control to add leverage instead of toil?
...
- A behavior contract for RAG Cost Control.
...
- [ ] The behavior contract for RAG Cost Control is explicit.
