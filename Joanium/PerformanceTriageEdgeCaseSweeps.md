---
name: Performance Triage Edge Case Sweeps
trigger: performance triage edge case sweeps, help with performance triage edge case sweeps, plan performance triage edge case sweeps, improve performance triage edge case sweeps, expert performance triage edge case sweeps
description: Expert-level guidance for performance triage edge case sweeps with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Edge Case Sweeps is an expert coding workflow skill for Performance Triage. The focal concern is Edge Case Sweeps, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Edge Case Sweeps is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Edge Case Sweeps is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Edge Case Sweeps would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Edge Case Sweeps create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Edge Case Sweeps to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Edge Case Sweeps.
...
- [ ] The evidence supporting Performance Triage Edge Case Sweeps is stronger than the assumptions.
