---
name: Remix Rendering Performance
trigger: remix rendering performance, help with remix rendering performance, plan remix rendering performance, improve remix rendering performance, expert remix rendering performance
description: Expert-level guidance for remix rendering performance with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Remix Rendering Performance is an expert frontend skill for Remix. The focal concern is Rendering Performance, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Remix Rendering Performance affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Remix Rendering Performance make user state harder to reason about than it needs to be?
- Which interaction in Remix Rendering Performance degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Remix Rendering Performance would break if the DOM or state changed unexpectedly?
- How does Remix Rendering Performance fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Remix Rendering Performance looks correct in a mock but collapses under production behavior?
...
- A state transition map for Remix Rendering Performance.
...
- [ ] The state model for Remix Rendering Performance is clear enough to debug.
...
- [ ] Another engineer can extend Remix Rendering Performance without guessing hidden UI contracts.
