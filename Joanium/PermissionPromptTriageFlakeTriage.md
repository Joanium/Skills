---
name: Permission Prompt Triage Flake Triage
trigger: permission prompt triage flake triage, help with permission prompt triage flake triage, plan permission prompt triage flake triage, improve permission prompt triage flake triage, expert permission prompt triage flake triage
description: Expert-level guidance for permission prompt triage flake triage with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage Flake Triage is an expert browser automation skill for Permission Prompt Triage. The focal concern is Flake Triage, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage Flake Triage needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage Flake Triage unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage Flake Triage is least stable under real conditions?
- How would Permission Prompt Triage Flake Triage behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage Flake Triage capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage Flake Triage is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage Flake Triage.
...
- [ ] The state model behind Permission Prompt Triage Flake Triage is explicit.
