---
name: TypeScript Interop Boundaries
trigger: typescript interop boundaries, help with typescript interop boundaries, plan typescript interop boundaries, improve typescript interop boundaries, expert typescript interop boundaries
description: Expert-level guidance for typescript interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

TypeScript Interop Boundaries is an expert programming-language skill for TypeScript. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when TypeScript Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside TypeScript Interop Boundaries?
- Which abstraction in TypeScript Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in TypeScript Interop Boundaries first?
- How will TypeScript Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of TypeScript Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for TypeScript Interop Boundaries.
- A failure-mode note that explains the sharp edges inside TypeScript Interop Boundaries.
...
- [ ] The main language-level risk in TypeScript Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend TypeScript Interop Boundaries without reverse-engineering hidden contracts.
