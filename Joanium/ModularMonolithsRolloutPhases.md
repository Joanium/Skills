---
name: Modular Monoliths Rollout Phases
trigger: modular monoliths rollout phases, help with modular monoliths rollout phases, plan modular monoliths rollout phases, improve modular monoliths rollout phases, expert modular monoliths rollout phases
description: Expert-level guidance for modular monoliths rollout phases with action-oriented workflow, review criteria, and failure patterns.
---

Modular Monoliths Rollout Phases is an expert architecture and systems skill for Modular Monoliths. The focal concern is Rollout Phases, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Modular Monoliths Rollout Phases needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Modular Monoliths Rollout Phases becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Modular Monoliths Rollout Phases is currently cheap only because the bill has not arrived yet?
- How would Modular Monoliths Rollout Phases be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Modular Monoliths Rollout Phases too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Modular Monoliths Rollout Phases.
...
- [ ] The key boundaries inside Modular Monoliths Rollout Phases are explicit and justified.
