---
name: Spark Query Profiling
trigger: spark query profiling, help with spark query profiling, plan spark query profiling, improve spark query profiling, expert spark query profiling
description: Expert-level guidance for spark query profiling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Spark Query Profiling is an expert data-systems skill for Spark. The focal concern is Query Profiling, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Spark Query Profiling affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Spark Query Profiling create if two teams interpreted it differently?
- Which recovery path for Spark Query Profiling is the least understood today?
- How will Spark Query Profiling behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Spark Query Profiling is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Spark Query Profiling drifted subtly?
...
- A semantic contract note for Spark Query Profiling.
...
- [ ] The meaning of Spark Query Profiling is written in language consumers will actually use.
