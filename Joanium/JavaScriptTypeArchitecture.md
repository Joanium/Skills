---
name: JavaScript Type Architecture
trigger: javascript type architecture, help with javascript type architecture, plan javascript type architecture, improve javascript type architecture, expert javascript type architecture
description: Expert-level guidance for javascript type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

JavaScript Type Architecture is an expert programming-language skill for JavaScript. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when JavaScript Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside JavaScript Type Architecture?
- Which abstraction in JavaScript Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in JavaScript Type Architecture first?
- How will JavaScript Type Architecture behave when multiple contributors evolve it independently?
- Which part of JavaScript Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for JavaScript Type Architecture.
- A failure-mode note that explains the sharp edges inside JavaScript Type Architecture.
...
- [ ] The main language-level risk in JavaScript Type Architecture is named explicitly.
...
- [ ] Future contributors can extend JavaScript Type Architecture without reverse-engineering hidden contracts.
