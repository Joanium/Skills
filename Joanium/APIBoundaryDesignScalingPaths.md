---
name: API Boundary Design Scaling Paths
trigger: api boundary design scaling paths, help with api boundary design scaling paths, plan api boundary design scaling paths, improve api boundary design scaling paths, expert api boundary design scaling paths
description: Expert-level guidance for api boundary design scaling paths with action-oriented workflow, review criteria, and failure patterns.
---

API Boundary Design Scaling Paths is an expert architecture and systems skill for API Boundary Design. The focal concern is Scaling Paths, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when API Boundary Design Scaling Paths needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of API Boundary Design Scaling Paths becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in API Boundary Design Scaling Paths is currently cheap only because the bill has not arrived yet?
- How would API Boundary Design Scaling Paths be migrated if the current assumption proved wrong in six months?
- Where is ownership inside API Boundary Design Scaling Paths too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for API Boundary Design Scaling Paths.
...
- [ ] The key boundaries inside API Boundary Design Scaling Paths are explicit and justified.
