---
name: Repository Audit Trace Isolation
trigger: repository audit trace isolation, help with repository audit trace isolation, plan repository audit trace isolation, improve repository audit trace isolation, expert repository audit trace isolation
description: Expert-level guidance for repository audit trace isolation with action-oriented workflow, review criteria, and failure patterns.
---

Repository Audit Trace Isolation is an expert coding workflow skill for Repository Audit. The focal concern is Trace Isolation, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Repository Audit Trace Isolation is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Repository Audit Trace Isolation is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Repository Audit Trace Isolation would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Repository Audit Trace Isolation create a silent regression that tests may miss?
- How would the AI explain the safety of Repository Audit Trace Isolation to a skeptical reviewer?
...
- A concise evidence note for Repository Audit Trace Isolation.
...
- [ ] The evidence supporting Repository Audit Trace Isolation is stronger than the assumptions.
