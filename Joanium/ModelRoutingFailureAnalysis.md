---
name: Model Routing Failure Analysis
trigger: model routing failure analysis, help with model routing failure analysis, plan model routing failure analysis, improve model routing failure analysis, expert model routing failure analysis
description: Expert-level guidance for model routing failure analysis with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Model Routing Failure Analysis is an expert applied-AI skill for Model Routing. The focal concern is Failure Analysis, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Model Routing Failure Analysis affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Model Routing Failure Analysis would look acceptable in a demo but fail under production ambiguity?
- Which part of Model Routing Failure Analysis lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Model Routing Failure Analysis fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Model Routing Failure Analysis is getting better rather than merely changing?
- Where should humans intervene in Model Routing Failure Analysis to add leverage instead of toil?
...
- A behavior contract for Model Routing Failure Analysis.
...
- [ ] The behavior contract for Model Routing Failure Analysis is explicit.
