---
name: C++ Interop Boundaries
trigger: c++ interop boundaries, help with c++ interop boundaries, plan c++ interop boundaries, improve c++ interop boundaries, expert c++ interop boundaries
description: Expert-level guidance for c++ interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

C++ Interop Boundaries is an expert programming-language skill for C++. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when C++ Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside C++ Interop Boundaries?
- Which abstraction in C++ Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in C++ Interop Boundaries first?
- How will C++ Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of C++ Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for C++ Interop Boundaries.
- A failure-mode note that explains the sharp edges inside C++ Interop Boundaries.
...
- [ ] The main language-level risk in C++ Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend C++ Interop Boundaries without reverse-engineering hidden contracts.
