---
name: Failure Injection Lab Fixture Hygiene
trigger: failure injection lab fixture hygiene, help with failure injection lab fixture hygiene, plan failure injection lab fixture hygiene, improve failure injection lab fixture hygiene, expert failure injection lab fixture hygiene
description: Expert-level guidance for failure injection lab fixture hygiene with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Fixture Hygiene is an expert testing and quality skill for Failure Injection Lab. The focal concern is Fixture Hygiene, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Fixture Hygiene needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Fixture Hygiene is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Fixture Hygiene would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Fixture Hygiene depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Fixture Hygiene need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Fixture Hygiene.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Fixture Hygiene should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Fixture Hygiene is explicit.
