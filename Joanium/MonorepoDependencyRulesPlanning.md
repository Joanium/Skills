---
name: Monorepo Dependency Rules Planning
trigger: monorepo dependency rules planning, help with monorepo dependency rules planning, plan monorepo dependency rules planning, improve monorepo dependency rules planning, expert monorepo dependency rules planning
description: Expert-level guidance for monorepo dependency rules planning, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Monorepo Dependency Rules Planning is an expert-level engineering systems skill for making architecture, standards, and change management explicit enough to scale beyond individual memory. The emphasis here is sequencing work, dependencies, and proof points so execution moves quickly without creating blind risk.

## When To Use

- Use this when Monorepo Dependency Rules Planning affects codebase shape, team interfaces, or long-term delivery speed.
...
- What must be true before Monorepo Dependency Rules Planning can start safely?
...
- A milestone plan for Monorepo Dependency Rules Planning with explicit gating criteria.
