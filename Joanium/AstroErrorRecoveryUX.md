---
name: Astro Error Recovery UX
trigger: astro error recovery ux, help with astro error recovery ux, plan astro error recovery ux, improve astro error recovery ux, expert astro error recovery ux
description: Expert-level guidance for astro error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Astro Error Recovery UX is an expert frontend skill for Astro. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Astro Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Astro Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in Astro Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Astro Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does Astro Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Astro Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for Astro Error Recovery UX.
...
- [ ] The state model for Astro Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend Astro Error Recovery UX without guessing hidden UI contracts.
