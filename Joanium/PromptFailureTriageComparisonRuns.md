---
name: Prompt Failure Triage Comparison Runs
trigger: prompt failure triage comparison runs, help with prompt failure triage comparison runs, plan prompt failure triage comparison runs, improve prompt failure triage comparison runs, expert prompt failure triage comparison runs
description: Expert-level guidance for prompt failure triage comparison runs with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Comparison Runs is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Comparison Runs, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Comparison Runs affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Comparison Runs is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Comparison Runs would be hardest to detect before it compounds?
- How would Prompt Failure Triage Comparison Runs behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Comparison Runs?
...
- A contract and failure-label set for Prompt Failure Triage Comparison Runs.
...
- [ ] The system behind Prompt Failure Triage Comparison Runs is described more clearly than the prompt surface.
