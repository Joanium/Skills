---
name: Permission Prompt Triage Retry Windows
trigger: permission prompt triage retry windows, help with permission prompt triage retry windows, plan permission prompt triage retry windows, improve permission prompt triage retry windows, expert permission prompt triage retry windows
description: Expert-level guidance for permission prompt triage retry windows with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage Retry Windows is an expert browser automation skill for Permission Prompt Triage. The focal concern is Retry Windows, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage Retry Windows needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage Retry Windows unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage Retry Windows is least stable under real conditions?
- How would Permission Prompt Triage Retry Windows behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage Retry Windows capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage Retry Windows is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage Retry Windows.
...
- [ ] The state model behind Permission Prompt Triage Retry Windows is explicit.
