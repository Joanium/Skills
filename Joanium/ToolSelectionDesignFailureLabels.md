---
name: Tool Selection Design Failure Labels
trigger: tool selection design failure labels, help with tool selection design failure labels, plan tool selection design failure labels, improve tool selection design failure labels, expert tool selection design failure labels
description: Expert-level guidance for tool selection design failure labels with action-oriented workflow, review criteria, and failure patterns.
---

Tool Selection Design Failure Labels is an expert agent and tooling skill for Tool Selection Design. The focal concern is Failure Labels, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Tool Selection Design Failure Labels affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Tool Selection Design Failure Labels is still governed by intuition instead of a contract or label set?
- Which failure in Tool Selection Design Failure Labels would be hardest to detect before it compounds?
- How would Tool Selection Design Failure Labels behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Tool Selection Design Failure Labels?
...
- A contract and failure-label set for Tool Selection Design Failure Labels.
...
- [ ] The system behind Tool Selection Design Failure Labels is described more clearly than the prompt surface.
