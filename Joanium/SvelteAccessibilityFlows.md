---
name: Svelte Accessibility Flows
trigger: svelte accessibility flows, help with svelte accessibility flows, plan svelte accessibility flows, improve svelte accessibility flows, expert svelte accessibility flows
description: Expert-level guidance for svelte accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte Accessibility Flows is an expert frontend skill for Svelte. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in Svelte Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does Svelte Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte Accessibility Flows.
...
- [ ] The state model for Svelte Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend Svelte Accessibility Flows without guessing hidden UI contracts.
