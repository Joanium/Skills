---
name: RAG Systems Task Contracts
trigger: rag systems task contracts, help with rag systems task contracts, plan rag systems task contracts, improve rag systems task contracts, expert rag systems task contracts
description: Expert-level guidance for rag systems task contracts with action-oriented workflow, review criteria, and failure patterns.
---

RAG Systems Task Contracts is an expert applied AI skill for RAG Systems. The focal concern is Task Contracts, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when RAG Systems Task Contracts affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of RAG Systems Task Contracts is currently measured by vibes rather than a real contract?
- Which failure in RAG Systems Task Contracts would be hardest to notice before it harms users or operators?
- How will RAG Systems Task Contracts behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside RAG Systems Task Contracts?
...
- A behavior contract for RAG Systems Task Contracts.
...
- [ ] The contract behind RAG Systems Task Contracts is explicit enough to evaluate.
