---
name: SolidJS Build Pipelines
trigger: solidjs build pipelines, help with solidjs build pipelines, plan solidjs build pipelines, improve solidjs build pipelines, expert solidjs build pipelines
description: Expert-level guidance for solidjs build pipelines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

SolidJS Build Pipelines is an expert frontend skill for SolidJS. The focal concern is Build Pipelines, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when SolidJS Build Pipelines affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does SolidJS Build Pipelines make user state harder to reason about than it needs to be?
- Which interaction in SolidJS Build Pipelines degrades first under latency, stale data, or partial failure?
- What accessibility behavior in SolidJS Build Pipelines would break if the DOM or state changed unexpectedly?
- How does SolidJS Build Pipelines fail when routing, hydration, or rendering order becomes non-ideal?
- What part of SolidJS Build Pipelines looks correct in a mock but collapses under production behavior?
...
- A state transition map for SolidJS Build Pipelines.
...
- [ ] The state model for SolidJS Build Pipelines is clear enough to debug.
...
- [ ] Another engineer can extend SolidJS Build Pipelines without guessing hidden UI contracts.
