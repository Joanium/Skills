---
name: Tool Failure Recovery Playbook
trigger: tool failure recovery playbook, help with tool failure recovery playbook, plan tool failure recovery playbook, improve tool failure recovery playbook, expert tool failure recovery playbook
description: Expert-level guidance for tool failure recovery playbook, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Tool Failure Recovery Playbook is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is response execution: who does what, in what order, with what evidence, when conditions are degraded.

## When To Use

- Use this when Tool Failure Recovery Playbook influences user-visible output, automation, or safety-sensitive decisions.
...
- What threshold or trigger causes Tool Failure Recovery Playbook to move from monitoring to active response?
...
- A trigger table for Tool Failure Recovery Playbook.
