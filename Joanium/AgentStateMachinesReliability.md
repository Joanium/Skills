---
name: Agent State Machines Reliability
trigger: agent state machines reliability, help with agent state machines reliability, plan agent state machines reliability, improve agent state machines reliability, expert agent state machines reliability
description: Expert-level guidance for agent state machines reliability, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Agent State Machines Reliability is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is designing graceful degradation, recovery paths, and explicit reliability targets before the system is stressed.

## When To Use

- Use this when Agent State Machines Reliability influences user-visible output, automation, or safety-sensitive decisions.
...
- What are the most likely and most damaging failure modes in Agent State Machines Reliability?
...
- A failure mode catalog for Agent State Machines Reliability.
