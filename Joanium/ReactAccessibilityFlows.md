---
name: React Accessibility Flows
trigger: react accessibility flows, help with react accessibility flows, plan react accessibility flows, improve react accessibility flows, expert react accessibility flows
description: Expert-level guidance for react accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

React Accessibility Flows is an expert frontend skill for React. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when React Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does React Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in React Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in React Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does React Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of React Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for React Accessibility Flows.
...
- [ ] The state model for React Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend React Accessibility Flows without guessing hidden UI contracts.
