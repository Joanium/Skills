---
name: Model Routing Evaluation Rubrics
trigger: model routing evaluation rubrics, help with model routing evaluation rubrics, plan model routing evaluation rubrics, improve model routing evaluation rubrics, expert model routing evaluation rubrics
description: Expert-level guidance for model routing evaluation rubrics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Model Routing Evaluation Rubrics is an expert applied-AI skill for Model Routing. The focal concern is Evaluation Rubrics, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Model Routing Evaluation Rubrics affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Model Routing Evaluation Rubrics would look acceptable in a demo but fail under production ambiguity?
- Which part of Model Routing Evaluation Rubrics lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Model Routing Evaluation Rubrics fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Model Routing Evaluation Rubrics is getting better rather than merely changing?
- Where should humans intervene in Model Routing Evaluation Rubrics to add leverage instead of toil?
...
- A behavior contract for Model Routing Evaluation Rubrics.
...
- [ ] The behavior contract for Model Routing Evaluation Rubrics is explicit.
