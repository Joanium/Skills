---
name: Monorepo Dependency Rules Strategy
trigger: monorepo dependency rules strategy, help with monorepo dependency rules strategy, plan monorepo dependency rules strategy, improve monorepo dependency rules strategy, expert monorepo dependency rules strategy
description: Expert-level guidance for monorepo dependency rules strategy, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Monorepo Dependency Rules Strategy is an expert-level engineering systems skill for making architecture, standards, and change management explicit enough to scale beyond individual memory. The emphasis here is on choosing the operating model, decision boundaries, and success criteria before local optimizations become policy by accident.

## When To Use

- Use this when Monorepo Dependency Rules Strategy affects codebase shape, team interfaces, or long-term delivery speed.
...
- What decision is Monorepo Dependency Rules Strategy supposed to improve over the next planning horizon?
- Which constraints are fixed, and which are merely habits that can be challenged?
- Where does failure in Monorepo Dependency Rules Strategy create the largest technical, business, or trust cost?
...
- A written decision statement for Monorepo Dependency Rules Strategy.
