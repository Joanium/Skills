---
name: Retrieval Quality Operations Trace Review
trigger: retrieval quality operations trace review, help with retrieval quality operations trace review, plan retrieval quality operations trace review, improve retrieval quality operations trace review, expert retrieval quality operations trace review
description: Expert-level guidance for retrieval quality operations trace review with action-oriented workflow, review criteria, and failure patterns.
---

Retrieval Quality Operations Trace Review is an expert agent and tooling skill for Retrieval Quality Operations. The focal concern is Trace Review, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Retrieval Quality Operations Trace Review affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Retrieval Quality Operations Trace Review is still governed by intuition instead of a contract or label set?
- Which failure in Retrieval Quality Operations Trace Review would be hardest to detect before it compounds?
- How would Retrieval Quality Operations Trace Review behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Retrieval Quality Operations Trace Review?
...
- A contract and failure-label set for Retrieval Quality Operations Trace Review.
...
- [ ] The system behind Retrieval Quality Operations Trace Review is described more clearly than the prompt surface.
