---
name: gRPC Contract Evolution
trigger: grpc contract evolution, help with grpc contract evolution, plan grpc contract evolution, improve grpc contract evolution, expert grpc contract evolution
description: Expert-level guidance for grpc contract evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

gRPC Contract Evolution is an expert backend skill for gRPC. The focal concern is Contract Evolution, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when gRPC Contract Evolution shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside gRPC Contract Evolution breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in gRPC Contract Evolution?
- Where does gRPC Contract Evolution need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether gRPC Contract Evolution is unhealthy before customers do?
- What compatibility promise is implied by gRPC Contract Evolution even if it was never written down?
...
- A contract note for gRPC Contract Evolution that includes failure semantics.
...
- [ ] The contract for gRPC Contract Evolution is explicit about success and failure behavior.
