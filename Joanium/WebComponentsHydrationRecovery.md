---
name: Web Components Hydration Recovery
trigger: web components hydration recovery, help with web components hydration recovery, plan web components hydration recovery, improve web components hydration recovery, expert web components hydration recovery
description: Expert-level guidance for web components hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Web Components Hydration Recovery is an expert frontend skill for Web Components. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Web Components Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Web Components Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in Web Components Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Web Components Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does Web Components Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Web Components Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for Web Components Hydration Recovery.
...
- [ ] The state model for Web Components Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend Web Components Hydration Recovery without guessing hidden UI contracts.
