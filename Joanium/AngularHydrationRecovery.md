---
name: Angular Hydration Recovery
trigger: angular hydration recovery, help with angular hydration recovery, plan angular hydration recovery, improve angular hydration recovery, expert angular hydration recovery
description: Expert-level guidance for angular hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Angular Hydration Recovery is an expert frontend skill for Angular. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Angular Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Angular Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in Angular Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Angular Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does Angular Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Angular Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for Angular Hydration Recovery.
...
- [ ] The state model for Angular Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend Angular Hydration Recovery without guessing hidden UI contracts.
