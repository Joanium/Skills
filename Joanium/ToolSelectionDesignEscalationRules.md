---
name: Tool Selection Design Escalation Rules
trigger: tool selection design escalation rules, help with tool selection design escalation rules, plan tool selection design escalation rules, improve tool selection design escalation rules, expert tool selection design escalation rules
description: Expert-level guidance for tool selection design escalation rules with action-oriented workflow, review criteria, and failure patterns.
---

Tool Selection Design Escalation Rules is an expert agent and tooling skill for Tool Selection Design. The focal concern is Escalation Rules, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Tool Selection Design Escalation Rules affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Tool Selection Design Escalation Rules is still governed by intuition instead of a contract or label set?
- Which failure in Tool Selection Design Escalation Rules would be hardest to detect before it compounds?
- How would Tool Selection Design Escalation Rules behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Tool Selection Design Escalation Rules?
...
- A contract and failure-label set for Tool Selection Design Escalation Rules.
...
- [ ] The system behind Tool Selection Design Escalation Rules is described more clearly than the prompt surface.
