---
name: Modular Monoliths Scaling Paths
trigger: modular monoliths scaling paths, help with modular monoliths scaling paths, plan modular monoliths scaling paths, improve modular monoliths scaling paths, expert modular monoliths scaling paths
description: Expert-level guidance for modular monoliths scaling paths with action-oriented workflow, review criteria, and failure patterns.
---

Modular Monoliths Scaling Paths is an expert architecture and systems skill for Modular Monoliths. The focal concern is Scaling Paths, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Modular Monoliths Scaling Paths needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Modular Monoliths Scaling Paths becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Modular Monoliths Scaling Paths is currently cheap only because the bill has not arrived yet?
- How would Modular Monoliths Scaling Paths be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Modular Monoliths Scaling Paths too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Modular Monoliths Scaling Paths.
...
- [ ] The key boundaries inside Modular Monoliths Scaling Paths are explicit and justified.
