---
name: Bug Reproduction Regression Guards
trigger: bug reproduction regression guards, help with bug reproduction regression guards, plan bug reproduction regression guards, improve bug reproduction regression guards, expert bug reproduction regression guards
description: Expert-level guidance for bug reproduction regression guards with action-oriented workflow, review criteria, and failure patterns.
---

Bug Reproduction Regression Guards is an expert coding workflow skill for Bug Reproduction. The focal concern is Regression Guards, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Bug Reproduction Regression Guards is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Bug Reproduction Regression Guards is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Bug Reproduction Regression Guards would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Bug Reproduction Regression Guards create a silent regression that tests may miss?
- How would the AI explain the safety of Bug Reproduction Regression Guards to a skeptical reviewer?
...
- A concise evidence note for Bug Reproduction Regression Guards.
...
- [ ] The evidence supporting Bug Reproduction Regression Guards is stronger than the assumptions.
