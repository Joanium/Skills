---
name: Technical Debt Burn Down Approval Heuristics
trigger: technical debt burn down approval heuristics, help with technical debt burn down approval heuristics, plan technical debt burn down approval heuristics, improve technical debt burn down approval heuristics, expert technical debt burn down approval heuristics
description: Expert-level guidance for technical debt burn down approval heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Technical Debt Burn Down Approval Heuristics is an expert repository and delivery skill for Technical Debt Burn Down. The focal concern is Approval Heuristics, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Technical Debt Burn Down Approval Heuristics determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Technical Debt Burn Down Approval Heuristics riskier than the diff itself suggests?
- Which reviewer would challenge Technical Debt Burn Down Approval Heuristics first, and what evidence would they need?
- How could Technical Debt Burn Down Approval Heuristics fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Technical Debt Burn Down Approval Heuristics would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Technical Debt Burn Down Approval Heuristics concretely?
...
- A blast-radius note for Technical Debt Burn Down Approval Heuristics.
...
- [ ] The repository-level blast radius of Technical Debt Burn Down Approval Heuristics is explicit.
