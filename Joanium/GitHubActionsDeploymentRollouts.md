---
name: GitHub Actions Deployment Rollouts
trigger: github actions deployment rollouts, help with github actions deployment rollouts, plan github actions deployment rollouts, improve github actions deployment rollouts, expert github actions deployment rollouts
description: Expert-level guidance for github actions deployment rollouts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

GitHub Actions Deployment Rollouts is an expert cloud and platform skill for GitHub Actions. The focal concern is Deployment Rollouts, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when GitHub Actions Deployment Rollouts changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside GitHub Actions Deployment Rollouts would fail first during a rushed rollout or incident?
- Which control in GitHub Actions Deployment Rollouts is automated but not truly reversible?
- How will operators know whether GitHub Actions Deployment Rollouts is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in GitHub Actions Deployment Rollouts is invisible until it becomes expensive?
- Which dependency makes GitHub Actions Deployment Rollouts less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for GitHub Actions Deployment Rollouts.
...
- [ ] The rollback path for GitHub Actions Deployment Rollouts is explicit and tested.
