---
name: Permission Prompt Triage State Reset
trigger: permission prompt triage state reset, help with permission prompt triage state reset, plan permission prompt triage state reset, improve permission prompt triage state reset, expert permission prompt triage state reset
description: Expert-level guidance for permission prompt triage state reset with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage State Reset is an expert browser automation skill for Permission Prompt Triage. The focal concern is State Reset, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage State Reset needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage State Reset unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage State Reset is least stable under real conditions?
- How would Permission Prompt Triage State Reset behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage State Reset capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage State Reset is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage State Reset.
...
- [ ] The state model behind Permission Prompt Triage State Reset is explicit.
