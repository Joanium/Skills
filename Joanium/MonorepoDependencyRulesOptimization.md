---
name: Monorepo Dependency Rules Optimization
trigger: monorepo dependency rules optimization, help with monorepo dependency rules optimization, plan monorepo dependency rules optimization, improve monorepo dependency rules optimization, expert monorepo dependency rules optimization
description: Expert-level guidance for monorepo dependency rules optimization, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Monorepo Dependency Rules Optimization is an expert-level engineering systems skill for making architecture, standards, and change management explicit enough to scale beyond individual memory. The emphasis here is improving the limiting resource without merely moving cost, fragility, or risk somewhere else.

## When To Use

- Use this when Monorepo Dependency Rules Optimization affects codebase shape, team interfaces, or long-term delivery speed.
...
- What is the actual bottleneck in Monorepo Dependency Rules Optimization: CPU, memory, latency, network, human time, or policy friction?
...
- A baseline report for Monorepo Dependency Rules Optimization with representative workloads.
