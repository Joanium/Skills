---
name: ASP.NET Core Observability Contracts
trigger: asp.net core observability contracts, help with asp.net core observability contracts, plan asp.net core observability contracts, improve asp.net core observability contracts, expert asp.net core observability contracts
description: Expert-level guidance for asp.net core observability contracts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

ASP.NET Core Observability Contracts is an expert backend skill for ASP.NET Core. The focal concern is Observability Contracts, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when ASP.NET Core Observability Contracts shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside ASP.NET Core Observability Contracts breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in ASP.NET Core Observability Contracts?
- Where does ASP.NET Core Observability Contracts need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether ASP.NET Core Observability Contracts is unhealthy before customers do?
- What compatibility promise is implied by ASP.NET Core Observability Contracts even if it was never written down?
...
- A contract note for ASP.NET Core Observability Contracts that includes failure semantics.
...
- [ ] The contract for ASP.NET Core Observability Contracts is explicit about success and failure behavior.
