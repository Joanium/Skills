---
name: API Incident Triage Contract Proofs
trigger: api incident triage contract proofs, help with api incident triage contract proofs, plan api incident triage contract proofs, improve api incident triage contract proofs, expert api incident triage contract proofs
description: Expert-level guidance for api incident triage contract proofs with action-oriented workflow, review criteria, and failure patterns.
---

API Incident Triage Contract Proofs is an expert backend execution skill for API Incident Triage. The focal concern is Contract Proofs, but the real objective is helping an AI design service behavior that stays coherent under retries, queues, partial failure, and operational pressure.

## When To Use

- Use this when API Incident Triage Contract Proofs affects whether backend changes remain safe once real production behavior kicks in.
...
- Which failure ordering inside API Incident Triage Contract Proofs would create the most misleading symptom?
- Where does API Incident Triage Contract Proofs need stronger idempotency or retry discipline?
- What dependency in API Incident Triage Contract Proofs is currently hiding behind timeout or queue noise?
- How would an operator explain the health of API Incident Triage Contract Proofs in the first five minutes of an incident?
- What contract assumption in API Incident Triage Contract Proofs is least defensible under partial failure?
...
- A failure-sequence note for API Incident Triage Contract Proofs.
...
- [ ] The real behavior of API Incident Triage Contract Proofs under retries and failures is explicit.
