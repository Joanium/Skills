---
name: Tool Selection Design Task Contracts
trigger: tool selection design task contracts, help with tool selection design task contracts, plan tool selection design task contracts, improve tool selection design task contracts, expert tool selection design task contracts
description: Expert-level guidance for tool selection design task contracts with action-oriented workflow, review criteria, and failure patterns.
---

Tool Selection Design Task Contracts is an expert agent and tooling skill for Tool Selection Design. The focal concern is Task Contracts, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Tool Selection Design Task Contracts affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Tool Selection Design Task Contracts is still governed by intuition instead of a contract or label set?
- Which failure in Tool Selection Design Task Contracts would be hardest to detect before it compounds?
- How would Tool Selection Design Task Contracts behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Tool Selection Design Task Contracts?
...
- A contract and failure-label set for Tool Selection Design Task Contracts.
...
- [ ] The system behind Tool Selection Design Task Contracts is described more clearly than the prompt surface.
