---
name: Computer Vision Failure Analysis
trigger: computer vision failure analysis, help with computer vision failure analysis, plan computer vision failure analysis, improve computer vision failure analysis, expert computer vision failure analysis
description: Expert-level guidance for computer vision failure analysis with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Computer Vision Failure Analysis is an expert systems skill for Computer Vision. The focal concern is Failure Analysis, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Computer Vision Failure Analysis is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Computer Vision Failure Analysis would break first under scale, timing, or hardware stress?
- Which state or resource in Computer Vision Failure Analysis is hardest to observe when behavior goes wrong?
- How does Computer Vision Failure Analysis trade correctness, throughput, and recoverability today?
- What tooling gap makes Computer Vision Failure Analysis harder to reason about than it should be?
- Which optimization target inside Computer Vision Failure Analysis is attractive but strategically secondary?
...
- A system model for Computer Vision Failure Analysis that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Computer Vision Failure Analysis is explicit.
