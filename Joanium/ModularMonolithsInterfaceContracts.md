---
name: Modular Monoliths Interface Contracts
trigger: modular monoliths interface contracts, help with modular monoliths interface contracts, plan modular monoliths interface contracts, improve modular monoliths interface contracts, expert modular monoliths interface contracts
description: Expert-level guidance for modular monoliths interface contracts with action-oriented workflow, review criteria, and failure patterns.
---

Modular Monoliths Interface Contracts is an expert architecture and systems skill for Modular Monoliths. The focal concern is Interface Contracts, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Modular Monoliths Interface Contracts needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Modular Monoliths Interface Contracts becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Modular Monoliths Interface Contracts is currently cheap only because the bill has not arrived yet?
- How would Modular Monoliths Interface Contracts be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Modular Monoliths Interface Contracts too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Modular Monoliths Interface Contracts.
...
- [ ] The key boundaries inside Modular Monoliths Interface Contracts are explicit and justified.
