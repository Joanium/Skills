---
name: Patch Backporting Reviewer Alignment
trigger: patch backporting reviewer alignment, help with patch backporting reviewer alignment, plan patch backporting reviewer alignment, improve patch backporting reviewer alignment, expert patch backporting reviewer alignment
description: Expert-level guidance for patch backporting reviewer alignment with action-oriented workflow, review criteria, and failure patterns.
---

Patch Backporting Reviewer Alignment is an expert repository and delivery skill for Patch Backporting. The focal concern is Reviewer Alignment, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Patch Backporting Reviewer Alignment determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Patch Backporting Reviewer Alignment riskier than the diff itself suggests?
- Which reviewer would challenge Patch Backporting Reviewer Alignment first, and what evidence would they need?
- How could Patch Backporting Reviewer Alignment fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Patch Backporting Reviewer Alignment would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Patch Backporting Reviewer Alignment concretely?
...
- A blast-radius note for Patch Backporting Reviewer Alignment.
...
- [ ] The repository-level blast radius of Patch Backporting Reviewer Alignment is explicit.
