---
name: Bug Reproduction Change Scoping
trigger: bug reproduction change scoping, help with bug reproduction change scoping, plan bug reproduction change scoping, improve bug reproduction change scoping, expert bug reproduction change scoping
description: Expert-level guidance for bug reproduction change scoping with action-oriented workflow, review criteria, and failure patterns.
---

Bug Reproduction Change Scoping is an expert coding workflow skill for Bug Reproduction. The focal concern is Change Scoping, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Bug Reproduction Change Scoping is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Bug Reproduction Change Scoping is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Bug Reproduction Change Scoping would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Bug Reproduction Change Scoping create a silent regression that tests may miss?
- How would the AI explain the safety of Bug Reproduction Change Scoping to a skeptical reviewer?
...
- A concise evidence note for Bug Reproduction Change Scoping.
...
- [ ] The evidence supporting Bug Reproduction Change Scoping is stronger than the assumptions.
