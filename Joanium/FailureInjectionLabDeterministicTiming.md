---
name: Failure Injection Lab Deterministic Timing
trigger: failure injection lab deterministic timing, help with failure injection lab deterministic timing, plan failure injection lab deterministic timing, improve failure injection lab deterministic timing, expert failure injection lab deterministic timing
description: Expert-level guidance for failure injection lab deterministic timing with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Deterministic Timing is an expert testing and quality skill for Failure Injection Lab. The focal concern is Deterministic Timing, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Deterministic Timing needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Deterministic Timing is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Deterministic Timing would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Deterministic Timing depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Deterministic Timing need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Deterministic Timing.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Deterministic Timing should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Deterministic Timing is explicit.
