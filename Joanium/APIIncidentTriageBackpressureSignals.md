---
name: API Incident Triage Backpressure Signals
trigger: api incident triage backpressure signals, help with api incident triage backpressure signals, plan api incident triage backpressure signals, improve api incident triage backpressure signals, expert api incident triage backpressure signals
description: Expert-level guidance for api incident triage backpressure signals with action-oriented workflow, review criteria, and failure patterns.
---

API Incident Triage Backpressure Signals is an expert backend execution skill for API Incident Triage. The focal concern is Backpressure Signals, but the real objective is helping an AI design service behavior that stays coherent under retries, queues, partial failure, and operational pressure.

## When To Use

- Use this when API Incident Triage Backpressure Signals affects whether backend changes remain safe once real production behavior kicks in.
...
- Which failure ordering inside API Incident Triage Backpressure Signals would create the most misleading symptom?
- Where does API Incident Triage Backpressure Signals need stronger idempotency or retry discipline?
- What dependency in API Incident Triage Backpressure Signals is currently hiding behind timeout or queue noise?
- How would an operator explain the health of API Incident Triage Backpressure Signals in the first five minutes of an incident?
- What contract assumption in API Incident Triage Backpressure Signals is least defensible under partial failure?
...
- A failure-sequence note for API Incident Triage Backpressure Signals.
...
- [ ] The real behavior of API Incident Triage Backpressure Signals under retries and failures is explicit.
