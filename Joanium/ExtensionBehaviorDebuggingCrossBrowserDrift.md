---
name: Extension Behavior Debugging Cross-Browser Drift
trigger: extension behavior debugging cross-browser drift, help with extension behavior debugging cross-browser drift, plan extension behavior debugging cross-browser drift, improve extension behavior debugging cross-browser drift, expert extension behavior debugging cross-browser drift
description: Expert-level guidance for extension behavior debugging cross-browser drift with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Cross-Browser Drift is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Cross-Browser Drift, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Cross-Browser Drift needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Cross-Browser Drift unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Cross-Browser Drift is least stable under real conditions?
- How would Extension Behavior Debugging Cross-Browser Drift behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Cross-Browser Drift capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Cross-Browser Drift is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Cross-Browser Drift.
...
- [ ] The state model behind Extension Behavior Debugging Cross-Browser Drift is explicit.
