---
name: Visual Diff Triage Cross-Browser Drift
trigger: visual diff triage cross-browser drift, help with visual diff triage cross-browser drift, plan visual diff triage cross-browser drift, improve visual diff triage cross-browser drift, expert visual diff triage cross-browser drift
description: Expert-level guidance for visual diff triage cross-browser drift with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage Cross-Browser Drift is an expert browser automation skill for Visual Diff Triage. The focal concern is Cross-Browser Drift, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage Cross-Browser Drift needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage Cross-Browser Drift unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage Cross-Browser Drift is least stable under real conditions?
- How would Visual Diff Triage Cross-Browser Drift behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage Cross-Browser Drift capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage Cross-Browser Drift is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage Cross-Browser Drift.
...
- [ ] The state model behind Visual Diff Triage Cross-Browser Drift is explicit.
