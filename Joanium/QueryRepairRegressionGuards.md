---
name: Query Repair Regression Guards
trigger: query repair regression guards, help with query repair regression guards, plan query repair regression guards, improve query repair regression guards, expert query repair regression guards
description: Expert-level guidance for query repair regression guards with action-oriented workflow, review criteria, and failure patterns.
---

Query Repair Regression Guards is an expert coding workflow skill for Query Repair. The focal concern is Regression Guards, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Query Repair Regression Guards is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Query Repair Regression Guards is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Query Repair Regression Guards would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Query Repair Regression Guards create a silent regression that tests may miss?
- How would the AI explain the safety of Query Repair Regression Guards to a skeptical reviewer?
...
- A concise evidence note for Query Repair Regression Guards.
...
- [ ] The evidence supporting Query Repair Regression Guards is stronger than the assumptions.
