---
name: Nuxt Accessibility Flows
trigger: nuxt accessibility flows, help with nuxt accessibility flows, plan nuxt accessibility flows, improve nuxt accessibility flows, expert nuxt accessibility flows
description: Expert-level guidance for nuxt accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Nuxt Accessibility Flows is an expert frontend skill for Nuxt. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Nuxt Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Nuxt Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in Nuxt Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Nuxt Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does Nuxt Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Nuxt Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for Nuxt Accessibility Flows.
...
- [ ] The state model for Nuxt Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend Nuxt Accessibility Flows without guessing hidden UI contracts.
