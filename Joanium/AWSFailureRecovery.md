---
name: AWS Failure Recovery
trigger: aws failure recovery, help with aws failure recovery, plan aws failure recovery, improve aws failure recovery, expert aws failure recovery
description: Expert-level guidance for aws failure recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

AWS Failure Recovery is an expert cloud and platform skill for AWS. The focal concern is Failure Recovery, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when AWS Failure Recovery changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside AWS Failure Recovery would fail first during a rushed rollout or incident?
- Which control in AWS Failure Recovery is automated but not truly reversible?
- How will operators know whether AWS Failure Recovery is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in AWS Failure Recovery is invisible until it becomes expensive?
- Which dependency makes AWS Failure Recovery less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for AWS Failure Recovery.
...
- [ ] The rollback path for AWS Failure Recovery is explicit and tested.
