---
name: RAG Data Contracts
trigger: rag data contracts, help with rag data contracts, plan rag data contracts, improve rag data contracts, expert rag data contracts
description: Expert-level guidance for rag data contracts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

RAG Data Contracts is an expert applied-AI skill for RAG. The focal concern is Data Contracts, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when RAG Data Contracts affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in RAG Data Contracts would look acceptable in a demo but fail under production ambiguity?
- Which part of RAG Data Contracts lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will RAG Data Contracts fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that RAG Data Contracts is getting better rather than merely changing?
- Where should humans intervene in RAG Data Contracts to add leverage instead of toil?
...
- A behavior contract for RAG Data Contracts.
...
- [ ] The behavior contract for RAG Data Contracts is explicit.
