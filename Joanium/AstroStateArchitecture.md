---
name: Astro State Architecture
trigger: astro state architecture, help with astro state architecture, plan astro state architecture, improve astro state architecture, expert astro state architecture
description: Expert-level guidance for astro state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Astro State Architecture is an expert frontend skill for Astro. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Astro State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Astro State Architecture make user state harder to reason about than it needs to be?
- Which interaction in Astro State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Astro State Architecture would break if the DOM or state changed unexpectedly?
- How does Astro State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Astro State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for Astro State Architecture.
...
- [ ] The state model for Astro State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend Astro State Architecture without guessing hidden UI contracts.
