---
name: Java Error Semantics
trigger: java error semantics, help with java error semantics, plan java error semantics, improve java error semantics, expert java error semantics
description: Expert-level guidance for java error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Java Error Semantics is an expert programming-language skill for Java. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Java Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Java Error Semantics?
- Which abstraction in Java Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Java Error Semantics first?
- How will Java Error Semantics behave when multiple contributors evolve it independently?
- Which part of Java Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Java Error Semantics.
- A failure-mode note that explains the sharp edges inside Java Error Semantics.
...
- [ ] The main language-level risk in Java Error Semantics is named explicitly.
...
- [ ] Future contributors can extend Java Error Semantics without reverse-engineering hidden contracts.
