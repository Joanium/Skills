---
name: Failure Injection Lab Parallel Execution
trigger: failure injection lab parallel execution, help with failure injection lab parallel execution, plan failure injection lab parallel execution, improve failure injection lab parallel execution, expert failure injection lab parallel execution
description: Expert-level guidance for failure injection lab parallel execution with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Parallel Execution is an expert testing and quality skill for Failure Injection Lab. The focal concern is Parallel Execution, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Parallel Execution needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Parallel Execution is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Parallel Execution would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Parallel Execution depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Parallel Execution need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Parallel Execution.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Parallel Execution should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Parallel Execution is explicit.
