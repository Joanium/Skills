---
name: Threat Modeling Evidence Handling
trigger: threat modeling evidence handling, help with threat modeling evidence handling, plan threat modeling evidence handling, improve threat modeling evidence handling, expert threat modeling evidence handling
description: Expert-level guidance for threat modeling evidence handling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Threat Modeling Evidence Handling is an expert security skill for Threat Modeling. The focal concern is Evidence Handling, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when Threat Modeling Evidence Handling influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in Threat Modeling Evidence Handling first?
- Which exception or emergency path makes Threat Modeling Evidence Handling less true in practice than in policy?
- What evidence would be missing if Threat Modeling Evidence Handling failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does Threat Modeling Evidence Handling degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for Threat Modeling Evidence Handling.
...
- [ ] The attacker model for Threat Modeling Evidence Handling is realistic and current.
...
- [ ] The team can explain how Threat Modeling Evidence Handling fails and how it recovers.
