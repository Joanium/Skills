---
name: Angular State Architecture
trigger: angular state architecture, help with angular state architecture, plan angular state architecture, improve angular state architecture, expert angular state architecture
description: Expert-level guidance for angular state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Angular State Architecture is an expert frontend skill for Angular. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Angular State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Angular State Architecture make user state harder to reason about than it needs to be?
- Which interaction in Angular State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Angular State Architecture would break if the DOM or state changed unexpectedly?
- How does Angular State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Angular State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for Angular State Architecture.
...
- [ ] The state model for Angular State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend Angular State Architecture without guessing hidden UI contracts.
