---
name: NestJS Rate Limit Design
trigger: nestjs rate limit design, help with nestjs rate limit design, plan nestjs rate limit design, improve nestjs rate limit design, expert nestjs rate limit design
description: Expert-level guidance for nestjs rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

NestJS Rate Limit Design is an expert backend skill for NestJS. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when NestJS Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside NestJS Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in NestJS Rate Limit Design?
- Where does NestJS Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether NestJS Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by NestJS Rate Limit Design even if it was never written down?
...
- A contract note for NestJS Rate Limit Design that includes failure semantics.
...
- [ ] The contract for NestJS Rate Limit Design is explicit about success and failure behavior.
