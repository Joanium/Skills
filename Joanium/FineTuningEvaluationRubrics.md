---
name: Fine Tuning Evaluation Rubrics
trigger: fine tuning evaluation rubrics, help with fine tuning evaluation rubrics, plan fine tuning evaluation rubrics, improve fine tuning evaluation rubrics, expert fine tuning evaluation rubrics
description: Expert-level guidance for fine tuning evaluation rubrics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Fine Tuning Evaluation Rubrics is an expert applied-AI skill for Fine Tuning. The focal concern is Evaluation Rubrics, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Fine Tuning Evaluation Rubrics affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Fine Tuning Evaluation Rubrics would look acceptable in a demo but fail under production ambiguity?
- Which part of Fine Tuning Evaluation Rubrics lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Fine Tuning Evaluation Rubrics fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Fine Tuning Evaluation Rubrics is getting better rather than merely changing?
- Where should humans intervene in Fine Tuning Evaluation Rubrics to add leverage instead of toil?
...
- A behavior contract for Fine Tuning Evaluation Rubrics.
...
- [ ] The behavior contract for Fine Tuning Evaluation Rubrics is explicit.
