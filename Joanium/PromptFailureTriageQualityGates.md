---
name: Prompt Failure Triage Quality Gates
trigger: prompt failure triage quality gates, help with prompt failure triage quality gates, plan prompt failure triage quality gates, improve prompt failure triage quality gates, expert prompt failure triage quality gates
description: Expert-level guidance for prompt failure triage quality gates with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Quality Gates is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Quality Gates, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Quality Gates affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Quality Gates is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Quality Gates would be hardest to detect before it compounds?
- How would Prompt Failure Triage Quality Gates behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Quality Gates?
...
- A contract and failure-label set for Prompt Failure Triage Quality Gates.
...
- [ ] The system behind Prompt Failure Triage Quality Gates is described more clearly than the prompt surface.
