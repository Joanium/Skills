---
name: PostgreSQL Schema Evolution
trigger: postgresql schema evolution, help with postgresql schema evolution, plan postgresql schema evolution, improve postgresql schema evolution, expert postgresql schema evolution
description: Expert-level guidance for postgresql schema evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

PostgreSQL Schema Evolution is an expert data-systems skill for PostgreSQL. The focal concern is Schema Evolution, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when PostgreSQL Schema Evolution affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would PostgreSQL Schema Evolution create if two teams interpreted it differently?
- Which recovery path for PostgreSQL Schema Evolution is the least understood today?
- How will PostgreSQL Schema Evolution behave under backfills, reprocessing, or delayed sources?
- What cost pattern in PostgreSQL Schema Evolution is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside PostgreSQL Schema Evolution drifted subtly?
...
- A semantic contract note for PostgreSQL Schema Evolution.
...
- [ ] The meaning of PostgreSQL Schema Evolution is written in language consumers will actually use.
