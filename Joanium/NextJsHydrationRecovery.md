---
name: Next.js Hydration Recovery
trigger: next.js hydration recovery, help with next.js hydration recovery, plan next.js hydration recovery, improve next.js hydration recovery, expert next.js hydration recovery
description: Expert-level guidance for next.js hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Next.js Hydration Recovery is an expert frontend skill for Next.js. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Next.js Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Next.js Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in Next.js Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Next.js Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does Next.js Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Next.js Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for Next.js Hydration Recovery.
...
- [ ] The state model for Next.js Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend Next.js Hydration Recovery without guessing hidden UI contracts.
