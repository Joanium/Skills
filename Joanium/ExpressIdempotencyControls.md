---
name: Express Idempotency Controls
trigger: express idempotency controls, help with express idempotency controls, plan express idempotency controls, improve express idempotency controls, expert express idempotency controls
description: Expert-level guidance for express idempotency controls with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Express Idempotency Controls is an expert backend skill for Express. The focal concern is Idempotency Controls, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Express Idempotency Controls shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Express Idempotency Controls breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Express Idempotency Controls?
- Where does Express Idempotency Controls need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Express Idempotency Controls is unhealthy before customers do?
- What compatibility promise is implied by Express Idempotency Controls even if it was never written down?
...
- A contract note for Express Idempotency Controls that includes failure semantics.
...
- [ ] The contract for Express Idempotency Controls is explicit about success and failure behavior.
