---
name: Rails Rate Limit Design
trigger: rails rate limit design, help with rails rate limit design, plan rails rate limit design, improve rails rate limit design, expert rails rate limit design
description: Expert-level guidance for rails rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rails Rate Limit Design is an expert backend skill for Rails. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Rails Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Rails Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Rails Rate Limit Design?
- Where does Rails Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Rails Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by Rails Rate Limit Design even if it was never written down?
...
- A contract note for Rails Rate Limit Design that includes failure semantics.
...
- [ ] The contract for Rails Rate Limit Design is explicit about success and failure behavior.
