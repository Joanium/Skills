---
name: Failure Injection Lab Failure Matrices
trigger: failure injection lab failure matrices, help with failure injection lab failure matrices, plan failure injection lab failure matrices, improve failure injection lab failure matrices, expert failure injection lab failure matrices
description: Expert-level guidance for failure injection lab failure matrices with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Failure Matrices is an expert testing and quality skill for Failure Injection Lab. The focal concern is Failure Matrices, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Failure Matrices needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Failure Matrices is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Failure Matrices would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Failure Matrices depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Failure Matrices need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Failure Matrices.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Failure Matrices should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Failure Matrices is explicit.
