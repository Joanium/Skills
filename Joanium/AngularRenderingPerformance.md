---
name: Angular Rendering Performance
trigger: angular rendering performance, help with angular rendering performance, plan angular rendering performance, improve angular rendering performance, expert angular rendering performance
description: Expert-level guidance for angular rendering performance with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Angular Rendering Performance is an expert frontend skill for Angular. The focal concern is Rendering Performance, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Angular Rendering Performance affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Angular Rendering Performance make user state harder to reason about than it needs to be?
- Which interaction in Angular Rendering Performance degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Angular Rendering Performance would break if the DOM or state changed unexpectedly?
- How does Angular Rendering Performance fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Angular Rendering Performance looks correct in a mock but collapses under production behavior?
...
- A state transition map for Angular Rendering Performance.
...
- [ ] The state model for Angular Rendering Performance is clear enough to debug.
...
- [ ] Another engineer can extend Angular Rendering Performance without guessing hidden UI contracts.
