---
name: API Change Watch Version Diffs
trigger: api change watch version diffs, help with api change watch version diffs, plan api change watch version diffs, improve api change watch version diffs, expert api change watch version diffs
description: Expert-level guidance for api change watch version diffs with action-oriented workflow, review criteria, and failure patterns.
---

API Change Watch Version Diffs is an expert API and integration skill for API Change Watch. The focal concern is Version Diffs, but the real objective is helping an AI make external integrations reliable, reviewable, and survivable when contracts, auth, and partners drift.

## When To Use

- Use this when API Change Watch Version Diffs affects whether external integrations remain trustworthy after partner behavior changes.
...
- What partner or contract assumption inside API Change Watch Version Diffs is least likely to hold over time?
- How would API Change Watch Version Diffs fail if the partner changed validation, latency, or auth behavior without notice?
- What evidence proves sandbox behavior is close enough to production for API Change Watch Version Diffs?
- Which part of API Change Watch Version Diffs needs stronger replay, traceability, or escalation support?
- How will the AI explain the safety model of API Change Watch Version Diffs to an integration reviewer?
...
- A contract and drift note for API Change Watch Version Diffs.
...
- [ ] The external contract and drift risk behind API Change Watch Version Diffs are explicit.
