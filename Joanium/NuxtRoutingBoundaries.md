---
name: Nuxt Routing Boundaries
trigger: nuxt routing boundaries, help with nuxt routing boundaries, plan nuxt routing boundaries, improve nuxt routing boundaries, expert nuxt routing boundaries
description: Expert-level guidance for nuxt routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Nuxt Routing Boundaries is an expert frontend skill for Nuxt. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Nuxt Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Nuxt Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in Nuxt Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Nuxt Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does Nuxt Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Nuxt Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for Nuxt Routing Boundaries.
...
- [ ] The state model for Nuxt Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend Nuxt Routing Boundaries without guessing hidden UI contracts.
