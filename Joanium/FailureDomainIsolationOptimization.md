---
name: Failure Domain Isolation Optimization
trigger: failure domain isolation optimization, help with failure domain isolation optimization, plan failure domain isolation optimization, improve failure domain isolation optimization, expert failure domain isolation optimization
description: Expert-level guidance for failure domain isolation optimization, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Failure Domain Isolation Optimization is an expert-level infrastructure skill for reducing operational risk while keeping rollout control, recovery, and ownership explicit. The emphasis here is improving the limiting resource without merely moving cost, fragility, or risk somewhere else.

## When To Use

- Use this when Failure Domain Isolation Optimization changes service availability, deployment safety, or recovery behavior.
...
- What is the actual bottleneck in Failure Domain Isolation Optimization: CPU, memory, latency, network, human time, or policy friction?
...
- A baseline report for Failure Domain Isolation Optimization with representative workloads.
