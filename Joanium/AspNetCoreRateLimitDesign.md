---
name: ASP.NET Core Rate Limit Design
trigger: asp.net core rate limit design, help with asp.net core rate limit design, plan asp.net core rate limit design, improve asp.net core rate limit design, expert asp.net core rate limit design
description: Expert-level guidance for asp.net core rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

ASP.NET Core Rate Limit Design is an expert backend skill for ASP.NET Core. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when ASP.NET Core Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside ASP.NET Core Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in ASP.NET Core Rate Limit Design?
- Where does ASP.NET Core Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether ASP.NET Core Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by ASP.NET Core Rate Limit Design even if it was never written down?
...
- A contract note for ASP.NET Core Rate Limit Design that includes failure semantics.
...
- [ ] The contract for ASP.NET Core Rate Limit Design is explicit about success and failure behavior.
