---
name: Robotics State Machines
trigger: robotics state machines, help with robotics state machines, plan robotics state machines, improve robotics state machines, expert robotics state machines
description: Expert-level guidance for robotics state machines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Robotics State Machines is an expert systems skill for Robotics. The focal concern is State Machines, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Robotics State Machines is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Robotics State Machines would break first under scale, timing, or hardware stress?
- Which state or resource in Robotics State Machines is hardest to observe when behavior goes wrong?
- How does Robotics State Machines trade correctness, throughput, and recoverability today?
- What tooling gap makes Robotics State Machines harder to reason about than it should be?
- Which optimization target inside Robotics State Machines is attractive but strategically secondary?
...
- A system model for Robotics State Machines that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Robotics State Machines is explicit.
