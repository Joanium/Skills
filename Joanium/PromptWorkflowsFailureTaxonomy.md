---
name: Prompt Workflows Failure Taxonomy
trigger: prompt workflows failure taxonomy, help with prompt workflows failure taxonomy, plan prompt workflows failure taxonomy, improve prompt workflows failure taxonomy, expert prompt workflows failure taxonomy
description: Expert-level guidance for prompt workflows failure taxonomy with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Workflows Failure Taxonomy is an expert applied AI skill for Prompt Workflows. The focal concern is Failure Taxonomy, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Prompt Workflows Failure Taxonomy affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Prompt Workflows Failure Taxonomy is currently measured by vibes rather than a real contract?
- Which failure in Prompt Workflows Failure Taxonomy would be hardest to notice before it harms users or operators?
- How will Prompt Workflows Failure Taxonomy behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Prompt Workflows Failure Taxonomy?
...
- A behavior contract for Prompt Workflows Failure Taxonomy.
...
- [ ] The contract behind Prompt Workflows Failure Taxonomy is explicit enough to evaluate.
