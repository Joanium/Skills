---
name: Spark Schema Evolution
trigger: spark schema evolution, help with spark schema evolution, plan spark schema evolution, improve spark schema evolution, expert spark schema evolution
description: Expert-level guidance for spark schema evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Spark Schema Evolution is an expert data-systems skill for Spark. The focal concern is Schema Evolution, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Spark Schema Evolution affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Spark Schema Evolution create if two teams interpreted it differently?
- Which recovery path for Spark Schema Evolution is the least understood today?
- How will Spark Schema Evolution behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Spark Schema Evolution is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Spark Schema Evolution drifted subtly?
...
- A semantic contract note for Spark Schema Evolution.
...
- [ ] The meaning of Spark Schema Evolution is written in language consumers will actually use.
