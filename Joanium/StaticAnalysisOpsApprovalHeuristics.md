---
name: Static Analysis Operations Approval Heuristics
trigger: static analysis operations approval heuristics, help with static analysis operations approval heuristics, plan static analysis operations approval heuristics, improve static analysis operations approval heuristics, expert static analysis operations approval heuristics
description: Expert-level guidance for static analysis operations approval heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Static Analysis Operations Approval Heuristics is an expert repository and delivery skill for Static Analysis Operations. The focal concern is Approval Heuristics, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Static Analysis Operations Approval Heuristics determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Static Analysis Operations Approval Heuristics riskier than the diff itself suggests?
- Which reviewer would challenge Static Analysis Operations Approval Heuristics first, and what evidence would they need?
- How could Static Analysis Operations Approval Heuristics fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Static Analysis Operations Approval Heuristics would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Static Analysis Operations Approval Heuristics concretely?
...
- A blast-radius note for Static Analysis Operations Approval Heuristics.
...
- [ ] The repository-level blast radius of Static Analysis Operations Approval Heuristics is explicit.
