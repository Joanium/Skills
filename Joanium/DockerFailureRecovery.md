---
name: Docker Failure Recovery
trigger: docker failure recovery, help with docker failure recovery, plan docker failure recovery, improve docker failure recovery, expert docker failure recovery
description: Expert-level guidance for docker failure recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Docker Failure Recovery is an expert cloud and platform skill for Docker. The focal concern is Failure Recovery, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Docker Failure Recovery changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Docker Failure Recovery would fail first during a rushed rollout or incident?
- Which control in Docker Failure Recovery is automated but not truly reversible?
- How will operators know whether Docker Failure Recovery is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Docker Failure Recovery is invisible until it becomes expensive?
- Which dependency makes Docker Failure Recovery less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Docker Failure Recovery.
...
- [ ] The rollback path for Docker Failure Recovery is explicit and tested.
