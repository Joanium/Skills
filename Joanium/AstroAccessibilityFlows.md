---
name: Astro Accessibility Flows
trigger: astro accessibility flows, help with astro accessibility flows, plan astro accessibility flows, improve astro accessibility flows, expert astro accessibility flows
description: Expert-level guidance for astro accessibility flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Astro Accessibility Flows is an expert frontend skill for Astro. The focal concern is Accessibility Flows, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Astro Accessibility Flows affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Astro Accessibility Flows make user state harder to reason about than it needs to be?
- Which interaction in Astro Accessibility Flows degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Astro Accessibility Flows would break if the DOM or state changed unexpectedly?
- How does Astro Accessibility Flows fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Astro Accessibility Flows looks correct in a mock but collapses under production behavior?
...
- A state transition map for Astro Accessibility Flows.
...
- [ ] The state model for Astro Accessibility Flows is clear enough to debug.
...
- [ ] Another engineer can extend Astro Accessibility Flows without guessing hidden UI contracts.
