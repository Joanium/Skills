---
name: Legacy Refactor Edge Case Sweeps
trigger: legacy refactor edge case sweeps, help with legacy refactor edge case sweeps, plan legacy refactor edge case sweeps, improve legacy refactor edge case sweeps, expert legacy refactor edge case sweeps
description: Expert-level guidance for legacy refactor edge case sweeps with action-oriented workflow, review criteria, and failure patterns.
---

Legacy Refactor Edge Case Sweeps is an expert coding workflow skill for Legacy Refactor. The focal concern is Edge Case Sweeps, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Legacy Refactor Edge Case Sweeps is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Legacy Refactor Edge Case Sweeps is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Legacy Refactor Edge Case Sweeps would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Legacy Refactor Edge Case Sweeps create a silent regression that tests may miss?
- How would the AI explain the safety of Legacy Refactor Edge Case Sweeps to a skeptical reviewer?
...
- A concise evidence note for Legacy Refactor Edge Case Sweeps.
...
- [ ] The evidence supporting Legacy Refactor Edge Case Sweeps is stronger than the assumptions.
