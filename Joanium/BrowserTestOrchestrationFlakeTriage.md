---
name: Browser Test Orchestration Flake Triage
trigger: browser test orchestration flake triage, help with browser test orchestration flake triage, plan browser test orchestration flake triage, improve browser test orchestration flake triage, expert browser test orchestration flake triage
description: Expert-level guidance for browser test orchestration flake triage with action-oriented workflow, review criteria, and failure patterns.
---

Browser Test Orchestration Flake Triage is an expert browser automation skill for Browser Test Orchestration. The focal concern is Flake Triage, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Browser Test Orchestration Flake Triage needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Browser Test Orchestration Flake Triage unreliable across runs?
- Which timing or selector assumption in Browser Test Orchestration Flake Triage is least stable under real conditions?
- How would Browser Test Orchestration Flake Triage behave differently across browsers, profiles, or CI environments?
- What artifacts should Browser Test Orchestration Flake Triage capture so the next failure is diagnosable instead of mysterious?
- Which part of Browser Test Orchestration Flake Triage is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Browser Test Orchestration Flake Triage.
...
- [ ] The state model behind Browser Test Orchestration Flake Triage is explicit.
