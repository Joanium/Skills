---
name: Form Autofill Recovery Flake Triage
trigger: form autofill recovery flake triage, help with form autofill recovery flake triage, plan form autofill recovery flake triage, improve form autofill recovery flake triage, expert form autofill recovery flake triage
description: Expert-level guidance for form autofill recovery flake triage with action-oriented workflow, review criteria, and failure patterns.
---

Form Autofill Recovery Flake Triage is an expert browser automation skill for Form Autofill Recovery. The focal concern is Flake Triage, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Form Autofill Recovery Flake Triage needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Form Autofill Recovery Flake Triage unreliable across runs?
- Which timing or selector assumption in Form Autofill Recovery Flake Triage is least stable under real conditions?
- How would Form Autofill Recovery Flake Triage behave differently across browsers, profiles, or CI environments?
- What artifacts should Form Autofill Recovery Flake Triage capture so the next failure is diagnosable instead of mysterious?
- Which part of Form Autofill Recovery Flake Triage is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Form Autofill Recovery Flake Triage.
...
- [ ] The state model behind Form Autofill Recovery Flake Triage is explicit.
