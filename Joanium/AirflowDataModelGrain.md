---
name: Airflow Data Model Grain
trigger: airflow data model grain, help with airflow data model grain, plan airflow data model grain, improve airflow data model grain, expert airflow data model grain
description: Expert-level guidance for airflow data model grain with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Airflow Data Model Grain is an expert data-systems skill for Airflow. The focal concern is Data Model Grain, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Airflow Data Model Grain affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Airflow Data Model Grain create if two teams interpreted it differently?
- Which recovery path for Airflow Data Model Grain is the least understood today?
- How will Airflow Data Model Grain behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Airflow Data Model Grain is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Airflow Data Model Grain drifted subtly?
...
- A semantic contract note for Airflow Data Model Grain.
...
- [ ] The meaning of Airflow Data Model Grain is written in language consumers will actually use.
