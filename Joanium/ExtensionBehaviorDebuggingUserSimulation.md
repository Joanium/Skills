---
name: Extension Behavior Debugging User Simulation
trigger: extension behavior debugging user simulation, help with extension behavior debugging user simulation, plan extension behavior debugging user simulation, improve extension behavior debugging user simulation, expert extension behavior debugging user simulation
description: Expert-level guidance for extension behavior debugging user simulation with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging User Simulation is an expert browser automation skill for Extension Behavior Debugging. The focal concern is User Simulation, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging User Simulation needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging User Simulation unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging User Simulation is least stable under real conditions?
- How would Extension Behavior Debugging User Simulation behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging User Simulation capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging User Simulation is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging User Simulation.
...
- [ ] The state model behind Extension Behavior Debugging User Simulation is explicit.
