---
name: Go Error Semantics
trigger: go error semantics, help with go error semantics, plan go error semantics, improve go error semantics, expert go error semantics
description: Expert-level guidance for go error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Go Error Semantics is an expert programming-language skill for Go. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Go Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Go Error Semantics?
- Which abstraction in Go Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Go Error Semantics first?
- How will Go Error Semantics behave when multiple contributors evolve it independently?
- Which part of Go Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Go Error Semantics.
- A failure-mode note that explains the sharp edges inside Go Error Semantics.
...
- [ ] The main language-level risk in Go Error Semantics is named explicitly.
...
- [ ] Future contributors can extend Go Error Semantics without reverse-engineering hidden contracts.
