---
name: Failure Injection Lab Boundary Scenarios
trigger: failure injection lab boundary scenarios, help with failure injection lab boundary scenarios, plan failure injection lab boundary scenarios, improve failure injection lab boundary scenarios, expert failure injection lab boundary scenarios
description: Expert-level guidance for failure injection lab boundary scenarios with action-oriented workflow, review criteria, and failure patterns.
---

Failure Injection Lab Boundary Scenarios is an expert testing and quality skill for Failure Injection Lab. The focal concern is Boundary Scenarios, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Failure Injection Lab Boundary Scenarios needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Failure Injection Lab Boundary Scenarios is still untested even if coverage looks healthy?
- Which assertion in Failure Injection Lab Boundary Scenarios would stay green while the user-visible behavior regressed?
- How much of Failure Injection Lab Boundary Scenarios depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Failure Injection Lab Boundary Scenarios need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Failure Injection Lab Boundary Scenarios.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Failure Injection Lab Boundary Scenarios should mean.
...
- [ ] The key behavior protected by Failure Injection Lab Boundary Scenarios is explicit.
