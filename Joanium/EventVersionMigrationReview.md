---
name: Event Version Migration Review
trigger: event version migration review, help with event version migration review, plan event version migration review, improve event version migration review, expert event version migration review
description: Expert-level guidance for event version migration review, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Event Version Migration Review is an expert-level data systems skill for turning analysis and pipelines into governed, trusted, and operationally stable assets. The emphasis here is challenging assumptions, proving readiness, and deciding whether the work is actually safe to adopt.

## When To Use

- Use this when Event Version Migration Review affects downstream metrics, decisions, or customer-facing data products.
...
- What claims about Event Version Migration Review are supported by evidence, and which are still assertions?
...
- A review summary for Event Version Migration Review with clear approval status.
