---
name: Prompt Failure Triage Task Contracts
trigger: prompt failure triage task contracts, help with prompt failure triage task contracts, plan prompt failure triage task contracts, improve prompt failure triage task contracts, expert prompt failure triage task contracts
description: Expert-level guidance for prompt failure triage task contracts with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Task Contracts is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Task Contracts, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Task Contracts affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Task Contracts is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Task Contracts would be hardest to detect before it compounds?
- How would Prompt Failure Triage Task Contracts behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Task Contracts?
...
- A contract and failure-label set for Prompt Failure Triage Task Contracts.
...
- [ ] The system behind Prompt Failure Triage Task Contracts is described more clearly than the prompt surface.
