---
name: Logging Instrumentation Edge Case Sweeps
trigger: logging instrumentation edge case sweeps, help with logging instrumentation edge case sweeps, plan logging instrumentation edge case sweeps, improve logging instrumentation edge case sweeps, expert logging instrumentation edge case sweeps
description: Expert-level guidance for logging instrumentation edge case sweeps with action-oriented workflow, review criteria, and failure patterns.
---

Logging Instrumentation Edge Case Sweeps is an expert coding workflow skill for Logging Instrumentation. The focal concern is Edge Case Sweeps, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Logging Instrumentation Edge Case Sweeps is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Logging Instrumentation Edge Case Sweeps is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Logging Instrumentation Edge Case Sweeps would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Logging Instrumentation Edge Case Sweeps create a silent regression that tests may miss?
- How would the AI explain the safety of Logging Instrumentation Edge Case Sweeps to a skeptical reviewer?
...
- A concise evidence note for Logging Instrumentation Edge Case Sweeps.
...
- [ ] The evidence supporting Logging Instrumentation Edge Case Sweeps is stronger than the assumptions.
