---
name: API Incident Triage Dependency Isolation
trigger: api incident triage dependency isolation, help with api incident triage dependency isolation, plan api incident triage dependency isolation, improve api incident triage dependency isolation, expert api incident triage dependency isolation
description: Expert-level guidance for api incident triage dependency isolation with action-oriented workflow, review criteria, and failure patterns.
---

API Incident Triage Dependency Isolation is an expert backend execution skill for API Incident Triage. The focal concern is Dependency Isolation, but the real objective is helping an AI design service behavior that stays coherent under retries, queues, partial failure, and operational pressure.

## When To Use

- Use this when API Incident Triage Dependency Isolation affects whether backend changes remain safe once real production behavior kicks in.
...
- Which failure ordering inside API Incident Triage Dependency Isolation would create the most misleading symptom?
- Where does API Incident Triage Dependency Isolation need stronger idempotency or retry discipline?
- What dependency in API Incident Triage Dependency Isolation is currently hiding behind timeout or queue noise?
- How would an operator explain the health of API Incident Triage Dependency Isolation in the first five minutes of an incident?
- What contract assumption in API Incident Triage Dependency Isolation is least defensible under partial failure?
...
- A failure-sequence note for API Incident Triage Dependency Isolation.
...
- [ ] The real behavior of API Incident Triage Dependency Isolation under retries and failures is explicit.
