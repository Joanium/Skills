---
name: Build Failure Response Evidence Packets
trigger: build failure response evidence packets, help with build failure response evidence packets, plan build failure response evidence packets, improve build failure response evidence packets, expert build failure response evidence packets
description: Expert-level guidance for build failure response evidence packets with action-oriented workflow, review criteria, and failure patterns.
---

Build Failure Response Evidence Packets is an expert repository and delivery skill for Build Failure Response. The focal concern is Evidence Packets, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Build Failure Response Evidence Packets determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Build Failure Response Evidence Packets riskier than the diff itself suggests?
- Which reviewer would challenge Build Failure Response Evidence Packets first, and what evidence would they need?
- How could Build Failure Response Evidence Packets fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Build Failure Response Evidence Packets would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Build Failure Response Evidence Packets concretely?
...
- A blast-radius note for Build Failure Response Evidence Packets.
...
- [ ] The repository-level blast radius of Build Failure Response Evidence Packets is explicit.
