---
name: Robotics API Design
trigger: robotics api design, help with robotics api design, plan robotics api design, improve robotics api design, expert robotics api design
description: Expert-level guidance for robotics api design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Robotics API Design is an expert systems skill for Robotics. The focal concern is API Design, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Robotics API Design is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Robotics API Design would break first under scale, timing, or hardware stress?
- Which state or resource in Robotics API Design is hardest to observe when behavior goes wrong?
- How does Robotics API Design trade correctness, throughput, and recoverability today?
- What tooling gap makes Robotics API Design harder to reason about than it should be?
- Which optimization target inside Robotics API Design is attractive but strategically secondary?
...
- A system model for Robotics API Design that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Robotics API Design is explicit.
