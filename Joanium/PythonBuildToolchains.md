---
name: Python Build Toolchains
trigger: python build toolchains, help with python build toolchains, plan python build toolchains, improve python build toolchains, expert python build toolchains
description: Expert-level guidance for python build toolchains with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Python Build Toolchains is an expert programming-language skill for Python. The focal concern is Build Toolchains, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Python Build Toolchains affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Python Build Toolchains?
- Which abstraction in Python Build Toolchains becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Python Build Toolchains first?
- How will Python Build Toolchains behave when multiple contributors evolve it independently?
- Which part of Python Build Toolchains is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Python Build Toolchains.
- A failure-mode note that explains the sharp edges inside Python Build Toolchains.
...
- [ ] The main language-level risk in Python Build Toolchains is named explicitly.
...
- [ ] Future contributors can extend Python Build Toolchains without reverse-engineering hidden contracts.
