---
name: Algorithms Memory Safety
trigger: algorithms memory safety, help with algorithms memory safety, plan algorithms memory safety, improve algorithms memory safety, expert algorithms memory safety
description: Expert-level guidance for algorithms memory safety with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Algorithms Memory Safety is an expert systems skill for Algorithms. The focal concern is Memory Safety, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Algorithms Memory Safety is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Algorithms Memory Safety would break first under scale, timing, or hardware stress?
- Which state or resource in Algorithms Memory Safety is hardest to observe when behavior goes wrong?
- How does Algorithms Memory Safety trade correctness, throughput, and recoverability today?
- What tooling gap makes Algorithms Memory Safety harder to reason about than it should be?
- Which optimization target inside Algorithms Memory Safety is attractive but strategically secondary?
...
- A system model for Algorithms Memory Safety that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Algorithms Memory Safety is explicit.
