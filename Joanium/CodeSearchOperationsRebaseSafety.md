---
name: Code Search Operations Rebase Safety
trigger: code search operations rebase safety, help with code search operations rebase safety, plan code search operations rebase safety, improve code search operations rebase safety, expert code search operations rebase safety
description: Expert-level guidance for code search operations rebase safety with action-oriented workflow, review criteria, and failure patterns.
---

Code Search Operations Rebase Safety is an expert repository and delivery skill for Code Search Operations. The focal concern is Rebase Safety, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Code Search Operations Rebase Safety determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Code Search Operations Rebase Safety riskier than the diff itself suggests?
- Which reviewer would challenge Code Search Operations Rebase Safety first, and what evidence would they need?
- How could Code Search Operations Rebase Safety fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Code Search Operations Rebase Safety would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Code Search Operations Rebase Safety concretely?
...
- A blast-radius note for Code Search Operations Rebase Safety.
...
- [ ] The repository-level blast radius of Code Search Operations Rebase Safety is explicit.
