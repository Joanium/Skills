---
name: Auth Flow Repair Causal Tracing
trigger: auth flow repair causal tracing, help with auth flow repair causal tracing, plan auth flow repair causal tracing, improve auth flow repair causal tracing, expert auth flow repair causal tracing
description: Expert-level guidance for auth flow repair causal tracing with action-oriented workflow, review criteria, and failure patterns.
---

Auth Flow Repair Causal Tracing is an expert backend execution skill for Auth Flow Repair. The focal concern is Causal Tracing, but the real objective is helping an AI design service behavior that stays coherent under retries, queues, partial failure, and operational pressure.

## When To Use

- Use this when Auth Flow Repair Causal Tracing affects whether backend changes remain safe once real production behavior kicks in.
...
- Which failure ordering inside Auth Flow Repair Causal Tracing would create the most misleading symptom?
- Where does Auth Flow Repair Causal Tracing need stronger idempotency or retry discipline?
- What dependency in Auth Flow Repair Causal Tracing is currently hiding behind timeout or queue noise?
- How would an operator explain the health of Auth Flow Repair Causal Tracing in the first five minutes of an incident?
- What contract assumption in Auth Flow Repair Causal Tracing is least defensible under partial failure?
...
- A failure-sequence note for Auth Flow Repair Causal Tracing.
...
- [ ] The real behavior of Auth Flow Repair Causal Tracing under retries and failures is explicit.
