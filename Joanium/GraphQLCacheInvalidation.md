---
name: GraphQL Cache Invalidation
trigger: graphql cache invalidation, help with graphql cache invalidation, plan graphql cache invalidation, improve graphql cache invalidation, expert graphql cache invalidation
description: Expert-level guidance for graphql cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

GraphQL Cache Invalidation is an expert backend skill for GraphQL. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when GraphQL Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside GraphQL Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in GraphQL Cache Invalidation?
- Where does GraphQL Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether GraphQL Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by GraphQL Cache Invalidation even if it was never written down?
...
- A contract note for GraphQL Cache Invalidation that includes failure semantics.
...
- [ ] The contract for GraphQL Cache Invalidation is explicit about success and failure behavior.
