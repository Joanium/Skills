---
name: Snapshot Testing Operations Mock Discipline
trigger: snapshot testing operations mock discipline, help with snapshot testing operations mock discipline, plan snapshot testing operations mock discipline, improve snapshot testing operations mock discipline, expert snapshot testing operations mock discipline
description: Expert-level guidance for snapshot testing operations mock discipline with action-oriented workflow, review criteria, and failure patterns.
---

Snapshot Testing Operations Mock Discipline is an expert testing and quality skill for Snapshot Testing Operations. The focal concern is Mock Discipline, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Snapshot Testing Operations Mock Discipline needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Snapshot Testing Operations Mock Discipline is still untested even if coverage looks healthy?
- Which assertion in Snapshot Testing Operations Mock Discipline would stay green while the user-visible behavior regressed?
- How much of Snapshot Testing Operations Mock Discipline depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Snapshot Testing Operations Mock Discipline need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Snapshot Testing Operations Mock Discipline.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Snapshot Testing Operations Mock Discipline should mean.
...
- [ ] The key behavior protected by Snapshot Testing Operations Mock Discipline is explicit.
