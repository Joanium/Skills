---
name: Repository Audit Rollback Plans
trigger: repository audit rollback plans, help with repository audit rollback plans, plan repository audit rollback plans, improve repository audit rollback plans, expert repository audit rollback plans
description: Expert-level guidance for repository audit rollback plans with action-oriented workflow, review criteria, and failure patterns.
---

Repository Audit Rollback Plans is an expert coding workflow skill for Repository Audit. The focal concern is Rollback Plans, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Repository Audit Rollback Plans is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Repository Audit Rollback Plans is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Repository Audit Rollback Plans would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Repository Audit Rollback Plans create a silent regression that tests may miss?
- How would the AI explain the safety of Repository Audit Rollback Plans to a skeptical reviewer?
...
- A concise evidence note for Repository Audit Rollback Plans.
...
- [ ] The evidence supporting Repository Audit Rollback Plans is stronger than the assumptions.
