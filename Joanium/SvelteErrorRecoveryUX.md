---
name: Svelte Error Recovery UX
trigger: svelte error recovery ux, help with svelte error recovery ux, plan svelte error recovery ux, improve svelte error recovery ux, expert svelte error recovery ux
description: Expert-level guidance for svelte error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte Error Recovery UX is an expert frontend skill for Svelte. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in Svelte Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does Svelte Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte Error Recovery UX.
...
- [ ] The state model for Svelte Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend Svelte Error Recovery UX without guessing hidden UI contracts.
