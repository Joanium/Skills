---
name: Java Interop Boundaries
trigger: java interop boundaries, help with java interop boundaries, plan java interop boundaries, improve java interop boundaries, expert java interop boundaries
description: Expert-level guidance for java interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Java Interop Boundaries is an expert programming-language skill for Java. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Java Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Java Interop Boundaries?
- Which abstraction in Java Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Java Interop Boundaries first?
- How will Java Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of Java Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Java Interop Boundaries.
- A failure-mode note that explains the sharp edges inside Java Interop Boundaries.
...
- [ ] The main language-level risk in Java Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend Java Interop Boundaries without reverse-engineering hidden contracts.
