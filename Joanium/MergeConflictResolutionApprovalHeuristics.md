---
name: Merge Conflict Resolution Approval Heuristics
trigger: merge conflict resolution approval heuristics, help with merge conflict resolution approval heuristics, plan merge conflict resolution approval heuristics, improve merge conflict resolution approval heuristics, expert merge conflict resolution approval heuristics
description: Expert-level guidance for merge conflict resolution approval heuristics with action-oriented workflow, review criteria, and failure patterns.
---

Merge Conflict Resolution Approval Heuristics is an expert repository and delivery skill for Merge Conflict Resolution. The focal concern is Approval Heuristics, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Merge Conflict Resolution Approval Heuristics determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Merge Conflict Resolution Approval Heuristics riskier than the diff itself suggests?
- Which reviewer would challenge Merge Conflict Resolution Approval Heuristics first, and what evidence would they need?
- How could Merge Conflict Resolution Approval Heuristics fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Merge Conflict Resolution Approval Heuristics would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Merge Conflict Resolution Approval Heuristics concretely?
...
- A blast-radius note for Merge Conflict Resolution Approval Heuristics.
...
- [ ] The repository-level blast radius of Merge Conflict Resolution Approval Heuristics is explicit.
