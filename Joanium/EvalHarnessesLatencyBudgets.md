---
name: Evaluation Harnesses Latency Budgets
trigger: evaluation harnesses latency budgets, help with evaluation harnesses latency budgets, plan evaluation harnesses latency budgets, improve evaluation harnesses latency budgets, expert evaluation harnesses latency budgets
description: Expert-level guidance for evaluation harnesses latency budgets with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Evaluation Harnesses Latency Budgets is an expert applied-AI skill for Evaluation Harnesses. The focal concern is Latency Budgets, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Evaluation Harnesses Latency Budgets affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Evaluation Harnesses Latency Budgets would look acceptable in a demo but fail under production ambiguity?
- Which part of Evaluation Harnesses Latency Budgets lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Evaluation Harnesses Latency Budgets fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Evaluation Harnesses Latency Budgets is getting better rather than merely changing?
- Where should humans intervene in Evaluation Harnesses Latency Budgets to add leverage instead of toil?
...
- A behavior contract for Evaluation Harnesses Latency Budgets.
...
- [ ] The behavior contract for Evaluation Harnesses Latency Budgets is explicit.
