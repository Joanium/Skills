---
name: NestJS Error Modeling
trigger: nestjs error modeling, help with nestjs error modeling, plan nestjs error modeling, improve nestjs error modeling, expert nestjs error modeling
description: Expert-level guidance for nestjs error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

NestJS Error Modeling is an expert backend skill for NestJS. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when NestJS Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside NestJS Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in NestJS Error Modeling?
- Where does NestJS Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether NestJS Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by NestJS Error Modeling even if it was never written down?
...
- A contract note for NestJS Error Modeling that includes failure semantics.
...
- [ ] The contract for NestJS Error Modeling is explicit about success and failure behavior.
