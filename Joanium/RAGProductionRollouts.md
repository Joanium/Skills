---
name: RAG Production Rollouts
trigger: rag production rollouts, help with rag production rollouts, plan rag production rollouts, improve rag production rollouts, expert rag production rollouts
description: Expert-level guidance for rag production rollouts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

RAG Production Rollouts is an expert applied-AI skill for RAG. The focal concern is Production Rollouts, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when RAG Production Rollouts affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in RAG Production Rollouts would look acceptable in a demo but fail under production ambiguity?
- Which part of RAG Production Rollouts lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will RAG Production Rollouts fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that RAG Production Rollouts is getting better rather than merely changing?
- Where should humans intervene in RAG Production Rollouts to add leverage instead of toil?
...
- A behavior contract for RAG Production Rollouts.
...
- [ ] The behavior contract for RAG Production Rollouts is explicit.
