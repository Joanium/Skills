---
name: Extension Behavior Debugging Timing Stability
trigger: extension behavior debugging timing stability, help with extension behavior debugging timing stability, plan extension behavior debugging timing stability, improve extension behavior debugging timing stability, expert extension behavior debugging timing stability
description: Expert-level guidance for extension behavior debugging timing stability with action-oriented workflow, review criteria, and failure patterns.
---

Extension Behavior Debugging Timing Stability is an expert browser automation skill for Extension Behavior Debugging. The focal concern is Timing Stability, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Extension Behavior Debugging Timing Stability needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Extension Behavior Debugging Timing Stability unreliable across runs?
- Which timing or selector assumption in Extension Behavior Debugging Timing Stability is least stable under real conditions?
- How would Extension Behavior Debugging Timing Stability behave differently across browsers, profiles, or CI environments?
- What artifacts should Extension Behavior Debugging Timing Stability capture so the next failure is diagnosable instead of mysterious?
- Which part of Extension Behavior Debugging Timing Stability is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Extension Behavior Debugging Timing Stability.
...
- [ ] The state model behind Extension Behavior Debugging Timing Stability is explicit.
