---
name: Docker Incident Triage
trigger: docker incident triage, help with docker incident triage, plan docker incident triage, improve docker incident triage, expert docker incident triage
description: Expert-level guidance for docker incident triage with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Docker Incident Triage is an expert cloud and platform skill for Docker. The focal concern is Incident Triage, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Docker Incident Triage changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Docker Incident Triage would fail first during a rushed rollout or incident?
- Which control in Docker Incident Triage is automated but not truly reversible?
- How will operators know whether Docker Incident Triage is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Docker Incident Triage is invisible until it becomes expensive?
- Which dependency makes Docker Incident Triage less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Docker Incident Triage.
...
- [ ] The rollback path for Docker Incident Triage is explicit and tested.
