---
name: RAG Latency Budgets
trigger: rag latency budgets, help with rag latency budgets, plan rag latency budgets, improve rag latency budgets, expert rag latency budgets
description: Expert-level guidance for rag latency budgets with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

RAG Latency Budgets is an expert applied-AI skill for RAG. The focal concern is Latency Budgets, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when RAG Latency Budgets affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in RAG Latency Budgets would look acceptable in a demo but fail under production ambiguity?
- Which part of RAG Latency Budgets lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will RAG Latency Budgets fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that RAG Latency Budgets is getting better rather than merely changing?
- Where should humans intervene in RAG Latency Budgets to add leverage instead of toil?
...
- A behavior contract for RAG Latency Budgets.
...
- [ ] The behavior contract for RAG Latency Budgets is explicit.
