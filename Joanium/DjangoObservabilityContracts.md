---
name: Django Observability Contracts
trigger: django observability contracts, help with django observability contracts, plan django observability contracts, improve django observability contracts, expert django observability contracts
description: Expert-level guidance for django observability contracts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Django Observability Contracts is an expert backend skill for Django. The focal concern is Observability Contracts, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Django Observability Contracts shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Django Observability Contracts breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Django Observability Contracts?
- Where does Django Observability Contracts need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Django Observability Contracts is unhealthy before customers do?
- What compatibility promise is implied by Django Observability Contracts even if it was never written down?
...
- A contract note for Django Observability Contracts that includes failure semantics.
...
- [ ] The contract for Django Observability Contracts is explicit about success and failure behavior.
