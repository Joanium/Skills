---
name: Agent State Machines Design
trigger: agent state machines design, help with agent state machines design, plan agent state machines design, improve agent state machines design, expert agent state machines design
description: Expert-level guidance for agent state machines design, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Agent State Machines Design is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is on interfaces, invariants, failure handling, and why one structure is safer than its alternatives.

## When To Use

- Use this when Agent State Machines Design influences user-visible output, automation, or safety-sensitive decisions.
...
- What are the core boundaries, contracts, and invariants in Agent State Machines Design?
...
- An architecture or boundary diagram for Agent State Machines Design.
