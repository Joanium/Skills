---
name: Performance Triage Change Scoping
trigger: performance triage change scoping, help with performance triage change scoping, plan performance triage change scoping, improve performance triage change scoping, expert performance triage change scoping
description: Expert-level guidance for performance triage change scoping with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Change Scoping is an expert coding workflow skill for Performance Triage. The focal concern is Change Scoping, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Change Scoping is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Change Scoping is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Change Scoping would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Change Scoping create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Change Scoping to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Change Scoping.
...
- [ ] The evidence supporting Performance Triage Change Scoping is stronger than the assumptions.
