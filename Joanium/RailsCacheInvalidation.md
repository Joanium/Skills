---
name: Rails Cache Invalidation
trigger: rails cache invalidation, help with rails cache invalidation, plan rails cache invalidation, improve rails cache invalidation, expert rails cache invalidation
description: Expert-level guidance for rails cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rails Cache Invalidation is an expert backend skill for Rails. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Rails Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Rails Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Rails Cache Invalidation?
- Where does Rails Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Rails Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by Rails Cache Invalidation even if it was never written down?
...
- A contract note for Rails Cache Invalidation that includes failure semantics.
...
- [ ] The contract for Rails Cache Invalidation is explicit about success and failure behavior.
