---
name: Build Failure Response Diff Compression
trigger: build failure response diff compression, help with build failure response diff compression, plan build failure response diff compression, improve build failure response diff compression, expert build failure response diff compression
description: Expert-level guidance for build failure response diff compression with action-oriented workflow, review criteria, and failure patterns.
---

Build Failure Response Diff Compression is an expert repository and delivery skill for Build Failure Response. The focal concern is Diff Compression, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Build Failure Response Diff Compression determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Build Failure Response Diff Compression riskier than the diff itself suggests?
- Which reviewer would challenge Build Failure Response Diff Compression first, and what evidence would they need?
- How could Build Failure Response Diff Compression fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Build Failure Response Diff Compression would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Build Failure Response Diff Compression concretely?
...
- A blast-radius note for Build Failure Response Diff Compression.
...
- [ ] The repository-level blast radius of Build Failure Response Diff Compression is explicit.
