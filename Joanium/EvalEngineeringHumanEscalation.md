---
name: Evaluation Engineering Human Escalation
trigger: evaluation engineering human escalation, help with evaluation engineering human escalation, plan evaluation engineering human escalation, improve evaluation engineering human escalation, expert evaluation engineering human escalation
description: Expert-level guidance for evaluation engineering human escalation with action-oriented workflow, review criteria, and failure patterns.
---

Evaluation Engineering Human Escalation is an expert applied AI skill for Evaluation Engineering. The focal concern is Human Escalation, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Evaluation Engineering Human Escalation affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Evaluation Engineering Human Escalation is currently measured by vibes rather than a real contract?
- Which failure in Evaluation Engineering Human Escalation would be hardest to notice before it harms users or operators?
- How will Evaluation Engineering Human Escalation behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Evaluation Engineering Human Escalation?
...
- A behavior contract for Evaluation Engineering Human Escalation.
...
- [ ] The contract behind Evaluation Engineering Human Escalation is explicit enough to evaluate.
