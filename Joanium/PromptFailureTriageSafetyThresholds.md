---
name: Prompt Failure Triage Safety Thresholds
trigger: prompt failure triage safety thresholds, help with prompt failure triage safety thresholds, plan prompt failure triage safety thresholds, improve prompt failure triage safety thresholds, expert prompt failure triage safety thresholds
description: Expert-level guidance for prompt failure triage safety thresholds with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Safety Thresholds is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Safety Thresholds, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Safety Thresholds affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Safety Thresholds is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Safety Thresholds would be hardest to detect before it compounds?
- How would Prompt Failure Triage Safety Thresholds behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Safety Thresholds?
...
- A contract and failure-label set for Prompt Failure Triage Safety Thresholds.
...
- [ ] The system behind Prompt Failure Triage Safety Thresholds is described more clearly than the prompt surface.
