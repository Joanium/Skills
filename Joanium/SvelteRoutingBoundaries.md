---
name: Svelte Routing Boundaries
trigger: svelte routing boundaries, help with svelte routing boundaries, plan svelte routing boundaries, improve svelte routing boundaries, expert svelte routing boundaries
description: Expert-level guidance for svelte routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte Routing Boundaries is an expert frontend skill for Svelte. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in Svelte Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does Svelte Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte Routing Boundaries.
...
- [ ] The state model for Svelte Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend Svelte Routing Boundaries without guessing hidden UI contracts.
