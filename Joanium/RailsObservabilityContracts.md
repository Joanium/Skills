---
name: Rails Observability Contracts
trigger: rails observability contracts, help with rails observability contracts, plan rails observability contracts, improve rails observability contracts, expert rails observability contracts
description: Expert-level guidance for rails observability contracts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rails Observability Contracts is an expert backend skill for Rails. The focal concern is Observability Contracts, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Rails Observability Contracts shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Rails Observability Contracts breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Rails Observability Contracts?
- Where does Rails Observability Contracts need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Rails Observability Contracts is unhealthy before customers do?
- What compatibility promise is implied by Rails Observability Contracts even if it was never written down?
...
- A contract note for Rails Observability Contracts that includes failure semantics.
...
- [ ] The contract for Rails Observability Contracts is explicit about success and failure behavior.
