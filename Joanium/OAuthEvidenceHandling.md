---
name: OAuth Evidence Handling
trigger: oauth evidence handling, help with oauth evidence handling, plan oauth evidence handling, improve oauth evidence handling, expert oauth evidence handling
description: Expert-level guidance for oauth evidence handling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

OAuth Evidence Handling is an expert security skill for OAuth. The focal concern is Evidence Handling, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when OAuth Evidence Handling influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in OAuth Evidence Handling first?
- Which exception or emergency path makes OAuth Evidence Handling less true in practice than in policy?
- What evidence would be missing if OAuth Evidence Handling failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does OAuth Evidence Handling degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for OAuth Evidence Handling.
...
- [ ] The attacker model for OAuth Evidence Handling is realistic and current.
...
- [ ] The team can explain how OAuth Evidence Handling fails and how it recovers.
