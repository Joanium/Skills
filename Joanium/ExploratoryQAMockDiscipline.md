---
name: Exploratory QA Mock Discipline
trigger: exploratory qa mock discipline, help with exploratory qa mock discipline, plan exploratory qa mock discipline, improve exploratory qa mock discipline, expert exploratory qa mock discipline
description: Expert-level guidance for exploratory qa mock discipline with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Mock Discipline is an expert testing and quality skill for Exploratory QA. The focal concern is Mock Discipline, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Mock Discipline needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Mock Discipline is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Mock Discipline would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Mock Discipline depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Mock Discipline need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Mock Discipline.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Mock Discipline should mean.
...
- [ ] The key behavior protected by Exploratory QA Mock Discipline is explicit.
