---
name: Modular Monoliths Constraint Mapping
trigger: modular monoliths constraint mapping, help with modular monoliths constraint mapping, plan modular monoliths constraint mapping, improve modular monoliths constraint mapping, expert modular monoliths constraint mapping
description: Expert-level guidance for modular monoliths constraint mapping with action-oriented workflow, review criteria, and failure patterns.
---

Modular Monoliths Constraint Mapping is an expert architecture and systems skill for Modular Monoliths. The focal concern is Constraint Mapping, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Modular Monoliths Constraint Mapping needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Modular Monoliths Constraint Mapping becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Modular Monoliths Constraint Mapping is currently cheap only because the bill has not arrived yet?
- How would Modular Monoliths Constraint Mapping be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Modular Monoliths Constraint Mapping too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Modular Monoliths Constraint Mapping.
...
- [ ] The key boundaries inside Modular Monoliths Constraint Mapping are explicit and justified.
