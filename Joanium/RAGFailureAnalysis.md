---
name: RAG Failure Analysis
trigger: rag failure analysis, help with rag failure analysis, plan rag failure analysis, improve rag failure analysis, expert rag failure analysis
description: Expert-level guidance for rag failure analysis with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

RAG Failure Analysis is an expert applied-AI skill for RAG. The focal concern is Failure Analysis, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when RAG Failure Analysis affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in RAG Failure Analysis would look acceptable in a demo but fail under production ambiguity?
- Which part of RAG Failure Analysis lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will RAG Failure Analysis fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that RAG Failure Analysis is getting better rather than merely changing?
- Where should humans intervene in RAG Failure Analysis to add leverage instead of toil?
...
- A behavior contract for RAG Failure Analysis.
...
- [ ] The behavior contract for RAG Failure Analysis is explicit.
