---
name: REST API Authentication Flows
trigger: rest api authentication flows, help with rest api authentication flows, plan rest api authentication flows, improve rest api authentication flows, expert rest api authentication flows
description: Expert-level guidance for rest api authentication flows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

REST API Authentication Flows is an expert backend skill for REST API. The focal concern is Authentication Flows, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when REST API Authentication Flows shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside REST API Authentication Flows breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in REST API Authentication Flows?
- Where does REST API Authentication Flows need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether REST API Authentication Flows is unhealthy before customers do?
- What compatibility promise is implied by REST API Authentication Flows even if it was never written down?
...
- A contract note for REST API Authentication Flows that includes failure semantics.
...
- [ ] The contract for REST API Authentication Flows is explicit about success and failure behavior.
