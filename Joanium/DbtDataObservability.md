---
name: dbt Data Observability
trigger: dbt data observability, help with dbt data observability, plan dbt data observability, improve dbt data observability, expert dbt data observability
description: Expert-level guidance for dbt data observability with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

dbt Data Observability is an expert data-systems skill for dbt. The focal concern is Data Observability, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when dbt Data Observability affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would dbt Data Observability create if two teams interpreted it differently?
- Which recovery path for dbt Data Observability is the least understood today?
- How will dbt Data Observability behave under backfills, reprocessing, or delayed sources?
- What cost pattern in dbt Data Observability is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside dbt Data Observability drifted subtly?
...
- A semantic contract note for dbt Data Observability.
...
- [ ] The meaning of dbt Data Observability is written in language consumers will actually use.
