---
name: Go Interop Boundaries
trigger: go interop boundaries, help with go interop boundaries, plan go interop boundaries, improve go interop boundaries, expert go interop boundaries
description: Expert-level guidance for go interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Go Interop Boundaries is an expert programming-language skill for Go. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Go Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Go Interop Boundaries?
- Which abstraction in Go Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Go Interop Boundaries first?
- How will Go Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of Go Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Go Interop Boundaries.
- A failure-mode note that explains the sharp edges inside Go Interop Boundaries.
...
- [ ] The main language-level risk in Go Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend Go Interop Boundaries without reverse-engineering hidden contracts.
