---
name: Performance Triage Minimal Repros
trigger: performance triage minimal repros, help with performance triage minimal repros, plan performance triage minimal repros, improve performance triage minimal repros, expert performance triage minimal repros
description: Expert-level guidance for performance triage minimal repros with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Minimal Repros is an expert coding workflow skill for Performance Triage. The focal concern is Minimal Repros, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Minimal Repros is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Minimal Repros is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Minimal Repros would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Minimal Repros create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Minimal Repros to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Minimal Repros.
...
- [ ] The evidence supporting Performance Triage Minimal Repros is stronger than the assumptions.
