---
name: Django Rate Limit Design
trigger: django rate limit design, help with django rate limit design, plan django rate limit design, improve django rate limit design, expert django rate limit design
description: Expert-level guidance for django rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Django Rate Limit Design is an expert backend skill for Django. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Django Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Django Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Django Rate Limit Design?
- Where does Django Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Django Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by Django Rate Limit Design even if it was never written down?
...
- A contract note for Django Rate Limit Design that includes failure semantics.
...
- [ ] The contract for Django Rate Limit Design is explicit about success and failure behavior.
