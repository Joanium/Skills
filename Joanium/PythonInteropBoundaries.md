---
name: Python Interop Boundaries
trigger: python interop boundaries, help with python interop boundaries, plan python interop boundaries, improve python interop boundaries, expert python interop boundaries
description: Expert-level guidance for python interop boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Python Interop Boundaries is an expert programming-language skill for Python. The focal concern is Interop Boundaries, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Python Interop Boundaries affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Python Interop Boundaries?
- Which abstraction in Python Interop Boundaries becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Python Interop Boundaries first?
- How will Python Interop Boundaries behave when multiple contributors evolve it independently?
- Which part of Python Interop Boundaries is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Python Interop Boundaries.
- A failure-mode note that explains the sharp edges inside Python Interop Boundaries.
...
- [ ] The main language-level risk in Python Interop Boundaries is named explicitly.
...
- [ ] Future contributors can extend Python Interop Boundaries without reverse-engineering hidden contracts.
