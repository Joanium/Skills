---
name: Dependency Upgrade Trace Isolation
trigger: dependency upgrade trace isolation, help with dependency upgrade trace isolation, plan dependency upgrade trace isolation, improve dependency upgrade trace isolation, expert dependency upgrade trace isolation
description: Expert-level guidance for dependency upgrade trace isolation with action-oriented workflow, review criteria, and failure patterns.
---

Dependency Upgrade Trace Isolation is an expert coding workflow skill for Dependency Upgrade. The focal concern is Trace Isolation, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Dependency Upgrade Trace Isolation is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Dependency Upgrade Trace Isolation is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Dependency Upgrade Trace Isolation would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Dependency Upgrade Trace Isolation create a silent regression that tests may miss?
- How would the AI explain the safety of Dependency Upgrade Trace Isolation to a skeptical reviewer?
...
- A concise evidence note for Dependency Upgrade Trace Isolation.
...
- [ ] The evidence supporting Dependency Upgrade Trace Isolation is stronger than the assumptions.
