---
name: Kotlin Package Publishing
trigger: kotlin package publishing, help with kotlin package publishing, plan kotlin package publishing, improve kotlin package publishing, expert kotlin package publishing
description: Expert-level guidance for kotlin package publishing with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kotlin Package Publishing is an expert programming-language skill for Kotlin. The focal concern is Package Publishing, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Kotlin Package Publishing affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Kotlin Package Publishing?
- Which abstraction in Kotlin Package Publishing becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Kotlin Package Publishing first?
- How will Kotlin Package Publishing behave when multiple contributors evolve it independently?
- Which part of Kotlin Package Publishing is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Kotlin Package Publishing.
- A failure-mode note that explains the sharp edges inside Kotlin Package Publishing.
...
- [ ] The main language-level risk in Kotlin Package Publishing is named explicitly.
...
- [ ] Future contributors can extend Kotlin Package Publishing without reverse-engineering hidden contracts.
