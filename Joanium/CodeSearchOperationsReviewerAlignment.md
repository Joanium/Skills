---
name: Code Search Operations Reviewer Alignment
trigger: code search operations reviewer alignment, help with code search operations reviewer alignment, plan code search operations reviewer alignment, improve code search operations reviewer alignment, expert code search operations reviewer alignment
description: Expert-level guidance for code search operations reviewer alignment with action-oriented workflow, review criteria, and failure patterns.
---

Code Search Operations Reviewer Alignment is an expert repository and delivery skill for Code Search Operations. The focal concern is Reviewer Alignment, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Code Search Operations Reviewer Alignment determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Code Search Operations Reviewer Alignment riskier than the diff itself suggests?
- Which reviewer would challenge Code Search Operations Reviewer Alignment first, and what evidence would they need?
- How could Code Search Operations Reviewer Alignment fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Code Search Operations Reviewer Alignment would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Code Search Operations Reviewer Alignment concretely?
...
- A blast-radius note for Code Search Operations Reviewer Alignment.
...
- [ ] The repository-level blast radius of Code Search Operations Reviewer Alignment is explicit.
