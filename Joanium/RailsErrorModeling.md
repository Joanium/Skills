---
name: Rails Error Modeling
trigger: rails error modeling, help with rails error modeling, plan rails error modeling, improve rails error modeling, expert rails error modeling
description: Expert-level guidance for rails error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rails Error Modeling is an expert backend skill for Rails. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Rails Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Rails Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Rails Error Modeling?
- Where does Rails Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Rails Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by Rails Error Modeling even if it was never written down?
...
- A contract note for Rails Error Modeling that includes failure semantics.
...
- [ ] The contract for Rails Error Modeling is explicit about success and failure behavior.
