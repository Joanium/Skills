---
name: Event-Driven Systems Scaling Paths
trigger: event-driven systems scaling paths, help with event-driven systems scaling paths, plan event-driven systems scaling paths, improve event-driven systems scaling paths, expert event-driven systems scaling paths
description: Expert-level guidance for event-driven systems scaling paths with action-oriented workflow, review criteria, and failure patterns.
---

Event-Driven Systems Scaling Paths is an expert architecture and systems skill for Event-Driven Systems. The focal concern is Scaling Paths, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Event-Driven Systems Scaling Paths needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Event-Driven Systems Scaling Paths becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Event-Driven Systems Scaling Paths is currently cheap only because the bill has not arrived yet?
- How would Event-Driven Systems Scaling Paths be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Event-Driven Systems Scaling Paths too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Event-Driven Systems Scaling Paths.
...
- [ ] The key boundaries inside Event-Driven Systems Scaling Paths are explicit and justified.
