---
name: Remix Error Recovery UX
trigger: remix error recovery ux, help with remix error recovery ux, plan remix error recovery ux, improve remix error recovery ux, expert remix error recovery ux
description: Expert-level guidance for remix error recovery ux with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Remix Error Recovery UX is an expert frontend skill for Remix. The focal concern is Error Recovery UX, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Remix Error Recovery UX affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Remix Error Recovery UX make user state harder to reason about than it needs to be?
- Which interaction in Remix Error Recovery UX degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Remix Error Recovery UX would break if the DOM or state changed unexpectedly?
- How does Remix Error Recovery UX fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Remix Error Recovery UX looks correct in a mock but collapses under production behavior?
...
- A state transition map for Remix Error Recovery UX.
...
- [ ] The state model for Remix Error Recovery UX is clear enough to debug.
...
- [ ] Another engineer can extend Remix Error Recovery UX without guessing hidden UI contracts.
