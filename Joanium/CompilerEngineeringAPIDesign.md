---
name: Compiler Engineering API Design
trigger: compiler engineering api design, help with compiler engineering api design, plan compiler engineering api design, improve compiler engineering api design, expert compiler engineering api design
description: Expert-level guidance for compiler engineering api design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Compiler Engineering API Design is an expert systems skill for Compiler Engineering. The focal concern is API Design, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Compiler Engineering API Design is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Compiler Engineering API Design would break first under scale, timing, or hardware stress?
- Which state or resource in Compiler Engineering API Design is hardest to observe when behavior goes wrong?
- How does Compiler Engineering API Design trade correctness, throughput, and recoverability today?
- What tooling gap makes Compiler Engineering API Design harder to reason about than it should be?
- Which optimization target inside Compiler Engineering API Design is attractive but strategically secondary?
...
- A system model for Compiler Engineering API Design that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Compiler Engineering API Design is explicit.
