---
name: Evaluation Engineering Trace Analysis
trigger: evaluation engineering trace analysis, help with evaluation engineering trace analysis, plan evaluation engineering trace analysis, improve evaluation engineering trace analysis, expert evaluation engineering trace analysis
description: Expert-level guidance for evaluation engineering trace analysis with action-oriented workflow, review criteria, and failure patterns.
---

Evaluation Engineering Trace Analysis is an expert applied AI skill for Evaluation Engineering. The focal concern is Trace Analysis, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Evaluation Engineering Trace Analysis affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Evaluation Engineering Trace Analysis is currently measured by vibes rather than a real contract?
- Which failure in Evaluation Engineering Trace Analysis would be hardest to notice before it harms users or operators?
- How will Evaluation Engineering Trace Analysis behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Evaluation Engineering Trace Analysis?
...
- A behavior contract for Evaluation Engineering Trace Analysis.
...
- [ ] The contract behind Evaluation Engineering Trace Analysis is explicit enough to evaluate.
