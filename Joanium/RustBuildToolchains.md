---
name: Rust Build Toolchains
trigger: rust build toolchains, help with rust build toolchains, plan rust build toolchains, improve rust build toolchains, expert rust build toolchains
description: Expert-level guidance for rust build toolchains with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rust Build Toolchains is an expert programming-language skill for Rust. The focal concern is Build Toolchains, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Rust Build Toolchains affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Rust Build Toolchains?
- Which abstraction in Rust Build Toolchains becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Rust Build Toolchains first?
- How will Rust Build Toolchains behave when multiple contributors evolve it independently?
- Which part of Rust Build Toolchains is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Rust Build Toolchains.
- A failure-mode note that explains the sharp edges inside Rust Build Toolchains.
...
- [ ] The main language-level risk in Rust Build Toolchains is named explicitly.
...
- [ ] Future contributors can extend Rust Build Toolchains without reverse-engineering hidden contracts.
