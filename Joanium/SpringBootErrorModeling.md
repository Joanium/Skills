---
name: Spring Boot Error Modeling
trigger: spring boot error modeling, help with spring boot error modeling, plan spring boot error modeling, improve spring boot error modeling, expert spring boot error modeling
description: Expert-level guidance for spring boot error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Spring Boot Error Modeling is an expert backend skill for Spring Boot. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Spring Boot Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Spring Boot Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Spring Boot Error Modeling?
- Where does Spring Boot Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Spring Boot Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by Spring Boot Error Modeling even if it was never written down?
...
- A contract note for Spring Boot Error Modeling that includes failure semantics.
...
- [ ] The contract for Spring Boot Error Modeling is explicit about success and failure behavior.
