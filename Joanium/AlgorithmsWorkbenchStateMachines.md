---
name: Algorithms Workbench State Machines
trigger: algorithms workbench state machines, help with algorithms workbench state machines, plan algorithms workbench state machines, improve algorithms workbench state machines, expert algorithms workbench state machines
description: Expert-level guidance for algorithms workbench state machines with action-oriented workflow, review criteria, and failure patterns.
---

Algorithms Workbench State Machines is an expert systems and deep-engineering skill for Algorithms Workbench. The focal concern is State Machines, but the real objective is helping an AI reason about correctness, performance, and failure in technical domains where shallow intuition breaks down.

## When To Use

- Use this when Algorithms Workbench State Machines involves constraints where correctness, timing, memory, or physical behavior dominate the design.
...
- Which resource or state boundary inside Algorithms Workbench State Machines is least observable today?
- What hidden assumption in Algorithms Workbench State Machines would break first under load, timing pressure, or hardware limits?
- How would the AI distinguish a correctness issue from a performance symptom in Algorithms Workbench State Machines?
- What tooling gap makes Algorithms Workbench State Machines harder to debug or verify than it should be?
- Which optimization target in Algorithms Workbench State Machines is attractive but strategically secondary?
...
- A system model for Algorithms Workbench State Machines naming resources, states, and key constraints.
...
- [ ] The operating envelope for Algorithms Workbench State Machines is explicit.
