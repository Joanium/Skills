---
name: OAuth Attack Simulation
trigger: oauth attack simulation, help with oauth attack simulation, plan oauth attack simulation, improve oauth attack simulation, expert oauth attack simulation
description: Expert-level guidance for oauth attack simulation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

OAuth Attack Simulation is an expert security skill for OAuth. The focal concern is Attack Simulation, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when OAuth Attack Simulation influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in OAuth Attack Simulation first?
- Which exception or emergency path makes OAuth Attack Simulation less true in practice than in policy?
- What evidence would be missing if OAuth Attack Simulation failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does OAuth Attack Simulation degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for OAuth Attack Simulation.
...
- [ ] The attacker model for OAuth Attack Simulation is realistic and current.
...
- [ ] The team can explain how OAuth Attack Simulation fails and how it recovers.
