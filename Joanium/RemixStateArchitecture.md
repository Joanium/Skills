---
name: Remix State Architecture
trigger: remix state architecture, help with remix state architecture, plan remix state architecture, improve remix state architecture, expert remix state architecture
description: Expert-level guidance for remix state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Remix State Architecture is an expert frontend skill for Remix. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Remix State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Remix State Architecture make user state harder to reason about than it needs to be?
- Which interaction in Remix State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Remix State Architecture would break if the DOM or state changed unexpectedly?
- How does Remix State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Remix State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for Remix State Architecture.
...
- [ ] The state model for Remix State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend Remix State Architecture without guessing hidden UI contracts.
