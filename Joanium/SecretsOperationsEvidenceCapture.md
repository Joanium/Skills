---
name: Secrets Operations Evidence Capture
trigger: secrets operations evidence capture, help with secrets operations evidence capture, plan secrets operations evidence capture, improve secrets operations evidence capture, expert secrets operations evidence capture
description: Expert-level guidance for secrets operations evidence capture with action-oriented workflow, review criteria, and failure patterns.
---

Secrets Operations Evidence Capture is an expert platform and automation skill for Secrets Operations. The focal concern is Evidence Capture, but the real objective is helping an AI make infrastructure and operational change without creating invisible blast radius.

## When To Use

- Use this when Secrets Operations Evidence Capture can affect deployment safety, service recovery, or operator trust in automation.
...
- What part of Secrets Operations Evidence Capture could fail silently while the automation still reports success?
- Which rollback or recovery step inside Secrets Operations Evidence Capture depends too much on tribal memory?
- How will operators know when Secrets Operations Evidence Capture should stop, wait, or reverse course?
- Where does Secrets Operations Evidence Capture blur access, policy, and execution boundaries dangerously?
...
- A change-and-recovery map for Secrets Operations Evidence Capture.
...
- [ ] The rollback model for Secrets Operations Evidence Capture is as real as the forward path.
