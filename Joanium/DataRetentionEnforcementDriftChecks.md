---
name: Data Retention Enforcement Drift Checks
trigger: data retention enforcement drift checks, help with data retention enforcement drift checks, plan data retention enforcement drift checks, improve data retention enforcement drift checks, expert data retention enforcement drift checks
description: Expert-level guidance for data retention enforcement drift checks with action-oriented workflow, review criteria, and failure patterns.
---

Data Retention Enforcement Drift Checks is an expert database and migration skill for Data Retention Enforcement. The focal concern is Drift Checks, but the real objective is helping an AI change data structures and access paths without breaking trust, cost, or recovery.

## When To Use

- Use this when Data Retention Enforcement Drift Checks is changing a data surface that other people or systems depend on semantically, not just structurally.
...
- Which consumer of Data Retention Enforcement Drift Checks would be harmed first by semantic drift rather than outright failure?
- What recovery move for Data Retention Enforcement Drift Checks becomes hardest after the cutover begins?
- How will Data Retention Enforcement Drift Checks be validated if the data looks plausible but is subtly wrong?
- Where could Data Retention Enforcement Drift Checks create hidden cost spikes while technically succeeding?
- Which ownership boundary in Data Retention Enforcement Drift Checks is currently too vague for safe change management?
...
- A cutover and recovery plan for Data Retention Enforcement Drift Checks.
...
- [ ] The meaning of Data Retention Enforcement Drift Checks is clearer than the implementation steps.
