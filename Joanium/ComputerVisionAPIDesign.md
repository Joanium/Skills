---
name: Computer Vision API Design
trigger: computer vision api design, help with computer vision api design, plan computer vision api design, improve computer vision api design, expert computer vision api design
description: Expert-level guidance for computer vision api design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Computer Vision API Design is an expert systems skill for Computer Vision. The focal concern is API Design, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Computer Vision API Design is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Computer Vision API Design would break first under scale, timing, or hardware stress?
- Which state or resource in Computer Vision API Design is hardest to observe when behavior goes wrong?
- How does Computer Vision API Design trade correctness, throughput, and recoverability today?
- What tooling gap makes Computer Vision API Design harder to reason about than it should be?
- Which optimization target inside Computer Vision API Design is attractive but strategically secondary?
...
- A system model for Computer Vision API Design that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Computer Vision API Design is explicit.
