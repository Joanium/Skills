---
name: Legacy Refactor Rollback Plans
trigger: legacy refactor rollback plans, help with legacy refactor rollback plans, plan legacy refactor rollback plans, improve legacy refactor rollback plans, expert legacy refactor rollback plans
description: Expert-level guidance for legacy refactor rollback plans with action-oriented workflow, review criteria, and failure patterns.
---

Legacy Refactor Rollback Plans is an expert coding workflow skill for Legacy Refactor. The focal concern is Rollback Plans, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Legacy Refactor Rollback Plans is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Legacy Refactor Rollback Plans is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Legacy Refactor Rollback Plans would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Legacy Refactor Rollback Plans create a silent regression that tests may miss?
- How would the AI explain the safety of Legacy Refactor Rollback Plans to a skeptical reviewer?
...
- A concise evidence note for Legacy Refactor Rollback Plans.
...
- [ ] The evidence supporting Legacy Refactor Rollback Plans is stronger than the assumptions.
