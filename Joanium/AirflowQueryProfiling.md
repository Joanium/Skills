---
name: Airflow Query Profiling
trigger: airflow query profiling, help with airflow query profiling, plan airflow query profiling, improve airflow query profiling, expert airflow query profiling
description: Expert-level guidance for airflow query profiling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Airflow Query Profiling is an expert data-systems skill for Airflow. The focal concern is Query Profiling, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when Airflow Query Profiling affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would Airflow Query Profiling create if two teams interpreted it differently?
- Which recovery path for Airflow Query Profiling is the least understood today?
- How will Airflow Query Profiling behave under backfills, reprocessing, or delayed sources?
- What cost pattern in Airflow Query Profiling is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside Airflow Query Profiling drifted subtly?
...
- A semantic contract note for Airflow Query Profiling.
...
- [ ] The meaning of Airflow Query Profiling is written in language consumers will actually use.
