---
name: Swift Build Toolchains
trigger: swift build toolchains, help with swift build toolchains, plan swift build toolchains, improve swift build toolchains, expert swift build toolchains
description: Expert-level guidance for swift build toolchains with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Swift Build Toolchains is an expert programming-language skill for Swift. The focal concern is Build Toolchains, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Swift Build Toolchains affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Swift Build Toolchains?
- Which abstraction in Swift Build Toolchains becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Swift Build Toolchains first?
- How will Swift Build Toolchains behave when multiple contributors evolve it independently?
- Which part of Swift Build Toolchains is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Swift Build Toolchains.
- A failure-mode note that explains the sharp edges inside Swift Build Toolchains.
...
- [ ] The main language-level risk in Swift Build Toolchains is named explicitly.
...
- [ ] Future contributors can extend Swift Build Toolchains without reverse-engineering hidden contracts.
