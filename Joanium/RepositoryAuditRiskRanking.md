---
name: Repository Audit Risk Ranking
trigger: repository audit risk ranking, help with repository audit risk ranking, plan repository audit risk ranking, improve repository audit risk ranking, expert repository audit risk ranking
description: Expert-level guidance for repository audit risk ranking with action-oriented workflow, review criteria, and failure patterns.
---

Repository Audit Risk Ranking is an expert coding workflow skill for Repository Audit. The focal concern is Risk Ranking, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Repository Audit Risk Ranking is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Repository Audit Risk Ranking is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Repository Audit Risk Ranking would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Repository Audit Risk Ranking create a silent regression that tests may miss?
- How would the AI explain the safety of Repository Audit Risk Ranking to a skeptical reviewer?
...
- A concise evidence note for Repository Audit Risk Ranking.
...
- [ ] The evidence supporting Repository Audit Risk Ranking is stronger than the assumptions.
