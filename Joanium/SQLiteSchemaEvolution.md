---
name: SQLite Schema Evolution
trigger: sqlite schema evolution, help with sqlite schema evolution, plan sqlite schema evolution, improve sqlite schema evolution, expert sqlite schema evolution
description: Expert-level guidance for sqlite schema evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

SQLite Schema Evolution is an expert data-systems skill for SQLite. The focal concern is Schema Evolution, but the real bar is trustworthy data that survives scale, handoffs, and semantic drift.

## When To Use

- Use this when SQLite Schema Evolution affects a shared data surface where semantics, freshness, or cost matter as much as raw correctness.
...
- What semantic confusion would SQLite Schema Evolution create if two teams interpreted it differently?
- Which recovery path for SQLite Schema Evolution is the least understood today?
- How will SQLite Schema Evolution behave under backfills, reprocessing, or delayed sources?
- What cost pattern in SQLite Schema Evolution is growing silently because no one owns the boundary?
- Which consumer would fail first if the data contract inside SQLite Schema Evolution drifted subtly?
...
- A semantic contract note for SQLite Schema Evolution.
...
- [ ] The meaning of SQLite Schema Evolution is written in language consumers will actually use.
