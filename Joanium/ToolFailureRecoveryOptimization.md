---
name: Tool Failure Recovery Optimization
trigger: tool failure recovery optimization, help with tool failure recovery optimization, plan tool failure recovery optimization, improve tool failure recovery optimization, expert tool failure recovery optimization
description: Expert-level guidance for tool failure recovery optimization, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Tool Failure Recovery Optimization is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is improving the limiting resource without merely moving cost, fragility, or risk somewhere else.

## When To Use

- Use this when Tool Failure Recovery Optimization influences user-visible output, automation, or safety-sensitive decisions.
...
- What is the actual bottleneck in Tool Failure Recovery Optimization: CPU, memory, latency, network, human time, or policy friction?
...
- A baseline report for Tool Failure Recovery Optimization with representative workloads.
