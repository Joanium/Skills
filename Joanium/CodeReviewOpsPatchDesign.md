---
name: Code Review Operations Patch Design
trigger: code review operations patch design, help with code review operations patch design, plan code review operations patch design, improve code review operations patch design, expert code review operations patch design
description: Expert-level guidance for code review operations patch design with action-oriented workflow, review criteria, and failure patterns.
---

Code Review Operations Patch Design is an expert coding workflow skill for Code Review Operations. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Code Review Operations Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Code Review Operations Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Code Review Operations Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Code Review Operations Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Code Review Operations Patch Design to a skeptical reviewer?
...
- A concise evidence note for Code Review Operations Patch Design.
...
- [ ] The evidence supporting Code Review Operations Patch Design is stronger than the assumptions.
