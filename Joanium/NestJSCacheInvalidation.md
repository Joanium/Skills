---
name: NestJS Cache Invalidation
trigger: nestjs cache invalidation, help with nestjs cache invalidation, plan nestjs cache invalidation, improve nestjs cache invalidation, expert nestjs cache invalidation
description: Expert-level guidance for nestjs cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

NestJS Cache Invalidation is an expert backend skill for NestJS. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when NestJS Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside NestJS Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in NestJS Cache Invalidation?
- Where does NestJS Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether NestJS Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by NestJS Cache Invalidation even if it was never written down?
...
- A contract note for NestJS Cache Invalidation that includes failure semantics.
...
- [ ] The contract for NestJS Cache Invalidation is explicit about success and failure behavior.
