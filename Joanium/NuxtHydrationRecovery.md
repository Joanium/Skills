---
name: Nuxt Hydration Recovery
trigger: nuxt hydration recovery, help with nuxt hydration recovery, plan nuxt hydration recovery, improve nuxt hydration recovery, expert nuxt hydration recovery
description: Expert-level guidance for nuxt hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Nuxt Hydration Recovery is an expert frontend skill for Nuxt. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Nuxt Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Nuxt Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in Nuxt Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Nuxt Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does Nuxt Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Nuxt Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for Nuxt Hydration Recovery.
...
- [ ] The state model for Nuxt Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend Nuxt Hydration Recovery without guessing hidden UI contracts.
