---
name: Next.js State Architecture
trigger: next.js state architecture, help with next.js state architecture, plan next.js state architecture, improve next.js state architecture, expert next.js state architecture
description: Expert-level guidance for next.js state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Next.js State Architecture is an expert frontend skill for Next.js. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Next.js State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Next.js State Architecture make user state harder to reason about than it needs to be?
- Which interaction in Next.js State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Next.js State Architecture would break if the DOM or state changed unexpectedly?
- How does Next.js State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Next.js State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for Next.js State Architecture.
...
- [ ] The state model for Next.js State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend Next.js State Architecture without guessing hidden UI contracts.
