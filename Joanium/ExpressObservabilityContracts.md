---
name: Express Observability Contracts
trigger: express observability contracts, help with express observability contracts, plan express observability contracts, improve express observability contracts, expert express observability contracts
description: Expert-level guidance for express observability contracts with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Express Observability Contracts is an expert backend skill for Express. The focal concern is Observability Contracts, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Express Observability Contracts shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Express Observability Contracts breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Express Observability Contracts?
- Where does Express Observability Contracts need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Express Observability Contracts is unhealthy before customers do?
- What compatibility promise is implied by Express Observability Contracts even if it was never written down?
...
- A contract note for Express Observability Contracts that includes failure semantics.
...
- [ ] The contract for Express Observability Contracts is explicit about success and failure behavior.
