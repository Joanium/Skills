---
name: Model Fallback Routing Escalation Rules
trigger: model fallback routing escalation rules, help with model fallback routing escalation rules, plan model fallback routing escalation rules, improve model fallback routing escalation rules, expert model fallback routing escalation rules
description: Expert-level guidance for model fallback routing escalation rules with action-oriented workflow, review criteria, and failure patterns.
---

Model Fallback Routing Escalation Rules is an expert agent and tooling skill for Model Fallback Routing. The focal concern is Escalation Rules, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Model Fallback Routing Escalation Rules affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Model Fallback Routing Escalation Rules is still governed by intuition instead of a contract or label set?
- Which failure in Model Fallback Routing Escalation Rules would be hardest to detect before it compounds?
- How would Model Fallback Routing Escalation Rules behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Model Fallback Routing Escalation Rules?
...
- A contract and failure-label set for Model Fallback Routing Escalation Rules.
...
- [ ] The system behind Model Fallback Routing Escalation Rules is described more clearly than the prompt surface.
