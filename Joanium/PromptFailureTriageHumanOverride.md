---
name: Prompt Failure Triage Human Override
trigger: prompt failure triage human override, help with prompt failure triage human override, plan prompt failure triage human override, improve prompt failure triage human override, expert prompt failure triage human override
description: Expert-level guidance for prompt failure triage human override with action-oriented workflow, review criteria, and failure patterns.
---

Prompt Failure Triage Human Override is an expert agent and tooling skill for Prompt Failure Triage. The focal concern is Human Override, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Prompt Failure Triage Human Override affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Prompt Failure Triage Human Override is still governed by intuition instead of a contract or label set?
- Which failure in Prompt Failure Triage Human Override would be hardest to detect before it compounds?
- How would Prompt Failure Triage Human Override behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Prompt Failure Triage Human Override?
...
- A contract and failure-label set for Prompt Failure Triage Human Override.
...
- [ ] The system behind Prompt Failure Triage Human Override is described more clearly than the prompt surface.
