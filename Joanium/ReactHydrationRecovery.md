---
name: React Hydration Recovery
trigger: react hydration recovery, help with react hydration recovery, plan react hydration recovery, improve react hydration recovery, expert react hydration recovery
description: Expert-level guidance for react hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

React Hydration Recovery is an expert frontend skill for React. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when React Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does React Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in React Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in React Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does React Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of React Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for React Hydration Recovery.
...
- [ ] The state model for React Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend React Hydration Recovery without guessing hidden UI contracts.
