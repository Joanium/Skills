---
name: Astro Rendering Performance
trigger: astro rendering performance, help with astro rendering performance, plan astro rendering performance, improve astro rendering performance, expert astro rendering performance
description: Expert-level guidance for astro rendering performance with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Astro Rendering Performance is an expert frontend skill for Astro. The focal concern is Rendering Performance, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Astro Rendering Performance affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Astro Rendering Performance make user state harder to reason about than it needs to be?
- Which interaction in Astro Rendering Performance degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Astro Rendering Performance would break if the DOM or state changed unexpectedly?
- How does Astro Rendering Performance fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Astro Rendering Performance looks correct in a mock but collapses under production behavior?
...
- A state transition map for Astro Rendering Performance.
...
- [ ] The state model for Astro Rendering Performance is clear enough to debug.
...
- [ ] Another engineer can extend Astro Rendering Performance without guessing hidden UI contracts.
