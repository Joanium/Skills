---
name: Performance Triage Patch Design
trigger: performance triage patch design, help with performance triage patch design, plan performance triage patch design, improve performance triage patch design, expert performance triage patch design
description: Expert-level guidance for performance triage patch design with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Patch Design is an expert coding workflow skill for Performance Triage. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Patch Design to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Patch Design.
...
- [ ] The evidence supporting Performance Triage Patch Design is stronger than the assumptions.
