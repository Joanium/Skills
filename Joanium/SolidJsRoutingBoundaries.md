---
name: SolidJS Routing Boundaries
trigger: solidjs routing boundaries, help with solidjs routing boundaries, plan solidjs routing boundaries, improve solidjs routing boundaries, expert solidjs routing boundaries
description: Expert-level guidance for solidjs routing boundaries with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

SolidJS Routing Boundaries is an expert frontend skill for SolidJS. The focal concern is Routing Boundaries, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when SolidJS Routing Boundaries affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does SolidJS Routing Boundaries make user state harder to reason about than it needs to be?
- Which interaction in SolidJS Routing Boundaries degrades first under latency, stale data, or partial failure?
- What accessibility behavior in SolidJS Routing Boundaries would break if the DOM or state changed unexpectedly?
- How does SolidJS Routing Boundaries fail when routing, hydration, or rendering order becomes non-ideal?
- What part of SolidJS Routing Boundaries looks correct in a mock but collapses under production behavior?
...
- A state transition map for SolidJS Routing Boundaries.
...
- [ ] The state model for SolidJS Routing Boundaries is clear enough to debug.
...
- [ ] Another engineer can extend SolidJS Routing Boundaries without guessing hidden UI contracts.
