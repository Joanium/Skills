---
name: Evaluation Harnesses Failure Analysis
trigger: evaluation harnesses failure analysis, help with evaluation harnesses failure analysis, plan evaluation harnesses failure analysis, improve evaluation harnesses failure analysis, expert evaluation harnesses failure analysis
description: Expert-level guidance for evaluation harnesses failure analysis with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Evaluation Harnesses Failure Analysis is an expert applied-AI skill for Evaluation Harnesses. The focal concern is Failure Analysis, but the real bar is production behavior that is measurable, reviewable, and worth its cost.

## When To Use

- Use this when Evaluation Harnesses Failure Analysis affects model-assisted output that users, reviewers, or downstream systems will trust.
...
- What behavior in Evaluation Harnesses Failure Analysis would look acceptable in a demo but fail under production ambiguity?
- Which part of Evaluation Harnesses Failure Analysis lacks a measurable contract: inputs, outputs, tools, or review criteria?
- How will Evaluation Harnesses Failure Analysis fail when latency, cost, retrieval quality, or tool behavior shifts?
- What evidence will prove that Evaluation Harnesses Failure Analysis is getting better rather than merely changing?
- Where should humans intervene in Evaluation Harnesses Failure Analysis to add leverage instead of toil?
...
- A behavior contract for Evaluation Harnesses Failure Analysis.
...
- [ ] The behavior contract for Evaluation Harnesses Failure Analysis is explicit.
