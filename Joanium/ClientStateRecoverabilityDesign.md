---
name: Client State Recoverability Design
trigger: client state recoverability design, help with client state recoverability design, plan client state recoverability design, improve client state recoverability design, expert client state recoverability design
description: Expert-level guidance for client state recoverability design, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

Client State Recoverability Design is an expert-level frontend skill for making client behavior understandable, resilient, and consistent across design and engineering. The emphasis here is on interfaces, invariants, failure handling, and why one structure is safer than its alternatives.

## When To Use

- Use this when Client State Recoverability Design shapes user trust through responsiveness, clarity, or failure handling.
...
- What are the core boundaries, contracts, and invariants in Client State Recoverability Design?
...
- An architecture or boundary diagram for Client State Recoverability Design.
