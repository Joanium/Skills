---
name: Synthetic Dataset Design Failure Taxonomy
trigger: synthetic dataset design failure taxonomy, help with synthetic dataset design failure taxonomy, plan synthetic dataset design failure taxonomy, improve synthetic dataset design failure taxonomy, expert synthetic dataset design failure taxonomy
description: Expert-level guidance for synthetic dataset design failure taxonomy with action-oriented workflow, review criteria, and failure patterns.
---

Synthetic Dataset Design Failure Taxonomy is an expert applied AI skill for Synthetic Dataset Design. The focal concern is Failure Taxonomy, but the real objective is helping an AI system behave usefully in production where cost, latency, ambiguity, and safety all matter at once.

## When To Use

- Use this when Synthetic Dataset Design Failure Taxonomy affects how model-assisted behavior will actually work once users, tools, and ambiguity are involved.
...
- What part of Synthetic Dataset Design Failure Taxonomy is currently measured by vibes rather than a real contract?
- Which failure in Synthetic Dataset Design Failure Taxonomy would be hardest to notice before it harms users or operators?
- How will Synthetic Dataset Design Failure Taxonomy behave if tools are slow, retrieval is weak, or prompts drift over time?
- What should escalate to humans because the AI is structurally weak there, not just temporarily uncertain?
- How will the next model or prompt change be attributed cleanly inside Synthetic Dataset Design Failure Taxonomy?
...
- A behavior contract for Synthetic Dataset Design Failure Taxonomy.
...
- [ ] The contract behind Synthetic Dataset Design Failure Taxonomy is explicit enough to evaluate.
