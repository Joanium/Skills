---
name: gRPC Error Modeling
trigger: grpc error modeling, help with grpc error modeling, plan grpc error modeling, improve grpc error modeling, expert grpc error modeling
description: Expert-level guidance for grpc error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

gRPC Error Modeling is an expert backend skill for gRPC. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when gRPC Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside gRPC Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in gRPC Error Modeling?
- Where does gRPC Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether gRPC Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by gRPC Error Modeling even if it was never written down?
...
- A contract note for gRPC Error Modeling that includes failure semantics.
...
- [ ] The contract for gRPC Error Modeling is explicit about success and failure behavior.
