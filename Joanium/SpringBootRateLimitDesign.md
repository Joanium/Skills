---
name: Spring Boot Rate Limit Design
trigger: spring boot rate limit design, help with spring boot rate limit design, plan spring boot rate limit design, improve spring boot rate limit design, expert spring boot rate limit design
description: Expert-level guidance for spring boot rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Spring Boot Rate Limit Design is an expert backend skill for Spring Boot. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Spring Boot Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Spring Boot Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Spring Boot Rate Limit Design?
- Where does Spring Boot Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Spring Boot Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by Spring Boot Rate Limit Design even if it was never written down?
...
- A contract note for Spring Boot Rate Limit Design that includes failure semantics.
...
- [ ] The contract for Spring Boot Rate Limit Design is explicit about success and failure behavior.
