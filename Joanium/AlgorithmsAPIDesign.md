---
name: Algorithms API Design
trigger: algorithms api design, help with algorithms api design, plan algorithms api design, improve algorithms api design, expert algorithms api design
description: Expert-level guidance for algorithms api design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Algorithms API Design is an expert systems skill for Algorithms. The focal concern is API Design, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Algorithms API Design is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Algorithms API Design would break first under scale, timing, or hardware stress?
- Which state or resource in Algorithms API Design is hardest to observe when behavior goes wrong?
- How does Algorithms API Design trade correctness, throughput, and recoverability today?
- What tooling gap makes Algorithms API Design harder to reason about than it should be?
- Which optimization target inside Algorithms API Design is attractive but strategically secondary?
...
- A system model for Algorithms API Design that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Algorithms API Design is explicit.
