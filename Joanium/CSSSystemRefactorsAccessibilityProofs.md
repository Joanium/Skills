---
name: CSS System Refactors Accessibility Proofs
trigger: css system refactors accessibility proofs, help with css system refactors accessibility proofs, plan css system refactors accessibility proofs, improve css system refactors accessibility proofs, expert css system refactors accessibility proofs
description: Expert-level guidance for css system refactors accessibility proofs with action-oriented workflow, review criteria, and failure patterns.
---

CSS System Refactors Accessibility Proofs is an expert frontend implementation skill for CSS System Refactors. The focal concern is Accessibility Proofs, but the real objective is helping an AI create UI behavior that stays understandable when state, rendering, latency, and devices stop cooperating.

## When To Use

- Use this when CSS System Refactors Accessibility Proofs affects whether a frontend change will remain coherent outside the happy path.
...
- What part of CSS System Refactors Accessibility Proofs becomes hardest to reason about when the UI is slow or inconsistent?
- Which state transition inside CSS System Refactors Accessibility Proofs would fail silently without stronger instrumentation?
- How does CSS System Refactors Accessibility Proofs behave when the browser, network, or input method stops being ideal?
- What implementation choice in CSS System Refactors Accessibility Proofs most threatens the original design intent?
- Which regression in CSS System Refactors Accessibility Proofs would users notice long before the team does?
...
- A state and edge-case map for CSS System Refactors Accessibility Proofs.
...
- [ ] The behavioral contract behind CSS System Refactors Accessibility Proofs is clearer than the implementation shortcuts.
