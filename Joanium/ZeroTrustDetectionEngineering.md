---
name: Zero Trust Detection Engineering
trigger: zero trust detection engineering, help with zero trust detection engineering, plan zero trust detection engineering, improve zero trust detection engineering, expert zero trust detection engineering
description: Expert-level guidance for zero trust detection engineering with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Zero Trust Detection Engineering is an expert security skill for Zero Trust. The focal concern is Detection Engineering, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when Zero Trust Detection Engineering influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in Zero Trust Detection Engineering first?
- Which exception or emergency path makes Zero Trust Detection Engineering less true in practice than in policy?
- What evidence would be missing if Zero Trust Detection Engineering failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does Zero Trust Detection Engineering degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for Zero Trust Detection Engineering.
...
- [ ] The attacker model for Zero Trust Detection Engineering is realistic and current.
...
- [ ] The team can explain how Zero Trust Detection Engineering fails and how it recovers.
