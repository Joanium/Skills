---
name: Spring Boot Cache Invalidation
trigger: spring boot cache invalidation, help with spring boot cache invalidation, plan spring boot cache invalidation, improve spring boot cache invalidation, expert spring boot cache invalidation
description: Expert-level guidance for spring boot cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Spring Boot Cache Invalidation is an expert backend skill for Spring Boot. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Spring Boot Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Spring Boot Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Spring Boot Cache Invalidation?
- Where does Spring Boot Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Spring Boot Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by Spring Boot Cache Invalidation even if it was never written down?
...
- A contract note for Spring Boot Cache Invalidation that includes failure semantics.
...
- [ ] The contract for Spring Boot Cache Invalidation is explicit about success and failure behavior.
