---
name: Service Decomposition Scaling Paths
trigger: service decomposition scaling paths, help with service decomposition scaling paths, plan service decomposition scaling paths, improve service decomposition scaling paths, expert service decomposition scaling paths
description: Expert-level guidance for service decomposition scaling paths with action-oriented workflow, review criteria, and failure patterns.
---

Service Decomposition Scaling Paths is an expert architecture and systems skill for Service Decomposition. The focal concern is Scaling Paths, but the real objective is helping an AI reason about boundaries, failure paths, and evolution before the system gets too expensive to change.

## When To Use

- Use this when Service Decomposition Scaling Paths needs stronger reasoning about boundaries, contracts, and failure before implementation momentum hides the real tradeoffs.
...
- What part of Service Decomposition Scaling Paths becomes ambiguous when the system is stressed, scaled, or reorganized?
- Which dependency in Service Decomposition Scaling Paths is currently cheap only because the bill has not arrived yet?
- How would Service Decomposition Scaling Paths be migrated if the current assumption proved wrong in six months?
- Where is ownership inside Service Decomposition Scaling Paths too fuzzy for operators or adjacent teams?
...
- A boundary and ownership diagram for Service Decomposition Scaling Paths.
...
- [ ] The key boundaries inside Service Decomposition Scaling Paths are explicit and justified.
