---
name: Exploratory QA Parallel Execution
trigger: exploratory qa parallel execution, help with exploratory qa parallel execution, plan exploratory qa parallel execution, improve exploratory qa parallel execution, expert exploratory qa parallel execution
description: Expert-level guidance for exploratory qa parallel execution with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Parallel Execution is an expert testing and quality skill for Exploratory QA. The focal concern is Parallel Execution, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Parallel Execution needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Parallel Execution is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Parallel Execution would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Parallel Execution depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Parallel Execution need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Parallel Execution.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Parallel Execution should mean.
...
- [ ] The key behavior protected by Exploratory QA Parallel Execution is explicit.
