---
name: Vue Hydration Recovery
trigger: vue hydration recovery, help with vue hydration recovery, plan vue hydration recovery, improve vue hydration recovery, expert vue hydration recovery
description: Expert-level guidance for vue hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Vue Hydration Recovery is an expert frontend skill for Vue. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Vue Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Vue Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in Vue Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Vue Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does Vue Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Vue Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for Vue Hydration Recovery.
...
- [ ] The state model for Vue Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend Vue Hydration Recovery without guessing hidden UI contracts.
