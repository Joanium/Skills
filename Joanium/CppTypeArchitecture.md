---
name: C++ Type Architecture
trigger: c++ type architecture, help with c++ type architecture, plan c++ type architecture, improve c++ type architecture, expert c++ type architecture
description: Expert-level guidance for c++ type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

C++ Type Architecture is an expert programming-language skill for C++. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when C++ Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside C++ Type Architecture?
- Which abstraction in C++ Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in C++ Type Architecture first?
- How will C++ Type Architecture behave when multiple contributors evolve it independently?
- Which part of C++ Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for C++ Type Architecture.
- A failure-mode note that explains the sharp edges inside C++ Type Architecture.
...
- [ ] The main language-level risk in C++ Type Architecture is named explicitly.
...
- [ ] Future contributors can extend C++ Type Architecture without reverse-engineering hidden contracts.
