---
name: gRPC Cache Invalidation
trigger: grpc cache invalidation, help with grpc cache invalidation, plan grpc cache invalidation, improve grpc cache invalidation, expert grpc cache invalidation
description: Expert-level guidance for grpc cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

gRPC Cache Invalidation is an expert backend skill for gRPC. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when gRPC Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside gRPC Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in gRPC Cache Invalidation?
- Where does gRPC Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether gRPC Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by gRPC Cache Invalidation even if it was never written down?
...
- A contract note for gRPC Cache Invalidation that includes failure semantics.
...
- [ ] The contract for gRPC Cache Invalidation is explicit about success and failure behavior.
