---
name: Flaky Test Repair Deterministic Timing
trigger: flaky test repair deterministic timing, help with flaky test repair deterministic timing, plan flaky test repair deterministic timing, improve flaky test repair deterministic timing, expert flaky test repair deterministic timing
description: Expert-level guidance for flaky test repair deterministic timing with action-oriented workflow, review criteria, and failure patterns.
---

Flaky Test Repair Deterministic Timing is an expert testing and quality skill for Flaky Test Repair. The focal concern is Deterministic Timing, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Flaky Test Repair Deterministic Timing needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Flaky Test Repair Deterministic Timing is still untested even if coverage looks healthy?
- Which assertion in Flaky Test Repair Deterministic Timing would stay green while the user-visible behavior regressed?
- How much of Flaky Test Repair Deterministic Timing depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Flaky Test Repair Deterministic Timing need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Flaky Test Repair Deterministic Timing.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Flaky Test Repair Deterministic Timing should mean.
...
- [ ] The key behavior protected by Flaky Test Repair Deterministic Timing is explicit.
