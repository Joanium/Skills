---
name: Zero Trust Control Architecture
trigger: zero trust control architecture, help with zero trust control architecture, plan zero trust control architecture, improve zero trust control architecture, expert zero trust control architecture
description: Expert-level guidance for zero trust control architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Zero Trust Control Architecture is an expert security skill for Zero Trust. The focal concern is Control Architecture, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when Zero Trust Control Architecture influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in Zero Trust Control Architecture first?
- Which exception or emergency path makes Zero Trust Control Architecture less true in practice than in policy?
- What evidence would be missing if Zero Trust Control Architecture failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does Zero Trust Control Architecture degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for Zero Trust Control Architecture.
...
- [ ] The attacker model for Zero Trust Control Architecture is realistic and current.
...
- [ ] The team can explain how Zero Trust Control Architecture fails and how it recovers.
