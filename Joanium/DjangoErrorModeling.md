---
name: Django Error Modeling
trigger: django error modeling, help with django error modeling, plan django error modeling, improve django error modeling, expert django error modeling
description: Expert-level guidance for django error modeling with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Django Error Modeling is an expert backend skill for Django. The focal concern is Error Modeling, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Django Error Modeling shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Django Error Modeling breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Django Error Modeling?
- Where does Django Error Modeling need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Django Error Modeling is unhealthy before customers do?
- What compatibility promise is implied by Django Error Modeling even if it was never written down?
...
- A contract note for Django Error Modeling that includes failure semantics.
...
- [ ] The contract for Django Error Modeling is explicit about success and failure behavior.
