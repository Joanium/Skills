---
name: Query Repair Minimal Repros
trigger: query repair minimal repros, help with query repair minimal repros, plan query repair minimal repros, improve query repair minimal repros, expert query repair minimal repros
description: Expert-level guidance for query repair minimal repros with action-oriented workflow, review criteria, and failure patterns.
---

Query Repair Minimal Repros is an expert coding workflow skill for Query Repair. The focal concern is Minimal Repros, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Query Repair Minimal Repros is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Query Repair Minimal Repros is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Query Repair Minimal Repros would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Query Repair Minimal Repros create a silent regression that tests may miss?
- How would the AI explain the safety of Query Repair Minimal Repros to a skeptical reviewer?
...
- A concise evidence note for Query Repair Minimal Repros.
...
- [ ] The evidence supporting Query Repair Minimal Repros is stronger than the assumptions.
