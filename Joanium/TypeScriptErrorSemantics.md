---
name: TypeScript Error Semantics
trigger: typescript error semantics, help with typescript error semantics, plan typescript error semantics, improve typescript error semantics, expert typescript error semantics
description: Expert-level guidance for typescript error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

TypeScript Error Semantics is an expert programming-language skill for TypeScript. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when TypeScript Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside TypeScript Error Semantics?
- Which abstraction in TypeScript Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in TypeScript Error Semantics first?
- How will TypeScript Error Semantics behave when multiple contributors evolve it independently?
- Which part of TypeScript Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for TypeScript Error Semantics.
- A failure-mode note that explains the sharp edges inside TypeScript Error Semantics.
...
- [ ] The main language-level risk in TypeScript Error Semantics is named explicitly.
...
- [ ] Future contributors can extend TypeScript Error Semantics without reverse-engineering hidden contracts.
