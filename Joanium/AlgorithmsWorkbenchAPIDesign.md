---
name: Algorithms Workbench API Design
trigger: algorithms workbench api design, help with algorithms workbench api design, plan algorithms workbench api design, improve algorithms workbench api design, expert algorithms workbench api design
description: Expert-level guidance for algorithms workbench api design with action-oriented workflow, review criteria, and failure patterns.
---

Algorithms Workbench API Design is an expert systems and deep-engineering skill for Algorithms Workbench. The focal concern is API Design, but the real objective is helping an AI reason about correctness, performance, and failure in technical domains where shallow intuition breaks down.

## When To Use

- Use this when Algorithms Workbench API Design involves constraints where correctness, timing, memory, or physical behavior dominate the design.
...
- Which resource or state boundary inside Algorithms Workbench API Design is least observable today?
- What hidden assumption in Algorithms Workbench API Design would break first under load, timing pressure, or hardware limits?
- How would the AI distinguish a correctness issue from a performance symptom in Algorithms Workbench API Design?
- What tooling gap makes Algorithms Workbench API Design harder to debug or verify than it should be?
- Which optimization target in Algorithms Workbench API Design is attractive but strategically secondary?
...
- A system model for Algorithms Workbench API Design naming resources, states, and key constraints.
...
- [ ] The operating envelope for Algorithms Workbench API Design is explicit.
