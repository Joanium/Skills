---
name: C++ Concurrency Patterns
trigger: c++ concurrency patterns, help with c++ concurrency patterns, plan c++ concurrency patterns, improve c++ concurrency patterns, expert c++ concurrency patterns
description: Expert-level guidance for c++ concurrency patterns with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

C++ Concurrency Patterns is an expert programming-language skill for C++. The focal concern is Concurrency Patterns, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when C++ Concurrency Patterns affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside C++ Concurrency Patterns?
- Which abstraction in C++ Concurrency Patterns becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in C++ Concurrency Patterns first?
- How will C++ Concurrency Patterns behave when multiple contributors evolve it independently?
- Which part of C++ Concurrency Patterns is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for C++ Concurrency Patterns.
- A failure-mode note that explains the sharp edges inside C++ Concurrency Patterns.
...
- [ ] The main language-level risk in C++ Concurrency Patterns is named explicitly.
...
- [ ] Future contributors can extend C++ Concurrency Patterns without reverse-engineering hidden contracts.
