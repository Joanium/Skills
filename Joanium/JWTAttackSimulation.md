---
name: JWT Attack Simulation
trigger: jwt attack simulation, help with jwt attack simulation, plan jwt attack simulation, improve jwt attack simulation, expert jwt attack simulation
description: Expert-level guidance for jwt attack simulation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

JWT Attack Simulation is an expert security skill for JWT. The focal concern is Attack Simulation, but the real bar is a control model that withstands attackers, operator mistakes, and messy exceptions.

## When To Use

- Use this when JWT Attack Simulation influences trust boundaries, privileged behavior, or post-incident accountability.
...
- What realistic attacker behavior would expose the weakest assumption in JWT Attack Simulation first?
- Which exception or emergency path makes JWT Attack Simulation less true in practice than in policy?
- What evidence would be missing if JWT Attack Simulation failed during a real incident?
- Which operator action could accidentally bypass the intended control?
- How does JWT Attack Simulation degrade when dependencies, keys, or identities become stale?
...
- A control model and threat note for JWT Attack Simulation.
...
- [ ] The attacker model for JWT Attack Simulation is realistic and current.
...
- [ ] The team can explain how JWT Attack Simulation fails and how it recovers.
