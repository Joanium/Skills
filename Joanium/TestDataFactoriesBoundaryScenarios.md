---
name: Test Data Factories Boundary Scenarios
trigger: test data factories boundary scenarios, help with test data factories boundary scenarios, plan test data factories boundary scenarios, improve test data factories boundary scenarios, expert test data factories boundary scenarios
description: Expert-level guidance for test data factories boundary scenarios with action-oriented workflow, review criteria, and failure patterns.
---

Test Data Factories Boundary Scenarios is an expert testing and quality skill for Test Data Factories. The focal concern is Boundary Scenarios, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Test Data Factories Boundary Scenarios needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Test Data Factories Boundary Scenarios is still untested even if coverage looks healthy?
- Which assertion in Test Data Factories Boundary Scenarios would stay green while the user-visible behavior regressed?
- How much of Test Data Factories Boundary Scenarios depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Test Data Factories Boundary Scenarios need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Test Data Factories Boundary Scenarios.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Test Data Factories Boundary Scenarios should mean.
...
- [ ] The key behavior protected by Test Data Factories Boundary Scenarios is explicit.
