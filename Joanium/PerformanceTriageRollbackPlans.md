---
name: Performance Triage Rollback Plans
trigger: performance triage rollback plans, help with performance triage rollback plans, plan performance triage rollback plans, improve performance triage rollback plans, expert performance triage rollback plans
description: Expert-level guidance for performance triage rollback plans with action-oriented workflow, review criteria, and failure patterns.
---

Performance Triage Rollback Plans is an expert coding workflow skill for Performance Triage. The focal concern is Rollback Plans, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Performance Triage Rollback Plans is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Performance Triage Rollback Plans is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Performance Triage Rollback Plans would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Performance Triage Rollback Plans create a silent regression that tests may miss?
- How would the AI explain the safety of Performance Triage Rollback Plans to a skeptical reviewer?
...
- A concise evidence note for Performance Triage Rollback Plans.
...
- [ ] The evidence supporting Performance Triage Rollback Plans is stronger than the assumptions.
