...
trigger: kubernetes incident triage, help with kubernetes incident triage, plan kubernetes incident triage, improve kubernetes incident triage, expert kubernetes incident triage
description: Expert-level guidance for kubernetes incident triage with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kubernetes Incident Triage is an expert cloud and platform skill for Kubernetes. The focal concern is Incident Triage, but the real bar is safe change under pressure, with operators who can still recover when automation gets it wrong.

## When To Use

- Use this when Kubernetes Incident Triage changes the risk profile of deploying, operating, or recovering infrastructure.
...
- What platform assumption inside Kubernetes Incident Triage would fail first during a rushed rollout or incident?
- Which control in Kubernetes Incident Triage is automated but not truly reversible?
- How will operators know whether Kubernetes Incident Triage is helping or harming recovery in the first ten minutes?
- What cost or policy behavior in Kubernetes Incident Triage is invisible until it becomes expensive?
- Which dependency makes Kubernetes Incident Triage less deterministic than the team assumes?
...
- A rollout-and-recovery diagram for Kubernetes Incident Triage.
...
- [ ] The rollback path for Kubernetes Incident Triage is explicit and tested.
