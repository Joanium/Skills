---
name: Agent Memory Repair Escalation Rules
trigger: agent memory repair escalation rules, help with agent memory repair escalation rules, plan agent memory repair escalation rules, improve agent memory repair escalation rules, expert agent memory repair escalation rules
description: Expert-level guidance for agent memory repair escalation rules with action-oriented workflow, review criteria, and failure patterns.
---

Agent Memory Repair Escalation Rules is an expert agent and tooling skill for Agent Memory Repair. The focal concern is Escalation Rules, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Agent Memory Repair Escalation Rules affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Agent Memory Repair Escalation Rules is still governed by intuition instead of a contract or label set?
- Which failure in Agent Memory Repair Escalation Rules would be hardest to detect before it compounds?
- How would Agent Memory Repair Escalation Rules behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Agent Memory Repair Escalation Rules?
...
- A contract and failure-label set for Agent Memory Repair Escalation Rules.
...
- [ ] The system behind Agent Memory Repair Escalation Rules is described more clearly than the prompt surface.
