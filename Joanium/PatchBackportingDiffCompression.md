---
name: Patch Backporting Diff Compression
trigger: patch backporting diff compression, help with patch backporting diff compression, plan patch backporting diff compression, improve patch backporting diff compression, expert patch backporting diff compression
description: Expert-level guidance for patch backporting diff compression with action-oriented workflow, review criteria, and failure patterns.
---

Patch Backporting Diff Compression is an expert repository and delivery skill for Patch Backporting. The focal concern is Diff Compression, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Patch Backporting Diff Compression determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Patch Backporting Diff Compression riskier than the diff itself suggests?
- Which reviewer would challenge Patch Backporting Diff Compression first, and what evidence would they need?
- How could Patch Backporting Diff Compression fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Patch Backporting Diff Compression would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Patch Backporting Diff Compression concretely?
...
- A blast-radius note for Patch Backporting Diff Compression.
...
- [ ] The repository-level blast radius of Patch Backporting Diff Compression is explicit.
