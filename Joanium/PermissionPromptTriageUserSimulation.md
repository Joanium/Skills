---
name: Permission Prompt Triage User Simulation
trigger: permission prompt triage user simulation, help with permission prompt triage user simulation, plan permission prompt triage user simulation, improve permission prompt triage user simulation, expert permission prompt triage user simulation
description: Expert-level guidance for permission prompt triage user simulation with action-oriented workflow, review criteria, and failure patterns.
---

Permission Prompt Triage User Simulation is an expert browser automation skill for Permission Prompt Triage. The focal concern is User Simulation, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Permission Prompt Triage User Simulation needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Permission Prompt Triage User Simulation unreliable across runs?
- Which timing or selector assumption in Permission Prompt Triage User Simulation is least stable under real conditions?
- How would Permission Prompt Triage User Simulation behave differently across browsers, profiles, or CI environments?
- What artifacts should Permission Prompt Triage User Simulation capture so the next failure is diagnosable instead of mysterious?
- Which part of Permission Prompt Triage User Simulation is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Permission Prompt Triage User Simulation.
...
- [ ] The state model behind Permission Prompt Triage User Simulation is explicit.
