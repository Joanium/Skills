...
trigger: rust concurrency patterns, help with rust concurrency patterns, plan rust concurrency patterns, improve rust concurrency patterns, expert rust concurrency patterns
description: Expert-level guidance for rust concurrency patterns with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rust Concurrency Patterns is an expert programming-language skill for Rust. The focal concern is Concurrency Patterns, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Rust Concurrency Patterns affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Rust Concurrency Patterns?
- Which abstraction in Rust Concurrency Patterns becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Rust Concurrency Patterns first?
- How will Rust Concurrency Patterns behave when multiple contributors evolve it independently?
- Which part of Rust Concurrency Patterns is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Rust Concurrency Patterns.
- A failure-mode note that explains the sharp edges inside Rust Concurrency Patterns.
...
- [ ] The main language-level risk in Rust Concurrency Patterns is named explicitly.
...
- [ ] Future contributors can extend Rust Concurrency Patterns without reverse-engineering hidden contracts.
