...
trigger: svelte component testing, help with svelte component testing, plan svelte component testing, improve svelte component testing, expert svelte component testing
description: Expert-level guidance for svelte component testing with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Svelte Component Testing is an expert frontend skill for Svelte. The focal concern is Component Testing, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Svelte Component Testing affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Svelte Component Testing make user state harder to reason about than it needs to be?
- Which interaction in Svelte Component Testing degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Svelte Component Testing would break if the DOM or state changed unexpectedly?
- How does Svelte Component Testing fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Svelte Component Testing looks correct in a mock but collapses under production behavior?
...
- A state transition map for Svelte Component Testing.
...
- [ ] The state model for Svelte Component Testing is clear enough to debug.
...
- [ ] Another engineer can extend Svelte Component Testing without guessing hidden UI contracts.
