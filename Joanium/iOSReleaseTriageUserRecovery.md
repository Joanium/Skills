---
name: iOS Release Triage User Recovery
trigger: ios release triage user recovery, help with ios release triage user recovery, plan ios release triage user recovery, improve ios release triage user recovery, expert ios release triage user recovery
description: Expert-level guidance for ios release triage user recovery with client delivery workflow, review criteria, and failure patterns.
---

iOS Release Triage User Recovery is an expert mobile and desktop delivery skill for iOS Release Triage. The focal concern is User Recovery, but the real objective is helping an AI ship client software that survives device constraints, stores, upgrades, and real user recovery paths.

## When To Use

- Use this when iOS Release Triage User Recovery affects whether client software remains trustworthy across devices, upgrades, and degraded conditions.
...
- Which user or device condition would expose the weakest assumption in iOS Release Triage User Recovery first?
- How will iOS Release Triage User Recovery behave during upgrade, rollback, or stale local state?
- What store, signing, or permission dependency could block or confuse iOS Release Triage User Recovery unexpectedly?
- How would support or operations diagnose iOS Release Triage User Recovery from telemetry instead of guesswork?
- What path inside iOS Release Triage User Recovery should help the user recover rather than merely fail loudly?
...
- A device and release risk note for iOS Release Triage User Recovery.
...
- [ ] The device, store, and upgrade assumptions behind iOS Release Triage User Recovery are explicit.
