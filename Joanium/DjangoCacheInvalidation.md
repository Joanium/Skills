---
name: Django Cache Invalidation
trigger: django cache invalidation, help with django cache invalidation, plan django cache invalidation, improve django cache invalidation, expert django cache invalidation
description: Expert-level guidance for django cache invalidation with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Django Cache Invalidation is an expert backend skill for Django. The focal concern is Cache Invalidation, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when Django Cache Invalidation shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside Django Cache Invalidation breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in Django Cache Invalidation?
- Where does Django Cache Invalidation need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether Django Cache Invalidation is unhealthy before customers do?
- What compatibility promise is implied by Django Cache Invalidation even if it was never written down?
...
- A contract note for Django Cache Invalidation that includes failure semantics.
...
- [ ] The contract for Django Cache Invalidation is explicit about success and failure behavior.
