---
name: Repository Audit Edge Case Sweeps
trigger: repository audit edge case sweeps, help with repository audit edge case sweeps, plan repository audit edge case sweeps, improve repository audit edge case sweeps, expert repository audit edge case sweeps
description: Expert-level guidance for repository audit edge case sweeps with action-oriented workflow, review criteria, and failure patterns.
---

Repository Audit Edge Case Sweeps is an expert coding workflow skill for Repository Audit. The focal concern is Edge Case Sweeps, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Repository Audit Edge Case Sweeps is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Repository Audit Edge Case Sweeps is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Repository Audit Edge Case Sweeps would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Repository Audit Edge Case Sweeps create a silent regression that tests may miss?
- How would the AI explain the safety of Repository Audit Edge Case Sweeps to a skeptical reviewer?
...
- A concise evidence note for Repository Audit Edge Case Sweeps.
...
- [ ] The evidence supporting Repository Audit Edge Case Sweeps is stronger than the assumptions.
