---
name: Visual Diff Triage Environment Parity
trigger: visual diff triage environment parity, help with visual diff triage environment parity, plan visual diff triage environment parity, improve visual diff triage environment parity, expert visual diff triage environment parity
description: Expert-level guidance for visual diff triage environment parity with action-oriented workflow, review criteria, and failure patterns.
---

Visual Diff Triage Environment Parity is an expert browser automation skill for Visual Diff Triage. The focal concern is Environment Parity, but the real objective is helping an AI automate real user behavior without collapsing under timing, state, or environment noise.

## When To Use

- Use this when Visual Diff Triage Environment Parity needs to survive realistic browser noise rather than only pass in ideal local runs.
...
- What hidden browser state makes Visual Diff Triage Environment Parity unreliable across runs?
- Which timing or selector assumption in Visual Diff Triage Environment Parity is least stable under real conditions?
- How would Visual Diff Triage Environment Parity behave differently across browsers, profiles, or CI environments?
- What artifacts should Visual Diff Triage Environment Parity capture so the next failure is diagnosable instead of mysterious?
- Which part of Visual Diff Triage Environment Parity is simulating clicks while failing to model user intent?
...
- A browser state and timing map for Visual Diff Triage Environment Parity.
...
- [ ] The state model behind Visual Diff Triage Environment Parity is explicit.
