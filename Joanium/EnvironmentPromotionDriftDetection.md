---
name: Environment Promotion Drift Detection
trigger: environment promotion drift detection, help with environment promotion drift detection, plan environment promotion drift detection, improve environment promotion drift detection, expert environment promotion drift detection
description: Expert-level guidance for environment promotion drift detection with action-oriented workflow, review criteria, and failure patterns.
---

Environment Promotion Drift Detection is an expert platform and automation skill for Environment Promotion. The focal concern is Drift Detection, but the real objective is helping an AI make infrastructure and operational change without creating invisible blast radius.

## When To Use

- Use this when Environment Promotion Drift Detection can affect deployment safety, service recovery, or operator trust in automation.
...
- What part of Environment Promotion Drift Detection could fail silently while the automation still reports success?
- Which rollback or recovery step inside Environment Promotion Drift Detection depends too much on tribal memory?
- How will operators know when Environment Promotion Drift Detection should stop, wait, or reverse course?
- Where does Environment Promotion Drift Detection blur access, policy, and execution boundaries dangerously?
...
- A change-and-recovery map for Environment Promotion Drift Detection.
...
- [ ] The rollback model for Environment Promotion Drift Detection is as real as the forward path.
