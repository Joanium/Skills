---
name: Rust Error Semantics
trigger: rust error semantics, help with rust error semantics, plan rust error semantics, improve rust error semantics, expert rust error semantics
description: Expert-level guidance for rust error semantics with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rust Error Semantics is an expert programming-language skill for Rust. The focal concern is Error Semantics, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Rust Error Semantics affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Rust Error Semantics?
- Which abstraction in Rust Error Semantics becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Rust Error Semantics first?
- How will Rust Error Semantics behave when multiple contributors evolve it independently?
- Which part of Rust Error Semantics is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Rust Error Semantics.
- A failure-mode note that explains the sharp edges inside Rust Error Semantics.
...
- [ ] The main language-level risk in Rust Error Semantics is named explicitly.
...
- [ ] Future contributors can extend Rust Error Semantics without reverse-engineering hidden contracts.
