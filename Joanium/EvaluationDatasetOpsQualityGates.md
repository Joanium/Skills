---
name: Evaluation Dataset Operations Quality Gates
trigger: evaluation dataset operations quality gates, help with evaluation dataset operations quality gates, plan evaluation dataset operations quality gates, improve evaluation dataset operations quality gates, expert evaluation dataset operations quality gates
description: Expert-level guidance for evaluation dataset operations quality gates with action-oriented workflow, review criteria, and failure patterns.
---

Evaluation Dataset Operations Quality Gates is an expert agent and tooling skill for Evaluation Dataset Operations. The focal concern is Quality Gates, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Evaluation Dataset Operations Quality Gates affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Evaluation Dataset Operations Quality Gates is still governed by intuition instead of a contract or label set?
- Which failure in Evaluation Dataset Operations Quality Gates would be hardest to detect before it compounds?
- How would Evaluation Dataset Operations Quality Gates behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Evaluation Dataset Operations Quality Gates?
...
- A contract and failure-label set for Evaluation Dataset Operations Quality Gates.
...
- [ ] The system behind Evaluation Dataset Operations Quality Gates is described more clearly than the prompt surface.
