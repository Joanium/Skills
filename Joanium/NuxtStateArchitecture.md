---
name: Nuxt State Architecture
trigger: nuxt state architecture, help with nuxt state architecture, plan nuxt state architecture, improve nuxt state architecture, expert nuxt state architecture
description: Expert-level guidance for nuxt state architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Nuxt State Architecture is an expert frontend skill for Nuxt. The focal concern is State Architecture, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Nuxt State Architecture affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Nuxt State Architecture make user state harder to reason about than it needs to be?
- Which interaction in Nuxt State Architecture degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Nuxt State Architecture would break if the DOM or state changed unexpectedly?
- How does Nuxt State Architecture fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Nuxt State Architecture looks correct in a mock but collapses under production behavior?
...
- A state transition map for Nuxt State Architecture.
...
- [ ] The state model for Nuxt State Architecture is clear enough to debug.
...
- [ ] Another engineer can extend Nuxt State Architecture without guessing hidden UI contracts.
