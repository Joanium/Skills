---
name: LLM Cost Controls Governance
trigger: llm cost controls governance, help with llm cost controls governance, plan llm cost controls governance, improve llm cost controls governance, expert llm cost controls governance
description: Expert-level guidance for llm cost controls governance, including decision criteria, workflow, tradeoffs, signals, review checkpoints, and failure modes.
---

LLM Cost Controls Governance is an expert-level AI systems skill for making model-assisted behavior reliable, auditable, and economically sane in production. The emphasis here is on ownership, policy, exception handling, and auditability so the domain does not decay into ad hoc decisions.

## When To Use

- Use this when LLM Cost Controls Governance influences user-visible output, automation, or safety-sensitive decisions.
...
- Who sets the policy for LLM Cost Controls Governance, and who is allowed to grant exceptions?
...
- A policy document for LLM Cost Controls Governance with explicit scope.
