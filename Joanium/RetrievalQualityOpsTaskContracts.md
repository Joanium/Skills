---
name: Retrieval Quality Operations Task Contracts
trigger: retrieval quality operations task contracts, help with retrieval quality operations task contracts, plan retrieval quality operations task contracts, improve retrieval quality operations task contracts, expert retrieval quality operations task contracts
description: Expert-level guidance for retrieval quality operations task contracts with action-oriented workflow, review criteria, and failure patterns.
---

Retrieval Quality Operations Task Contracts is an expert agent and tooling skill for Retrieval Quality Operations. The focal concern is Task Contracts, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Retrieval Quality Operations Task Contracts affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Retrieval Quality Operations Task Contracts is still governed by intuition instead of a contract or label set?
- Which failure in Retrieval Quality Operations Task Contracts would be hardest to detect before it compounds?
- How would Retrieval Quality Operations Task Contracts behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Retrieval Quality Operations Task Contracts?
...
- A contract and failure-label set for Retrieval Quality Operations Task Contracts.
...
- [ ] The system behind Retrieval Quality Operations Task Contracts is described more clearly than the prompt surface.
