---
name: Snapshot Testing Operations Fixture Hygiene
trigger: snapshot testing operations fixture hygiene, help with snapshot testing operations fixture hygiene, plan snapshot testing operations fixture hygiene, improve snapshot testing operations fixture hygiene, expert snapshot testing operations fixture hygiene
description: Expert-level guidance for snapshot testing operations fixture hygiene with action-oriented workflow, review criteria, and failure patterns.
---

Snapshot Testing Operations Fixture Hygiene is an expert testing and quality skill for Snapshot Testing Operations. The focal concern is Fixture Hygiene, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Snapshot Testing Operations Fixture Hygiene needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Snapshot Testing Operations Fixture Hygiene is still untested even if coverage looks healthy?
- Which assertion in Snapshot Testing Operations Fixture Hygiene would stay green while the user-visible behavior regressed?
- How much of Snapshot Testing Operations Fixture Hygiene depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Snapshot Testing Operations Fixture Hygiene need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Snapshot Testing Operations Fixture Hygiene.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Snapshot Testing Operations Fixture Hygiene should mean.
...
- [ ] The key behavior protected by Snapshot Testing Operations Fixture Hygiene is explicit.
