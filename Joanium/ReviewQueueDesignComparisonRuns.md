---
name: Review Queue Design Comparison Runs
trigger: review queue design comparison runs, help with review queue design comparison runs, plan review queue design comparison runs, improve review queue design comparison runs, expert review queue design comparison runs
description: Expert-level guidance for review queue design comparison runs with action-oriented workflow, review criteria, and failure patterns.
---

Review Queue Design Comparison Runs is an expert agent and tooling skill for Review Queue Design. The focal concern is Comparison Runs, but the real objective is helping an AI system use tools, prompts, memory, and review loops without drifting into fragile automation.

## When To Use

- Use this when Review Queue Design Comparison Runs affects whether an AI system remains useful once prompts, tools, and real operational noise are involved.
...
- What part of Review Queue Design Comparison Runs is still governed by intuition instead of a contract or label set?
- Which failure in Review Queue Design Comparison Runs would be hardest to detect before it compounds?
- How would Review Queue Design Comparison Runs behave if the tool was slow, retrieval was weak, or the prompt version drifted?
- What should escalate to a human because the system is structurally unreliable there?
- Which comparison run would actually falsify the current design confidence in Review Queue Design Comparison Runs?
...
- A contract and failure-label set for Review Queue Design Comparison Runs.
...
- [ ] The system behind Review Queue Design Comparison Runs is described more clearly than the prompt surface.
