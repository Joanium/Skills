---
name: FastAPI Rate Limit Design
trigger: fastapi rate limit design, help with fastapi rate limit design, plan fastapi rate limit design, improve fastapi rate limit design, expert fastapi rate limit design
description: Expert-level guidance for fastapi rate limit design with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

FastAPI Rate Limit Design is an expert backend skill for FastAPI. The focal concern is Rate Limit Design, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when FastAPI Rate Limit Design shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside FastAPI Rate Limit Design breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in FastAPI Rate Limit Design?
- Where does FastAPI Rate Limit Design need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether FastAPI Rate Limit Design is unhealthy before customers do?
- What compatibility promise is implied by FastAPI Rate Limit Design even if it was never written down?
...
- A contract note for FastAPI Rate Limit Design that includes failure semantics.
...
- [ ] The contract for FastAPI Rate Limit Design is explicit about success and failure behavior.
