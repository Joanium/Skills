---
name: Kotlin Type Architecture
trigger: kotlin type architecture, help with kotlin type architecture, plan kotlin type architecture, improve kotlin type architecture, expert kotlin type architecture
description: Expert-level guidance for kotlin type architecture with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Kotlin Type Architecture is an expert programming-language skill for Kotlin. The focal concern is Type Architecture, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Kotlin Type Architecture affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Kotlin Type Architecture?
- Which abstraction in Kotlin Type Architecture becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Kotlin Type Architecture first?
- How will Kotlin Type Architecture behave when multiple contributors evolve it independently?
- Which part of Kotlin Type Architecture is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Kotlin Type Architecture.
- A failure-mode note that explains the sharp edges inside Kotlin Type Architecture.
...
- [ ] The main language-level risk in Kotlin Type Architecture is named explicitly.
...
- [ ] Future contributors can extend Kotlin Type Architecture without reverse-engineering hidden contracts.
