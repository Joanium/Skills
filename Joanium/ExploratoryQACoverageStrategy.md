---
name: Exploratory QA Coverage Strategy
trigger: exploratory qa coverage strategy, help with exploratory qa coverage strategy, plan exploratory qa coverage strategy, improve exploratory qa coverage strategy, expert exploratory qa coverage strategy
description: Expert-level guidance for exploratory qa coverage strategy with action-oriented workflow, review criteria, and failure patterns.
---

Exploratory QA Coverage Strategy is an expert testing and quality skill for Exploratory QA. The focal concern is Coverage Strategy, but the real objective is helping an AI create evidence that code is correct under change, not merely green under ideal conditions.

## When To Use

- Use this when Exploratory QA Coverage Strategy needs to prove behavior under realistic change and failure conditions rather than just increase test count.
...
- What uncertainty inside Exploratory QA Coverage Strategy is still untested even if coverage looks healthy?
- Which assertion in Exploratory QA Coverage Strategy would stay green while the user-visible behavior regressed?
- How much of Exploratory QA Coverage Strategy depends on mocks, fixtures, or timing assumptions that may be lying?
- What test failure pattern would be most actionable for a future AI debugging pass?
- Where does Exploratory QA Coverage Strategy need stronger negative cases rather than more positive cases?
...
- A behavior protection map for Exploratory QA Coverage Strategy.
- A test design note covering fixtures, timing, and oracle choice.
- A failure matrix for the important negative scenarios.
- A triage guide for what failing signals in Exploratory QA Coverage Strategy should mean.
...
- [ ] The key behavior protected by Exploratory QA Coverage Strategy is explicit.
