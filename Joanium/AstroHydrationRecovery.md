---
name: Astro Hydration Recovery
trigger: astro hydration recovery, help with astro hydration recovery, plan astro hydration recovery, improve astro hydration recovery, expert astro hydration recovery
description: Expert-level guidance for astro hydration recovery with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Astro Hydration Recovery is an expert frontend skill for Astro. The focal concern is Hydration Recovery, but the real task is shaping user-perceived behavior under real browser and device constraints.

## When To Use

- Use this when Astro Hydration Recovery affects a critical journey where users will notice latency, confusion, or inconsistency immediately.
...
- Where does Astro Hydration Recovery make user state harder to reason about than it needs to be?
- Which interaction in Astro Hydration Recovery degrades first under latency, stale data, or partial failure?
- What accessibility behavior in Astro Hydration Recovery would break if the DOM or state changed unexpectedly?
- How does Astro Hydration Recovery fail when routing, hydration, or rendering order becomes non-ideal?
- What part of Astro Hydration Recovery looks correct in a mock but collapses under production behavior?
...
- A state transition map for Astro Hydration Recovery.
...
- [ ] The state model for Astro Hydration Recovery is clear enough to debug.
...
- [ ] Another engineer can extend Astro Hydration Recovery without guessing hidden UI contracts.
