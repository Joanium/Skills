---
name: Merge Conflict Resolution Reviewer Alignment
trigger: merge conflict resolution reviewer alignment, help with merge conflict resolution reviewer alignment, plan merge conflict resolution reviewer alignment, improve merge conflict resolution reviewer alignment, expert merge conflict resolution reviewer alignment
description: Expert-level guidance for merge conflict resolution reviewer alignment with action-oriented workflow, review criteria, and failure patterns.
---

Merge Conflict Resolution Reviewer Alignment is an expert repository and delivery skill for Merge Conflict Resolution. The focal concern is Reviewer Alignment, but the real objective is helping an AI navigate large code changes without losing causal clarity, reviewer trust, or rollback safety.

## When To Use

- Use this when Merge Conflict Resolution Reviewer Alignment determines whether a large or risky code change can be reasoned about cleanly before it lands.
...
- What repository-level dependency makes Merge Conflict Resolution Reviewer Alignment riskier than the diff itself suggests?
- Which reviewer would challenge Merge Conflict Resolution Reviewer Alignment first, and what evidence would they need?
- How could Merge Conflict Resolution Reviewer Alignment fail after rebasing, backporting, or partial cherry-picking?
- What hidden coupling in Merge Conflict Resolution Reviewer Alignment would only become obvious during staging or release?
- How will the AI explain the intended blast radius of Merge Conflict Resolution Reviewer Alignment concretely?
...
- A blast-radius note for Merge Conflict Resolution Reviewer Alignment.
...
- [ ] The repository-level blast radius of Merge Conflict Resolution Reviewer Alignment is explicit.
