---
name: Repository Audit Regression Guards
trigger: repository audit regression guards, help with repository audit regression guards, plan repository audit regression guards, improve repository audit regression guards, expert repository audit regression guards
description: Expert-level guidance for repository audit regression guards with action-oriented workflow, review criteria, and failure patterns.
---

Repository Audit Regression Guards is an expert coding workflow skill for Repository Audit. The focal concern is Regression Guards, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Repository Audit Regression Guards is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Repository Audit Regression Guards is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Repository Audit Regression Guards would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Repository Audit Regression Guards create a silent regression that tests may miss?
- How would the AI explain the safety of Repository Audit Regression Guards to a skeptical reviewer?
...
- A concise evidence note for Repository Audit Regression Guards.
...
- [ ] The evidence supporting Repository Audit Regression Guards is stronger than the assumptions.
