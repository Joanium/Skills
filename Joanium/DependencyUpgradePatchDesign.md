---
name: Dependency Upgrade Patch Design
trigger: dependency upgrade patch design, help with dependency upgrade patch design, plan dependency upgrade patch design, improve dependency upgrade patch design, expert dependency upgrade patch design
description: Expert-level guidance for dependency upgrade patch design with action-oriented workflow, review criteria, and failure patterns.
---

Dependency Upgrade Patch Design is an expert coding workflow skill for Dependency Upgrade. The focal concern is Patch Design, but the real objective is helping an AI move from messy code context to a safe, reviewable, high-leverage change.

## When To Use

- Use this when Dependency Upgrade Patch Design is the difference between a plausible code edit and a change that can survive review, testing, and rollback.
...
- What part of Dependency Upgrade Patch Design is known from evidence versus inferred from incomplete context?
- Which file, boundary, or runtime path inside Dependency Upgrade Patch Design would hurt most if misunderstood?
- What is the smallest proof that the chosen change direction is correct?
- Where could Dependency Upgrade Patch Design create a silent regression that tests may miss?
- How would the AI explain the safety of Dependency Upgrade Patch Design to a skeptical reviewer?
...
- A concise evidence note for Dependency Upgrade Patch Design.
...
- [ ] The evidence supporting Dependency Upgrade Patch Design is stronger than the assumptions.
