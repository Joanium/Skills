---
name: REST API Contract Evolution
trigger: rest api contract evolution, help with rest api contract evolution, plan rest api contract evolution, improve rest api contract evolution, expert rest api contract evolution
description: Expert-level guidance for rest api contract evolution with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

REST API Contract Evolution is an expert backend skill for REST API. The focal concern is Contract Evolution, but the real task is creating server behavior that remains understandable under load, retries, and partial failure.

## When To Use

- Use this when REST API Contract Evolution shapes correctness, reliability, or compatibility for a backend surface that other systems depend on.
...
- What assumption inside REST API Contract Evolution breaks first under concurrency, retries, or partial failure?
- Which client or downstream system will feel contract ambiguity most painfully in REST API Contract Evolution?
- Where does REST API Contract Evolution need stronger semantics: authentication, errors, ordering, or isolation?
- How will operators know whether REST API Contract Evolution is unhealthy before customers do?
- What compatibility promise is implied by REST API Contract Evolution even if it was never written down?
...
- A contract note for REST API Contract Evolution that includes failure semantics.
...
- [ ] The contract for REST API Contract Evolution is explicit about success and failure behavior.
