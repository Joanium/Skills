---
name: Swift Package Publishing
trigger: swift package publishing, help with swift package publishing, plan swift package publishing, improve swift package publishing, expert swift package publishing
description: Expert-level guidance for swift package publishing with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Swift Package Publishing is an expert programming-language skill for Swift. The focal concern is Package Publishing, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Swift Package Publishing affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Swift Package Publishing?
- Which abstraction in Swift Package Publishing becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Swift Package Publishing first?
- How will Swift Package Publishing behave when multiple contributors evolve it independently?
- Which part of Swift Package Publishing is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Swift Package Publishing.
- A failure-mode note that explains the sharp edges inside Swift Package Publishing.
...
- [ ] The main language-level risk in Swift Package Publishing is named explicitly.
...
- [ ] Future contributors can extend Swift Package Publishing without reverse-engineering hidden contracts.
