---
name: Rust Debug Workflows
trigger: rust debug workflows, help with rust debug workflows, plan rust debug workflows, improve rust debug workflows, expert rust debug workflows
description: Expert-level guidance for rust debug workflows with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Rust Debug Workflows is an expert programming-language skill for Rust. The focal concern is Debug Workflows, but the real bar is production code that survives refactors, scale, and peer review.

## When To Use

- Use this when Rust Debug Workflows affects correctness, readability, and long-term maintainability in the same code path.
...
- What language feature is carrying the real complexity inside Rust Debug Workflows?
- Which abstraction in Rust Debug Workflows becomes harder to debug as the codebase grows?
- What runtime failure would reveal a weak design choice in Rust Debug Workflows first?
- How will Rust Debug Workflows behave when multiple contributors evolve it independently?
- Which part of Rust Debug Workflows is optimized for elegance instead of team comprehension?
...
- A minimal before-and-after code sample for Rust Debug Workflows.
- A failure-mode note that explains the sharp edges inside Rust Debug Workflows.
...
- [ ] The main language-level risk in Rust Debug Workflows is named explicitly.
...
- [ ] Future contributors can extend Rust Debug Workflows without reverse-engineering hidden contracts.
