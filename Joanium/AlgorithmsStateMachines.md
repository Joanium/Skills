---
name: Algorithms State Machines
trigger: algorithms state machines, help with algorithms state machines, plan algorithms state machines, improve algorithms state machines, expert algorithms state machines
description: Expert-level guidance for algorithms state machines with domain-specific heuristics, workflow, review criteria, and failure patterns.
---

Algorithms State Machines is an expert systems skill for Algorithms. The focal concern is State Machines, but the real bar is technical work that remains correct, observable, and optimizable in hard conditions.

## When To Use

- Use this when Algorithms State Machines is constrained by performance, correctness, or hardware realities that typical app-development patterns ignore.
...
- What hidden assumption inside Algorithms State Machines would break first under scale, timing, or hardware stress?
- Which state or resource in Algorithms State Machines is hardest to observe when behavior goes wrong?
- How does Algorithms State Machines trade correctness, throughput, and recoverability today?
- What tooling gap makes Algorithms State Machines harder to reason about than it should be?
- Which optimization target inside Algorithms State Machines is attractive but strategically secondary?
...
- A system model for Algorithms State Machines that names resources, state, and failure boundaries.
...
- [ ] The operating envelope for Algorithms State Machines is explicit.
